# Pendências de pesquisa e entrega

- [x] Concluir a leitura da dinâmica de preços, volume potencial e diferenças territoriais na locação residencial.
- [x] Investigar práticas de operação digital que alteram o papel de imobiliárias, proprietários e interessados.
- [x] Projetar o cadastro robusto em etapas: interesse inicial, qualificação, dossiê documental e contratação.
- [x] Validar campos, consentimentos, retenção e segurança à luz da LGPD e das garantias locatícias.
...
- [x] Mapear as fontes, recortes e indicadores do mercado de vendas urbanas e lotes urbanos.
- [x] Investigar demanda, preços, lançamentos, estoque e particularidades dos lotes urbanos.
- [x] Definir o cadastro progressivo de proprietários pessoa física, com documentação e evidências por etapa.
- [x] Definir o cadastro progressivo de proprietários pessoa jurídica, incluindo representação, poderes e documentação societária.
...
## Desenvolvimento — Etapa 0: fundação administrativa
...
- [x] Alternar blocos de auditoria entre Vendas Urbanas e Locação, sem permanecer concentrado em um único setor enquanto houver superfícies pendentes na outra coluna.
- [x] Registrar em cada bloco alternado a superfície, o controle, a evidência, o risco, a lacuna e a restauração segura do ambiente.
- [x] Exigir, em cada alternância, pelo menos uma evidência nova ou uma lacuna nova; quando não houver avanço seguro, registrar bloqueio e trocar de superfície sem repetir o mesmo teste.
- [x] Manter uma trilha de avanço que impeça retorno circular a controles já demonstrados, salvo validação de contradição ou correção de evidência incompleta.
...
## Criação do CRM: Vendas Urbanas — classificação interna de agenda

- [x] Consolidar o contrato de classificação de finalidade interna para agenda urbana em rascunho, sem comunicação, visita confirmada, calendário externo, proposta, reserva, contrato ou financeiro.
- [x] Preparar e aplicar migration de classificação contextual de agenda com RLS, grants mínimos, funções server-side e trilha redigida.
- [x] Implementar APIs protegidas para registrar e ler a classificação minimizada, sem acesso direto do navegador às tabelas.
...
- [x] Implementar a fronteira server-side de upload e metadados privados com isolamento por contexto.
- [x] Exigir sessão, subject Supabase e MFA recente antes de processar multipart na rota privada.
...
- [x] Endurecer a leitura privada do armazenamento para impedir acesso por chave previsível antes de habilitar upload.
- [x] Bloquear explicitamente o prefixo privado no proxy de armazenamento antes de aceitar anexos reais.
- [x] Validar contexto e intenção por RPC antes de gravar bytes no armazenamento privado.
...
- [x] Verificar no histórico a evidência explícita de checkpoint e material das sete entregas estratégicas pendentes antes de encerrá-las. A156 confirmou um marco já reconciliado e seis lacunas documentais; nenhuma foi encerrada sem prova individual.
...
## Experiência Locação: jornada operacional sem dados de negócio

- [x] Auditar a entrada de Locação para reduzir a aparência de formulário técnico e ordenar as jornadas de administração e locação.
...
## Locação: validação local do código interno de agenda
- [x] Auditar o código interno opcional e o contrato server-side vigente.
- [x] Definir normalização e validação local quando o código for informado, sem substituir o servidor.
- [x] Exibir feedback acessível e impedir somente a mutação localmente inválida.
- [x] Validar comportamento, testes, tipagem, build e artefatos ZIP/HTML saneados.

## Locação: validação local da origem em código
- [x] Auditar a origem em código e o contrato server-side vigente.
- [x] Definir normalização e validação local do padrão permitido, sem substituir o servidor.
- [x] Exibir feedback acessível e impedir somente a mutação localmente inválida.
- [x] Validar comportamento, testes, tipagem, build e artefatos ZIP/HTML saneados.

## Vendas Urbanas: validação local da preferência em código
- [x] Auditar a preferência em código e o contrato server-side vigente.
- [x] Definir normalização e validação local quando o código for informado, sem substituir o servidor.
- [x] Exibir feedback acessível e impedir somente a mutação localmente inválida.
- [x] Validar comportamento, testes, tipagem, build e artefatos ZIP/HTML saneados.

## Ativos Urbanos: validação local do motivo de bloqueio
- [x] Auditar o motivo em código e o contrato server-side vigente.
- [x] Definir normalização e validação local quando o estado for bloqueado, sem substituir o servidor.
- [x] Exibir feedback acessível e impedir somente a mutação localmente inválida.
- [x] Validar comportamento, testes, tipagem, build e artefatos ZIP/HTML saneados.

## Locação: validação local do código de escopo administrativo
- [x] Auditar o código interno opcional e o contrato server-side vigente.
- [x] Definir normalização e validação local quando o código for informado, sem substituir o servidor.
- [x] Exibir feedback acessível e impedir somente a mutação localmente inválida.
- [x] Validar comportamento, testes, tipagem, build e artefatos ZIP/HTML saneados.

## Vendas Urbanas: validação local do motivo de encerramento
- [x] Auditar o motivo em código e o contrato server-side vigente.
...
## Vendas Urbanas: validação local do motivo de agenda
- [x] Auditar o motivo em código e o contrato server-side vigente.
...
## Locação: validação local do motivo de agenda
- [x] Auditar o motivo em código e o contrato server-side vigente.
- [x] Definir normalização e validação local quando cancelamento ou não realização forem selecionados, sem substituir o servidor.
- [x] Exibir feedback acessível e impedir somente a mutação localmente inválida.
- [x] Validar comportamento, testes, tipagem, build e artefatos ZIP/HTML saneados.
...
## Exportação controlada e importação de clientes
- [x] Mapear os painéis e relatórios aptos a exportação sem revelar dados pessoais, financeiros, contratuais ou identificadores técnicos fora do escopo autorizado. **A192:** Central SUPER ADM, ADM, Clientes Loteadora, Vendas Urbanas, Locação e prévia de importação receberam somente resumos redigidos.
- [x] Implementar exportações de leitura em CSV e PDF com conteúdo redigido, metadados de contexto e bloqueio fail-closed sem identidade, organização ou escopo autorizado. **A192:** geração local no navegador, com botões bloqueados fora do contexto autorizado.
- [x] Criar jornada de importação de clientes em CSV com modelo baixável, análise local, validação de colunas, prévia sem persistência e relatório de inconsistências. **A192:** a matriz aceita exclusivamente Nome, Tipo e Perfil, com limite de 512 KB e 200 linhas.
...
## Cadastro governado do empreendimento Vista do Sol
- [x] Extrair dos materiais recebidos somente a estrutura física, estados do empreendimento e pendências documentais, sem expor pessoas, contatos, vendas já realizadas ou valores em relatórios de trabalho. **A202:** leitura saneada registrou somente estrutura, etapa declarada e pendências.
- [x] Comparar a matriz real de Quadras e Lotes com o construtor atual e registrar lacunas de tipologia, geometria, área, frente, profundidade, situação estrutural e documentação pendente. **A202:** lacunas estruturais foram implementadas sem dados comerciais.
- [x] Incluir uma prévia local de matriz física que leia somente Quadra, Lote e Área de fonte escolhida, descarte status comercial, valores e pessoas e aponte divergência de total antes de qualquer gravação. **A202:** a prévia autenticada confirmou 14 Quadras e 164 Lotes, mantendo a divergência de um Lote para reconciliação.
- [x] Adicionar atributos físicos opcionais por Quadra e Lote — setor/trecho, tipologia, área, frente, profundidade e posição — sob limites de validação e sem dados comerciais. **A202:** contratos, serviço, RPC e interface atualizados.
- [x] Adicionar dossiê de pendências padronizadas por prefeitura, cartório, jurídico, obras, ambiente e material técnico, com estado de trabalho e sem declarar aprovação por inferência. **A202:** dossiê estruturado e auditável implementado.
...
- [x] Exibir orientação e ação segura de nova verificação quando o estado do provedor não puder ser confirmado, evitando uma tela sem próximo passo. **A204:** estado de falha apresenta Verificar novamente e Validar contexto, sem liberar comandos.
- [x] Atualizar a cobertura da ordem de navegação para incluir Segurança e MFA entre os painéis administrativos e as colunas operacionais. **A204:** rota incluída entre os painéis administrativos e as colunas permanecem ordenadas.
- [x] Mapear o estado atual de sessão, fatores TOTP e rotas de identidade para explicar o bloqueio de criação sem expor segredos, QR codes ou chaves. **A204:** mapeamento alinhado à atestação AAL2/TOTP do servidor e à API oficial do provedor.
- [x] Criar uma rota Segurança e MFA que mostre estado redigido e permita iniciar inscrição ou desafio TOTP exclusivamente pelo provedor autenticado. **A204:** rota, navegação de Plataforma e item do perfil implementados.
- [x] Exigir confirmação local antes da inscrição, apresentar QR/segredo apenas à pessoa autenticada e nunca persistir, registrar ou devolver o segredo ao servidor. **A204:** QR/segredo apenas em estado de memória do navegador e cancelamento via provedor.
...
- [x] Validar em sessão autenticada a inscrição ou revalidação TOTP feita pela própria pessoa e salvar checkpoint sem publicar ou alterar alçadas. **A205:** a confirmação visual de MFA recente foi recebida; não houve inscrição automatizada, alteração de alçada ou publicação.

## Reconstrução da Identificação de Loteamentos
- [x] Auditar a tela atual do Módulo 01 e documentar lacunas de campos, dossiê, pendências e encaminhamentos, sem ler ou alterar informações de loteamentos existentes. **A206:** diagnóstico registrou a separação indevida de campos, ausência de dossiê inicial visível e completude superficial, sem consultar registros.
- [x] Definir seções de identificação cadastral, territorial, registral, urbanística, responsáveis internos e pendências, preservando a proibição de preço, venda, cliente, contrato, financeiro e dados pessoais. **A206:** os campos persistidos são modalidade, território amplo, uso predominante e nota interna; situações jurídicas e documentais ficam como estados de trabalho, sem números oficiais ou dados de terceiros.
- [x] Reorganizar o Módulo 01 como dossiê de identificação progressivo, com indicação de completude, estados de pendência e navegação clara para estrutura e documentos. **A206:** quatro seções agora ficam no mesmo módulo e a completude diferencia base criada de dossiê-base preenchido.
- [x] Tornar o encaminhamento de documento inicial visível desde a Identificação, mantendo o upload real no módulo de Documentos, sempre protegido por subject ativo, MFA recente, contexto, grant, finalidade e auditoria redigida. **A206:** CTA de documento inicial aparece no fim do Módulo 01, fica bloqueado antes do rascunho e abre o dossiê privado após o salvamento autorizado.
- [x] Cobrir validações, bloqueios de MFA, ausência de contexto, rotas, responsividade e ausência de campos comerciais com testes e revisão visual. **A206:** 17 testes dirigidos, 207 arquivos/499 testes na suíte, tipagem, build, diff e capturas desktop/móvel aprovados; gravação e upload reais continuam intencionalmente bloqueados até MFA pessoal.
- [x] Documentar e gerar ZIP e HTML saneados sem cadastrar, editar, anexar ou publicar dados reais. **A206:** documentação atualizada; ZIP e HTML regenerados após a validação integral, sem `.env`, dependências, logs, build, documentos históricos, checklist, identificadores reais conhecidos, credenciais ou endpoints de infraestrutura.
- [x] Salvar checkpoint da A206 sem cadastrar, editar, anexar ou publicar dados reais. **A206:** checkpoint de preservação salvo após validação integral, revisão visual e saneamento dos artefatos.

## Execução autorizada: rascunho estrutural do Vista do Sol
- [x] Revalidar em leitura, imediatamente antes do comando, subject ativo, organização ativa não demonstrativa, contexto Loteadora, finalidade autorizada, grant e atestação de MFA recente. **A207:** a sessão apresentou MFA recente reconhecido, contexto Loteadora autorizado e o servidor aceitou a criação inicial.
- [x] Criar exclusivamente o rascunho estrutural com a referência autorizada, sem preço, venda, cliente, corretor, contrato, financeiro ou documento real. **A207:** o servidor confirmou a criação de um novo rascunho separado; nenhum registro anterior foi alterado.
- [x] Aplicar exclusivamente a matriz física aprovada de 14 Quadras e 164 Lotes ao novo rascunho, sem preço, venda, cliente, corretor, contrato, financeiro ou documento real. **A207:** a matriz foi conciliada localmente e o servidor confirmou a aplicação no rascunho novo; a leitura posterior apresentou a mesma estrutura agregada por Quadra.
- [x] Registrar a pendência explícita de reconciliação do 165º Lote no dossiê interno permitido, sem inferir numeração, área, situação comercial ou dado documental. **A207:** a nota de identificação foi conferida em leitura e permanece limitada à necessidade de conciliação antes de qualquer ampliação da matriz.
- [x] Persistir uma única vez os enquadramentos estruturais autorizados que completam o dossiê-base, sem alterar a matriz, anexar documento ou incluir conteúdo comercial, financeiro ou pessoal. **A207:** após a revalidação pessoal, o servidor aceitou o único salvamento e a auditoria permaneceu redigida.
- [x] Revalidar em leitura a contagem agregada, as Quadras, os Lotes e a ausência de campos comerciais; interromper imediatamente se a matriz não coincidir com a autorização. **A207:** a recarga confirmou o dossiê-base completo e a mesma matriz de 14 Quadras e 164 Lotes; a interface continua sem disponibilidade, venda, contrato, preço, cobrança, pagamento ou repasse.
- [x] Gerar documentação redigida, ZIP e HTML saneados e testes proporcionais, sem incluir referências reais, dados pessoais ou publicação. **A207:** testes integrais aprovados, documentação redigida atualizada, ZIP com 802 arquivos sem diretórios ou arquivos excluídos e HTML autônomo conferido sem referências reais, credenciais ou endpoints de infraestrutura.
- [x] Salvar checkpoint A207 sem publicar e sem criar dados fora do cadastro estrutural autorizado. **A207:** consolidação preparada após as validações e os artefatos saneados; o checkpoint preservará somente este escopo autorizado.

## Correção de seleção após novo rascunho
- [x] Corrigir o retorno da interface para selecionar automaticamente o rascunho recém-criado, sem abrir, editar ou alterar rascunhos anteriores. **A207:** a criação passa a fixar a escolha explícita e a limpar somente o filtro local antes de invalidar a lista.
- [x] Cobrir o retorno de criação e a seleção do novo rascunho com teste dirigido antes de aplicar qualquer matriz física. **A207:** teste dirigido do estúdio aprovado com 12 verificações, incluindo o retorno ao novo rascunho.

## Correção de atualização da Identificação após MFA reconhecido
- [x] Diagnosticar por que a atualização de Identificação foi negada após criação e aplicação de matriz aceitas pelo mesmo contexto, sem repetir a mutação nem enfraquecer MFA, contexto, grant ou policy. **A207:** a negação fail-closed ocorreu antes da RPC de negócio porque a ponte tRPC mantinha token Supabase em memória após a confirmação TOTP.
- [x] Corrigir somente a causa confirmada e cobrir o caso de atualização após criação estrutural com teste dirigido. **A207:** a ponte agora consulta sessão fresca antes de cada comando protegido e os testes dirigidos de ponte e MFA foram aprovados.
- [x] Persistir uma única vez a identificação estrutural já conferida, validar em leitura e confirmar que não houve conteúdo comercial, financeiro, documental ou pessoal. **A207:** após MFA renovado, o servidor aceitou o salvamento; a recarga confirmou completude e preservou a matriz física sem novos módulos de negócio.
- [x] Alinhar a tela de Segurança e MFA ao limite de recência TOTP aplicado no servidor, exibindo revalidação quando o token reforçado estiver vencido e sem expor claims, códigos ou segredos. **A207:** AAL2 sem TOTP recente leva à revalidação visual; QR, segredo, código e claims continuam fora de logs, auditoria e mensagens.

## Reconciliação segura da divergência estrutural
- [x] Reunir e comparar somente as evidências físicas já autorizadas, sem acessar status comercial, valores, vendas, dados pessoais ou documentos reais. **A208:** a matriz persistida de 14 Quadras/164 Lotes coincide com a única fonte física revisada; o total declarado de 165 permanece uma divergência sem prova individual.
- [x] Tornar a pendência de reconciliação visível no Cadastro de Loteamentos e bloquear inclusão de Lote que não esteja comprovada por evidência física conciliada. **A208:** o módulo Estrutura passa a exibir a seção “Reconciliação estrutural”; ela orienta que não há inclusão por estimativa, registra apenas o estado de revisão pelo procedimento já protegido e mantém a matriz sujeita à prévia física conciliada.
- [x] Criar orientação operacional de conciliação com critérios de confirmação, trilha redigida e cancelamento seguro, sem automatizar a criação do Lote divergente. **A208:** procedimento documentado com comparação local, confirmação humana, MFA recente e bloqueio contra inferência.
...
## Restauração segura de Quadras arquivadas
- [x] Auditar o arquivamento lógico atual de Quadras e Lotes para definir condições de restauração sem alterar registros existentes. **A209:** o arquivamento atual é lógico, mas não mantém um retrato dos Lotes ativos no momento do comando; a restauração será limitada a futuros arquivamentos com retrato, evitando reativação indevida de Lotes já revisados.
- [x] Adicionar procedimento server-side para listar e restaurar somente Quadras/Lotes arquivados do rascunho e contexto autorizados, exigindo MFA recente, subject, organização, grant, finalidade, correlação e policy. **A209:** migração aplicada com listagem e restauração contextual, `SECURITY DEFINER`, `search_path` vazio, trava transacional, idempotência, auditoria redigida e privilégios exclusivos de serviço.
- [x] Exibir uma área de “Quadras arquivadas” no módulo Estrutura, com motivo, impacto e confirmação de restauração clara, sem criar Lotes novos nem tocar na matriz ativa sem solicitação explícita. **A209:** a área fica visível inclusive vazia; quando houver arquivo, mostra o botão Restaurar e confirmação explícita.
- [x] Cobrir restauração permitida, negação cross-context, MFA vencido, inexistência de registros arquivados, responsividade e ausência de efeitos comerciais. **A209:** testes dirigidos de serviço e migração, tipagem, revisão autenticada em leitura e validação integral de 208 arquivos/506 testes aprovados; nenhum registro real foi arquivado ou restaurado.
- [x] Gerar ZIP e HTML saneados sem restaurar qualquer Quadra/Lote real durante o desenvolvimento. **A209:** ZIP e HTML rotulados foram regenerados a partir do estado validado, sem `.env`, dependências, logs, build, documentação operacional, checklist, credenciais ou infraestrutura.
- [x] Salvar checkpoint A209 sem restaurar qualquer Quadra/Lote real durante o desenvolvimento. **A209:** consolidação preparada após migração aditiva, testes, revisão em leitura e geração de artefatos saneados.

## Correção do arquivamento e descoberta de Arquivados
- [x] Diagnosticar a negação exibida ao tentar arquivar uma Quadra com MFA recente, contexto e rascunho selecionados, sem repetir a mutação na estrutura real. **A210:** a primeira tentativa ocorreu com MFA vencido; o servidor negou o comando corretamente. Após revalidação, o arquivamento foi aceito.
- [x] Corrigir somente a pré-condição ou transmissão confirmada como causa do bloqueio, preservando MFA, contexto, grant, policy e auditoria. **A210:** a restauração posterior revelou somente uma normalização inválida no payload de auditoria; a migração corretiva preservou as guards e permitiu o retorno controlado.
- [x] Reposicionar e destacar visualmente a área de Quadras arquivadas para que seja encontrada de imediato, com estado vazio e ação de restauração compreensíveis. **A210:** cartão “Recuperação rápida” fica visível em estado vazio, destaca o local de retorno e mostra Restaurar como ação principal quando há item arquivado.
- [x] Validar o ciclo de arquivamento e restauração com estrutura controlada, sem alterar a matriz real, e registrar os resultados sem conteúdo operacional. **A210:** uma Quadra e seus Lotes existentes foram arquivados e restaurados no mesmo rascunho; a matriz voltou à contagem agregada inicial e a área Arquivados terminou vazia.
- [x] Gerar ZIP e HTML saneados sem publicar o CRM. **A210:** validação integral aprovou 210 arquivos/508 testes, tipagem, build e diff; geração de artefatos será concluída antes do checkpoint.
- [x] Diagnosticar a negação de restauração imediatamente após um arquivamento aceito na mesma sessão, sem repetir o comando nem deixar a Quadra de teste arquivada ao final. **A210:** o log PostgreSQL indicou falha na normalização de finalidade durante a inserção de auditoria; a migração corretiva foi aplicada e a restauração seguinte foi aceita.
- [x] Transformar a área Arquivados em um cartão de recuperação destacado, com ícone, promessa de restauração segura, estado vazio e aviso direto de revalidação quando MFA estiver vencido. **A210:** o cartão destacado foi aplicado e a operação sem MFA recente continua negada pelo servidor; a tela Segurança e MFA guia a revalidação antes de novo comando.
- [x] Salvar checkpoint A210 após gerar e conferir os artefatos saneados. **A210:** entrega rotulada foi regenerada depois da validação integral; consolidação pronta sem publicação ou mudança estrutural adicional.

## Correção da lista lateral de rascunhos
- [x] Auditar a regra de altura e distribuição da lista lateral que estica os cartões de rascunho e cria área vazia sem significado. **A211:** a lista em grade recebia estiramento vertical do espaço flexível da coluna, fazendo cartões ocuparem toda a altura disponível.
- [x] Redesenhar cada cartão lateral como resumo compacto com referência autorizada, nome, fase de trabalho, completude de módulos e contagem estrutural agregada, sem preço, pessoa, documento ou dado comercial. **A211:** cartões passaram a ter altura compacta, referência, fase e ação de abertura; completude e estrutura ficam no canvas após seleção para evitar consulta ou divulgação adicional na lista.
- [x] Manter seleção, busca local, navegação por teclado, contexto e limites de leitura existentes sem transformar o painel lateral em uma fonte de alçada. **A211:** o mesmo elemento de botão e filtro local foram preservados; somente o conteúdo e a composição visual foram aprimorados.
- [x] Cobrir a densidade visual, estado selecionado, itens vazios, responsividade e ausência de conteúdo sensível com testes e revisão desktop/móvel. **A211:** teste dirigido, revisão autenticada, capturas desktop/móvel e validação integral de 211 arquivos/509 testes aprovados.
- [x] Gerar ZIP e HTML saneados sem modificar rascunhos reais ou publicar o CRM. **A211:** artefatos rotulados foram gerados a partir do estado validado, com as exclusões de `.env`, dependências, logs, build, documentação operacional, checklist, credenciais e infraestrutura.
- [x] Salvar checkpoint A211 da correção visual, sem modificar rascunhos reais ou publicar o CRM. **A211:** consolidação preparada após testes, revisão visual e geração dos artefatos saneados.

## Nomenclatura operacional do Cadastro de Loteamentos
- [x] Inventariar toda ocorrência visível de “rascunho” no Cadastro de Loteamentos, incluindo cabeçalhos, cartões, avisos, confirmações, mensagens e estados vazios, sem alterar chaves técnicas ou dados existentes. **A212:** ocorrências de interface foram concentradas no estúdio e substituídas; chaves e nomes de procedures internas foram mantidos por segurança.
- [x] Substituir a nomenclatura visível por “Cadastro em estruturação”, “Cadastro selecionado” e “Matriz física do cadastro”, deixando claro o objetivo de cada etapa sem prometer aprovação, disponibilidade ou comercialização. **A212:** cabeçalho, lista, busca, contagem, matriz, mensagens, dossiê e arquivamento passaram a adotar a linguagem operacional.
- [x] Revisar testes e documentação de produto para a nova linguagem, preservando identificadores técnicos e procedures internas quando necessários à segurança. **A212:** asserções do estúdio foram atualizadas e a decisão terminológica foi documentada sem modificar contratos técnicos.
- [x] Validar em desktop e móvel e gerar ZIP e HTML saneados, sem modificar a matriz real ou publicar o CRM. **A212:** testes dirigidos e integrais, tipagem, build, diff e capturas desktop/móvel aprovados; artefatos rotulados foram gerados com exclusões de segurança.
- [x] Salvar checkpoint A212 da revisão terminológica sem modificar a matriz real ou publicar o CRM. **A212:** consolidação preparada após validações e entrega saneada, sem qualquer alteração de dados ou publicação.
- [x] Remover termos residuais visíveis de “rascunho” encontrados na descrição de contexto, nas confirmações de estrutura e nos estados do Cadastro de Loteamentos, sem renomear contratos técnicos internos. **A214:** mensagens de atualização, matriz, restauração, pendência, confirmação e dossiê agora adotam “cadastro em estruturação”; chaves técnicas permanecem internas.

## Gestão completa de Quadras, Lotes e precificação controlada
- [x] Auditar a matriz atual e mapear atributos físicos, operacionais e de leitura necessários por Quadra e por Lote, sem criar, excluir ou modificar a estrutura real. **A213:** a base já suporta área, frente, profundidade, tipologia e posição, mas a interface se limitava à quantidade por Quadra; modelo físico e lacunas foram registrados sem tocar na matriz.
- [x] Modelar uma camada de tabela de preços por metro quadrado, vigência, status, responsável e aprovação, completamente separada do estoque físico, de vendas, contratos e financeiro. **A213:** regra com escopo, BRL, preço/m², vigência, estado e transição de aprovação definida; valor-base é apenas área × preço/m² aprovado, sem proposta ou efeito comercial.
- [x] Definir cálculo transparente de valor-base por área e preço/m², com arredondamento, origem do dado, prévia e proibição de inferir preço para Lotes sem área ou tabela aprovada. **A213:** prévia local calcula somente Área × preço/m² informado, apresenta BRL e não atribui valor a Lote sem área física.
- [x] Reestruturar a tela Quadras e Lotes com visão por Quadra, busca por Lote, métricas físicas, atributos pendentes e encaminhamento explícito para a precificação ainda não aplicada. **A213:** Gestão física por unidade adicionada após a matriz; busca, métricas e cartões de Lote tornam atributos físicos consultáveis sem abrir módulo comercial.
- [x] Cobrir em testes os campos físicos, cálculo puro, vigência, ausência de área/preço, MFA, contexto, hierarquia SUPER ADM → ADM e segregação contra venda/financeiro. **A213:** teste dirigido de estúdio e validação integral de 211 arquivos/510 testes aprovados; a prévia não chama mutation e as guards existentes permanecem nos comandos protegidos.
- [x] Não inserir preço/m², valor de Lote, reajuste ou qualquer condição comercial no cadastro existente sem uma regra de precificação formalmente criada, aprovada e registrada. **A213:** nenhum valor real foi salvo; a tela declara que a prévia não grava nem aprova tabela e exige política formal futura.
- [x] Gerar ZIP e HTML saneados sem publicar o CRM ou alterar preço, venda, contrato, cobrança, pagamento ou repasse reais. **A213:** entrega rotulada foi gerada a partir do estado validado, preservando exclusões de `.env`, dependências, logs, build, documentação operacional, checklist, credenciais, infraestrutura e dados de cadastro real.
- [x] Salvar checkpoint A213 da gestão física e prévia local, sem persistir preço, venda, contrato, cobrança, pagamento ou repasse reais. **A213:** consolidação preparada após testes, revisão em leitura e geração de entrega saneada.

## Correção visual da gestão de Quadras e Lotes
- [x] Diagnosticar por que os estilos da gestão física e da prévia de preço foram renderizados como texto corrido na sessão real. **A214:** a folha isolada foi importada antes de existir, deixando erro de resolução no servidor de desenvolvimento; o reinício após a criação recuperou o carregamento e confirmou tipagem limpa.
- [x] Reestruturar a experiência em painéis compactos: indicadores, busca, cards de Lote e política de preço, sem perder os limites de estrutura física e precificação local. **A214:** a leitura foi agrupada por Quadra, com indicadores, busca, filtro explícito, cartões físicos e rótulos operacionais em português.
- [x] Validar a renderização carregada em desktop e móvel antes de executar testes, gerar artefatos ou solicitar nova revisão da pessoa usuária. **A214:** leitura autenticada confirmou filtros e Quadras expansíveis; capturas independentes de desktop/móvel confirmaram hierarquia, empilhamento responsivo e ausência de estiramento visual.
- [x] Cobrir a composição visual e as mensagens de segregação sem publicar ou persistir valores reais. **A214:** teste dirigido, suíte integral, tipagem, build e diff foram aprovados; mensagens preservam a separação entre leitura física, prévia local e operações comerciais.
- [x] Agrupar a leitura detalhada em Quadras expansíveis, preservando busca e filtro, para que os 164 Lotes não sejam apresentados como uma lista extensa sem navegação operacional. **A214:** uma Quadra abre por padrão; as demais mostram resumo e “Ver Lotes”, enquanto busca/filtro mostram somente o conjunto relevante.
- [x] Gerar ZIP/HTML saneados sem publicar ou persistir valores reais. **A214:** entrega gerada com rótulo de gestão de Quadras e Lotes, a partir do estado validado e com exclusões de `.env`, dependências, logs, build, documentação operacional, checklist, credenciais, infraestrutura e dados de cadastro real.
- [x] Salvar checkpoint A214 sem publicar ou persistir valores reais. **A214:** consolidação preparada após revisão visual real, validação integral e entrega saneada rotulada corretamente.

## Central operacional completa de Quadras e Lotes
- [x] Auditar a jornada ponta a ponta de estrutura física, dossiê técnico, busca, pendências, reconciliação e precificação para priorizar as lacunas que impedem uma operação profissional. **A215:** a matriz e a gestão física já existem, mas exigem centralização em camadas, visibilidade de pendências e uma política de preço separada para formar uma experiência operacional completa.
- [x] Pesquisar referências de gestão de parcelamento, cadastro técnico e políticas de preço controladas, mantendo a separação entre estrutura física e venda/financeiro. **A215:** pesquisa registrou padrões de unidades agrupadas, medidas, filtros, camadas territoriais e regras diferenciadas, sem incorporar recursos comerciais ou dados de terceiros.
- [x] Modelar Quadras, Lotes e dossiê técnico com atributos, estados de completude, histórico redigido e ações seguras, sem inventar informações físicas ou comerciais. **A215:** central definida em camadas de resumo, Quadras, Lotes, pendências/dossiê e preço controlado; dados físicos inexistentes permanecem pendentes.
- [x] Implementar uma central de gestão territorial por Quadra e Lote com visão resumida, detalhe expansível, busca, filtros, pendências, dossiê técnico e encaminhamentos claros. **A215:** a consulta fica priorizada; a matriz, as 14 Quadras e os 164 Lotes permanecem em leitura por padrão, com detalhe de Lotes agrupado por Quadra.
- [x] Projetar a política formal de preço/m² com escopo, vigência, preparação, aprovação, prévia e auditoria, sem inserir valores reais antes de regra homologada. **A215:** modelo separa escopo, BRL, vigência, estados e dupla aprovação; somente política aprovada e vigente poderá compor valor-base de leitura.
- [x] Cobrir a central em testes de estrutura, segregação, cálculo puro, MFA, contexto, alçadas, responsividade e leitura sem dados pessoais. **A215:** teste dirigido cobre política preparada, edição intencional e cópia; a suíte integral aprovou 212 arquivos e 514 testes, além de tipagem, build e diff limpo.
- [x] Gerar ZIP e HTML saneados e revisar a interface real antes de apresentar a evolução. **A215:** artefatos gerados e verificados com exclusão de ambiente, dependências, logs, build, documentação, checklist e padrões de credenciais ou infraestrutura; a revisão autenticada desktop confirmou a política preparada e a edição intencional.
...
## Importação controlada de estrutura e preço-base real — A223
- [x] Delimitar a importação inicial somente a Quadra, Lote, área e preço-base por m² existente em fonte oficial interna, excluindo clientes, disponibilidade, vendas, propostas, contratos, cobranças, pagamentos, repasses e anexos. **A223:** a fronteira está registrada na ficha de entidade e no blueprint, sem leitura da planilha real.
- [x] Pesquisar requisitos oficiais e técnicos brasileiros de proteção de dados, documentação fiscal/contábil, integridade, retenção, auditoria e segregação aplicáveis ao escopo delimitado. **A223:** foram consolidadas fontes oficiais da RFB, Planalto, ANPD, CFC, CPC e CVM, com distinção entre preço-base, ato registral, declaração e receita.
- [x] Produzir um desenho de política de preço-base e importação auditável com fonte, versão, competência, vigência, aprovador separado, idempotência, reversão lógica e bloqueio de sobreposição. **A223:** o blueprint define domínios, entidades, invariantes, prévia, máquina de estados e controles server-side.
- [x] Submeter o desenho para confirmação explícita antes de ler, copiar, persistir ou importar a planilha real e seus valores. **A223:** a autorização foi recebida; a fonte foi lida somente por parser efêmero e a contagem agregada registrou 163 linhas permitidas, uma exceção bloqueadora e nenhuma persistência.
- [x] Implementar somente após aprovação o fluxo protegido, testes, revisão visual, ZIP e HTML, sem publicar. **A223/A225:** parser efêmero, RPCs protegidas, RLS fail-closed explícito, política versionada, segregação de aprovação, UI autenticada e escopo server-side por cadastro foram implementados; a fonte permanece bloqueada por uma exceção até correção na origem, sem política ou linha persistida.
- [x] Salvar checkpoint A223–A225 após documentação, validação integral, revisão autenticada e saneamento dos artefatos. **A223/A225:** checkpoint `9b60a490` preserva a implementação protegida, a correção de escopo e a ausência de política/linha persistida.

## Estudo contratual e de distrato como referência — A224
- [x] Extrair dos documentos autorizados somente padrões de fluxo, cláusulas operacionais e eventos de ciclo, com mascaramento de todos os dados pessoais, identificadores, assinaturas e valores individuais. **A224:** o estudo saneado registra apenas eventos e controles, sem reproduzir identificação, assinaturas, endereços ou valores individuais.
- [x] Modelar a separação entre política de preço-base, proposta, contrato, recebível, pagamento, distrato e disponibilidade, sem criar qualquer um desses registros materiais. **A224:** preço-base ficou isolado como referência interna; contrato e distrato permanecem preparação de domínio sem comandos materiais.
- [x] Incorporar ao blueprint de importação as regras de proveniência, vigência, alteração, retirada e bloqueios que o estudo documental evidenciar. **A224:** o desenho exige fonte identificada, versão, vigência, dupla aprovação, auditoria redigida e reversão lógica.
- [x] Implementar somente os componentes de importação e política de preço-base expressamente autorizados, mantendo contrato, distrato e financeiro em preparação até uma etapa própria. **A224:** o estudo contratual orientou a segregação e os bloqueios; não foram criados contrato, distrato, recebível, cobrança, pagamento, repasse ou integração externa.
...
## Contexto de preço por Lote durante preparação — A240
- [x] Distinguir no detalhe de Lote os estados sem política, política em Preparação com exceção, política aguardando aprovação e referência aprovada vigente. **A240:** a função protegida v3 retorna somente categorias de disponibilidade; preço continua disponível apenas no estado ativo.
...
- [x] Cobrir serviço, migração e interface; revisar desktop/móvel, gerar ZIP/HTML saneados e salvar checkpoint sem editar Quadra real. **A256:** 219 arquivos/587 testes, tipagem, build Netlify, diff limpo, captura desktop/móvel e saneamento foram aprovados; as verificações cobrem o comando, a leitura v4 e o bloqueio de domínios comerciais. Checkpoint funcional `05b6117d` registrado sem editar Quadra real.

## Lapidação de medidas, edição e hierarquia visual — A257
- [x] Modelar as quatro divisas físicas independentes do Lote: frente, fundo, lateral esquerda e lateral direita; preservar a área como atributo distinto e nunca estimar medidas ausentes. **A257:** leitura e ficha usam campos independentes; a área continua explícita e não é recalculada pelas divisas.
- [x] Ampliar a ficha de Lote com títulos orientativos, resumo físico e textos claros que indiquem exatamente o que pode ser editado, sem misturar preço, disponibilidade, venda, contrato ou financeiro. **A257:** sequência de seleção, rótulos de divisa, notas de ajuda e observação interna saneada foram esclarecidas.
- [x] Ampliar a ficha de Quadra com identificação, composição agregada, campos físicos próprios e observação interna governada, sem preencher dados reais automaticamente. **A257:** a ficha identifica o escopo da Quadra, orienta a revisão e preserva os limites de domínio.
- [x] Corrigir a sobreposição observada entre estado e orientação no histórico de política-base, preservando o bloqueio por exceção e a hierarquia visual. **A257:** aviso e ações foram organizados em faixas independentes; a política permanece bloqueada por exceção.
- [x] Cobrir contratos, serviço, migração, interface e responsividade; revisar desktop, tablet, celular e tela ampla; gerar ZIP/HTML saneados e checkpoint sem editar Lote ou Quadra real. **A257:** 219 arquivos/588 testes, tipagem, build Netlify, diff limpo, revisão ampla e móvel, ZIP/HTML saneados. A tentativa de sessão autenticada não concluiu no prazo e não gerou interação material. Checkpoint funcional `287cca83` registrado.

## Edição contextual e referência por Lote — A258
- [x] Inserir uma ação de edição discreta em cada cartão de Lote que preencha a ficha física com a Quadra e o Lote selecionados, sem gravar, abrir venda ou alterar disponibilidade. **A258:** o atalho Editar abre a ficha física contextualizada e rola até o ponto de revisão.
- [x] Garantir que a ação contextual mantenha MFA, contexto, alçada e auditoria no salvamento existente; a interface nunca concede permissão. **A258:** o clique apenas preenche a ficha; o mesmo comando protegido continua responsável por qualquer gravação.
- [x] Tornar a seção de referência de preço por unidade acionável e explicativa: mostrar valor por m² e total somente quando a política estiver aprovada e vigente; para política preparada, encaminhar à governança de preço sem expor números. **A258:** cada cartão apresenta Consulta de referência; a leitura continua bloqueada até aprovação e vigência.
- [x] Preservar o Lote sem valor-base explicitamente vazio e impedir estimativa, cópia ou preenchimento automático a partir de outros Lotes. **A258:** nenhum valor foi copiado, calculado ou exibido da política em preparação.
- [x] Corrigir a legibilidade e a hierarquia das ações por cartão, cobrindo desktop, tablet e celular; validar testes, artefatos saneados e checkpoint sem editar Lote, preço ou política real. **A258:** 219 arquivos/588 testes, tipagem, build Netlify, diff limpo e revisão visual. ZIP/HTML saneados confirmados; nenhuma unidade, preço ou política foi editada. Checkpoint funcional `689dd00e` registrado.

## Referência interna de preço-base por Lote — A259
- [x] Exibir no cartão do Lote a referência interna preparada de preço-base por m² e o valor total calculado sobre a área física confirmada, rotulada como controle interno e sem alterar disponibilidade, venda, proposta, contrato ou financeiro. **A259:** a leitura interna protegida apresenta os dois números por unidade com rótulo de preparação.
- [x] Manter explicitamente vazio o Lote sem valor-base na fonte e indicar que requer definição humana, sem copiar, inferir ou estimar preço. **A259:** a unidade sem valor é indicada como pendência humana e não recebe total estimado.
- [x] Criar atalho de ajuste de preço por Lote que direcione para a correção manual governada, preservando nova versão, respaldo privado, MFA, segregação de aprovador, contexto, idempotência e auditoria redigida. **A259:** o atalho somente prepara o escopo da condição e campos vazios; não envia comando.
- [x] Diferenciar visualmente referência interna em preparação, referência aprovada e ausência de valor, sem permitir que a interface conceda alçada ou transforme preço interno em disponibilidade. **A259:** os estados têm rótulos próprios e a preparação declara que não gera efeito comercial.
- [x] Auditar a cobertura entre linhas de política e Lotes físicos, a presença de valores explícitos, o único valor-base ausente, os totais calculados e os bloqueios de domínio, mantendo a saída agregada e sem expor valores individuais. **A259:** a auditoria confirmou 164 Lotes físicos, 163 linhas e valores explícitos, 1 pendência, 163 totais calculáveis e nenhum valor sem área confirmada.
- [x] Cobrir serviço, migração, cálculo, interface e responsividade; revisar em sessão autorizada e gerar ZIP/HTML saneados e checkpoint sem editar preço, política ou Lote real. **A259:** 219 arquivos/591 testes, tipagem, build Netlify, diff limpo, MFA revalidada, conferência autenticada e artefatos saneados. Nenhum preço, política ou Lote foi editado. Checkpoint funcional `51ea5405` registrado.

## Redesenho da referência de preço por Lote — A260
- [x] Substituir os blocos estreitos e concorrentes de preço por uma composição de leitura principal/secundária, com valor por m² destacado, total claramente associado e estado interno em faixa própria. **A260:** preço-base e total agora participam da mesma grade física compacta; o estado de MFA tornou-se nota de apoio, sem caixa concorrente.
- [x] Integrar o atalho de ajuste à hierarquia do cartão, com área de toque consistente, rótulo claro e sem deslocar medidas físicas ou ocultar a finalidade reservada. **A260:** o atalho preserva a ação preparada e se alinha ao fim do cartão sem executar comando.
- [x] Reorganizar a grade dos cartões e das Quadras para evitar espaços vazios, compressão e quebras de texto em telas ampla, notebook, tablet e celular. **A260:** grade por Lote usa preenchimento adaptativo e foi revisada nos quatro pontos de corte requeridos.
- [x] Preservar cálculos, leitura interna por MFA, valor-base ausente vazio, política em preparação e todos os bloqueios de disponibilidade, venda, contrato e financeiro. **A260:** leitura autenticada confirmou valores autorizados; nenhuma operação material foi executada.
- [x] Cobrir a nova estrutura visual em testes e revisão responsiva; gerar ZIP/HTML saneados e checkpoint sem alterar preços, política ou Lotes reais. **A260:** 219 arquivos de teste e 592 testes aprovados, tipagem, build Netlify, diff limpo, leitura autenticada com MFA reforçado, revisão em 1920/1280/768/375 px e saneamento final de ZIP/HTML aprovados. Nenhum dado material foi alterado. Checkpoint funcional `d500f963` registrado.
- [x] Corrigir a hierarquia do cartão para apresentar Área física, preço-base interno por m² e valor total no mesmo fluxo compacto, reduzindo mensagens de estado a apoio discreto sem suprimir números autorizados. **A260-R1:** a conferência com MFA reforçado confirmou a leitura interna no cartão, sem quadro separado.

## Reestruturação da entrada operacional de Loteamentos — A261
- [x] Pesquisar padrões contemporâneos de painéis operacionais imobiliários e visualização de portfólio que privilegiem escolha de empreendimento, indicadores agregados e densidade legível. **A261:** fontes estudadas integralmente; foram aplicados poucos indicadores físicos, comparação linear e separação entre consulta e edição.
- [x] Remover a composição com faixas laterais que reduz a área útil no desktop, preservando navegação, contexto, MFA e todos os bloqueios existentes. **A261:** lista e painel laterais internos foram substituídos; somente a navegação global permanece lateral.
- [x] Substituir a abertura por uma seleção clara de empreendimento e uma Visão Operacional com indicadores e gráficos agregados por empreendimento, sem expor preços unitários, clientes, vendas, contratos ou financeiro. **A261:** a entrada oferece seleção, dois destinos explícitos, métricas físicas, barras de Quadras e cobertura física.
- [x] Manter a edição física e a matriz de Quadras/Lotes como jornada explícita posterior à escolha do empreendimento, sem executar alteração por navegação ou aparência. **A261:** Cadastro do Empreendimento abre a jornada modular em largura plena, sem comando material por troca de visão.
- [x] Cobrir a nova entrada e seus limites de leitura em testes, revisar em desktop, tablet e celular e confirmar que não há compressão, faixas sem função, cortes ou sobreposições. **A261:** 219 arquivos de teste e 593 testes aprovados; tipagem, build Netlify, diff limpo e revisão em 1920/1280/768/375 px concluídos.
- [x] Gerar ZIP/HTML saneados e checkpoints após validação integral, sem alterar Lotes, Quadras, áreas, preços, políticas, condições, reservas, vendas, contratos ou dados financeiros. **A261:** artefatos gerados após build final e verificados quanto a presença, exclusões e saneamento; checkpoint funcional `28996b1c` registrado.
- [x] Revisar integralmente cada referência estudada, tela relevante e estado posterior da jornada antes de concluir A261, sem decidir por recorte visual ou conteúdo parcial. **A261:** todas as referências visuais recebidas e as fontes estudadas foram verificadas de ponta a ponta e registradas sem reproduzir dados sensíveis.

## Correção de continuidade do Cadastro de Loteamentos — A262
- [x] Remover o mecanismo que oculta a área de cadastro quando a Visão Operacional está selecionada, preservando todas as informações e jornadas existentes em fluxo contínuo. **A262:** os dois destinos passaram a orientar por rolagem, sem condicionar a renderização do Cadastro.
- [x] Manter Visão Operacional como resumo inicial sem substituir, esconder ou bloquear módulos, matriz, fichas, pendências, anexos, evidências, políticas ou ciclo do empreendimento selecionado. **A262:** o resumo e todo o conteúdo preexistente agora coexistem na mesma jornada.
- [x] Confirmar no navegador autenticado que seleção, painel, cadastro, matriz e estados posteriores aparecem na mesma página sem perda de conteúdo nem mutação de dados. **A262:** confirmação autenticada e revisão móvel concluídas, sem comando material executado.
- [x] Cobrir a correção em testes, revisar desktop, tablet e celular, gerar ZIP/HTML saneados e salvar checkpoints após validação integral. **A262:** 219 arquivos de teste e 594 testes aprovados; tipagem, build Netlify, diff, revisão autenticada em desktop/celular e saneamento de ZIP/HTML concluídos. Checkpoint funcional `aaca565c` registrado.
- [x] Aplicar e documentar a regra permanente: toda atualização deve aprimorar e preservar o que já existe; exclusão, ocultação ou substituição de conteúdo, recurso, fluxo ou dado somente ocorre com ordem explícita do usuário. **A262:** regra registrada em `AGENTS.md` na raiz do projeto e observada na correção atual.

## Preservação de conhecimento e referência interna de preço — A263
- [x] Criar, validar e entregar uma habilidade reutilizável que conduza atualizações preservativas de CRM: estudo completo, registro de decisão, implementação sem exclusão, testes, revisão de jornada, saneamento e checkpoint. **A263:** habilidade `crm-preservative-evolution` criada e validada.
- [x] Consolidar no projeto a regra de preservação de conhecimento: estudos, decisões, evidências de UX, contratos de interface e validações devem permanecer localizáveis e acessíveis em atualizações futuras. **A263:** `AGENTS.md` e índice versionado registram a preservação e as referências obrigatórias.
- [x] Diagnosticar integralmente a cadeia de referência interna por Lote — sessão MFA, consulta autorizada, preço-base por m², área física confirmada, cálculo de total e apresentação — sem alterar dados, política ou preço-base. **A263:** a cadeia foi mapeada e a fonte/política foram conciliadas sem mutação.
- [x] Corrigir a experiência de leitura para que valores autorizados por MFA apareçam de forma estável e os estados de segurança expliquem claramente o bloqueio sem parecer que valor ou cálculo foi removido. **A263:** MFA, bloqueio e ausência intencional têm estados distintos no mesmo fluxo do cartão.
- [x] Cobrir leitura, cálculo, ausência intencional de valor, MFA e responsividade em testes e revisão real; gerar ZIP/HTML saneados e checkpoints após validação integral. **A263:** revisão autenticada e responsiva concluída; saneamento de entrega A264 confirmado.
- [x] Ampliar a grade de cartões de Lote para aproveitar a largura disponível e acomodar valores extensos por m² e totais referenciais sem sobreposição, texto quebrado ou faixas vazias. **A263:** três cartões ampliados em desktop e empilhamento responsivo preservado.
- [x] Eliminar o painel branco paralelo de referência de preço e incorporar Valor por m² e Valor total do Lote como células nativas da mesma grade de informações físicas. **A263:** confirmado com MFA recente na matriz; a grade não cria efeito comercial.

## Reconciliação manual de preço e metragem — A264
- [x] Inspecionar a fonte privada de controle exclusivamente para Quadra, Lote, área física e preço-base por m²; bloquear dados pessoais, vendas, clientes, corretores, contratos e colunas não autorizadas. **A264:** 164 linhas físicas tratadas; uma ausência explícita de preço-base preservada.
- [x] Reconciliar cada área física autorizada da fonte com a matriz do CRM antes de aplicar preço-base; isolar qualquer divergência sem estimativa, substituição ou alteração física automática. **A264:** 164 áreas correspondentes, sem itens exclusivos ou divergências.
- [x] Aplicar manualmente, via cadeia governada e sob MFA recente, apenas os preços-base explícitos para os Lotes reconciliados, preservando a exceção sem preço-base. **A264:** as 163 referências já preparadas foram verificadas como correspondentes à fonte; nenhuma duplicação ou sobrescrita foi feita.
...
- [x] Validar o documento, gerar ZIP/HTML saneados e salvar checkpoints da estratégia A266 antes de iniciar qualquer implementação. **A266:** estratégia revisada integralmente; ZIP/HTML gerados e saneados; alteração estrutural, migração e dado permanecem fora deste marco. Checkpoint funcional `ccdeaf5c` registrado.
- [x] Salvar checkpoint de consolidação da estratégia A266 após registrar a versão funcional no checklist. **A266:** checkpoint de consolidação `b58e400e` registrado; nenhuma implementação estrutural foi iniciada.

## Unificação preservativa do setor Loteamentos — A268
- [x] Renomear a entrada visível de Cadastro de Loteamentos para Loteamentos, preservando o rótulo e a rota atual de Estoque/Mapa de Lotes como compatibilidade explícita durante a transição. **A268:** grupo e entrada principal usam Loteamentos; as duas rotas permanecem acessíveis.
- [x] Criar uma entrada unificada que apresente Cadastro e Estoque/Mapa como áreas complementares do mesmo empreendimento, sem desmontar, ocultar, excluir ou duplicar as jornadas existentes. **A268:** a faixa de jornada unificada oferece acesso explícito ao Cadastro/Matriz e ao Estoque/Mapa antes do estúdio já existente.
- [x] Reutilizar o contexto autorizado e a seleção de empreendimento entre as duas entradas somente para leitura/navegação, sem conceder alçada nem disparar mutações. **A268:** as duas áreas permanecem sob o mesmo módulo/contexto; cada leitura continua revalidada no servidor.
- [x] Cobrir a compatibilidade de rotas, a coexistência das jornadas, os estados de acesso e a responsividade em testes e revisão autenticada. **A268:** 220 arquivos de teste e 601 testes aprovados; tipagem, build Netlify, diff e revisão em desktop/celular concluídos.
- [x] Gerar ZIP/HTML saneados e checkpoints após validação integral, sem migração, alteração de dados físicos, criação de estoque ou mudança comercial. **A268:** ZIP/HTML gerados e verificados quanto a presença, exclusões e saneamento; checkpoint funcional `51922792` registrado.
- [x] Substituir, nas superfícies visíveis de Loteamentos e Estoque/Mapa, nomenclaturas provisórias por estados profissionais de preparação, sem renomear dados, rotas ou contratos internos. **A268:** rótulos e mensagens visíveis de Estoque/Mapa foram atualizados sem mudar funções internas.

## Estoque interno moderno por Lote — A269
- [x] Modelar uma camada aditiva de perfil interno de estoque, referenciada ao Lote físico canônico e aos estados/eventos existentes, sem criar uma segunda identidade de Lote nem preencher dados reais automaticamente. **A269:** tabela privada 1:1 resolve o Lote físico canônico exclusivamente no servidor e iniciou sem perfis registrados.
- [x] Limitar os novos campos a classificação interna não comercial, revisão operacional, legenda de mapa e nota interna saneada; manter disponibilidade, reserva comercial, venda, proposta, contrato, preço comercial e financeiro fora do modelo. **A269:** contratos, RPCs e painel usam somente taxonomias fechadas e nota curta saneada.
- [x] Proteger leitura e escrita no banco por organização, contexto, escopo, finalidade, MFA recente, correlação, idempotência, RLS fail-closed e auditoria redigida; revogar acesso direto e manter execução apenas pelo serviço autorizado. **A269:** migração aplicada e coberta com RLS, RPCs `SECURITY DEFINER`, busca contextual no banco e execução restrita ao serviço.
- [x] Integrar uma seção contínua de Estoque Interno ao setor Loteamentos, reutilizando a matriz física e o contexto atual, com filtros e legenda de leitura sem desmontar Cadastro ou criar comandos por célula. **A269:** a seção aparece após a matriz e antes da ficha física; a leitura MFA confirmou a matriz preservada e perfis vazios.
- [x] Preservar a rota complementar Estoque/Mapa, os estados e eventos legados e preparar apenas o contrato de projeção externa futura, sem criar mapa público, publicação ou acesso de terceiros. **A269:** rotas e fontes legadas permaneceram acessíveis; a interface declara explicitamente que mapa público não foi criado.
- [x] Cobrir migração, serviço, contratos, rotas, UI, bloqueios e responsividade; conciliar contagens antes/depois e gerar ZIP/HTML saneados e checkpoints sem alterar Lotes reais ou dados comerciais. **A269:** 225 arquivos de teste/610 testes, tipagem, build, diff, revisão autenticada, contagem agregada vazia e artefatos saneados aprovados; checkpoints pendem apenas do registro desta conclusão.
- [x] Corrigir o estado de MFA do painel A269 para não apresentar contagens de perfil como confirmadas nem habilitar gravação enquanto a leitura protegida estiver indisponível. **A269:** a UI usa estado protegido, bloqueia a ficha e só confirma totais após leitura MFA autorizada.
- [x] Reconfigurar o autenticador MFA por fluxo visível e autorizado pelo usuário, sem expor QR, segredo ou código em conversa e sem executar gravações de perfil interno. **A269:** vinculação adicional confirmada pelo usuário; o fator anterior permaneceu preservado e a sessão passou a reconhecer MFA recente.
- [x] Ampliar o saneamento do HTML autônomo para remover endpoints codificados com barras escapadas antes de gerar a entrega A269. **A269:** saneador e teste cobrem URL codificada e padrão genérico de host; o HTML final passou na verificação de marcadores sensíveis e de infraestrutura.
- [x] Salvar checkpoint funcional A269 após validações, documentação e artefatos saneados, sem publicar ou criar perfil interno real. **A269:** checkpoint funcional registrado após migração aditiva, leitura MFA protegida, validação integral e saneamento; nenhum perfil interno real foi criado.
- [x] Salvar checkpoint de consolidação A269 após registrar o checkpoint funcional no checklist. **A269:** consolidação registrada sem publicação, criação de perfil interno real ou alteração de Lote, preço, disponibilidade, venda, contrato ou financeiro.

## Correção de hierarquia visual: Estoque/Mapa dentro de Loteamentos
- [x] Reorganizar a navegação para apresentar Estoque/Mapa como área interna de Loteamentos, preservando rota, conteúdo, segurança e compatibilidade sem mantê-lo como setor visual paralelo. **A270:** a barra lateral passou a oferecer apenas Loteamentos; a rota compatível foi preservada como mapa interno com retorno explícito à jornada principal.
- [x] Cobrir a nova hierarquia com testes, revisão responsiva, artefatos saneados e checkpoints, sem remover Cadastro, matriz, fichas, estados/eventos ou funcionalidades existentes. **A270:** revisão em tela ampla, desktop, tablet e celular; suíte integral, tipagem, build e diff aprovados; ZIP/HTML saneados gerados. Checkpoints serão registrados como etapas separadas após este checklist.
- [x] Salvar checkpoint funcional A270 após a documentação, validação e os artefatos saneados, sem publicar ou alterar dados reais. **A270:** checkpoint funcional salvo após a correção da navegação, revisão visual, validação integral e saneamento dos artefatos; nenhuma mutação de dados ou publicação foi executada.
- [x] Salvar checkpoint de consolidação A270 após registrar o checkpoint funcional no checklist. **A270:** consolidação salva com a entrada única de Loteamentos, rota interna compatível preservada e seletor local corrigido; não houve publicação ou mutação de dados reais.
- [x] Corrigir o seletor setorial interno que ainda inclui Segurança e MFA e conserva a numeração anterior após a retirada visual de Estoque/Mapa da sidebar. **A270:** o seletor agora deriva somente das rotas da coluna Loteamentos e apresenta os setores 01 a 05 sem deslocamento; a verificação autenticada confirmou a ausência de Segurança e MFA e de Estoque/Mapa como setor paralelo.
- [x] Corrigir o resumo da coluna Loteamentos no painel central, que ainda reproduz a antiga lista de Estoque/Mapa como setor separado apesar da nova hierarquia interna. **A270:** o painel central passou a usar Loteamentos e um único primeiro setor de cadastro, matriz e estoque interno; a revisão em tela ampla, tablet e celular confirmou a remoção da linha paralela.
- [x] Salvar o checkpoint complementar A270 após alinhar o painel central, repetir a validação integral e regenerar os artefatos saneados. **A270:** checkpoint complementar salvo com o resumo central coerente com a navegação única de Loteamentos; nenhuma rota, dado, guarda ou controle material foi removido ou alterado.

## Correção de regressão: valores internos por Lote
- [x] Diagnosticar por que o cartão físico voltou a renderizar indisponibilidade para Valor por m² e Valor total referencial, preservando a leitura MFA, a política interna preparada, a matriz física e a exceção intencionalmente vazia. **A271:** a indisponibilidade foi bloqueio correto de MFA recente, não perda de valores; a leitura protegida voltou a ser confirmada após revalidação.
- [x] Corrigir a cadeia de leitura e apresentação sem alterar qualquer valor, política, Lote, área, disponibilidade, venda, contrato ou dado financeiro. **A271:** os cartões agora identificam o bloqueio MFA de forma explícita e oferecem acesso direto à revalidação; a consulta protegida voltou a renderizar as referências internas autorizadas.
- [x] Cobrir a regressão, revisar a jornada autenticada e responsiva, gerar ZIP/HTML saneados e salvar checkpoints somente após comprovar a restauração visual protegida. **A271:** leitura MFA autenticada confirmou valores internos e totais referenciais, jornada preservada e exceção vazia; 226 arquivos de teste/615 testes, tipagem, build, diff e revisão ampla/tablet/celular aprovados; artefatos saneados gerados. Checkpoints seguem como etapas separadas.
- [x] Gerar uma nova vinculação adicional e temporária de MFA pelo fluxo visível solicitado pelo usuário, sem criar QR permanente, sem expor segredo ou código em conversa e sem remover fatores existentes. **A271:** inscrição adicional confirmada diretamente pelo usuário; a sessão reforçada foi reconhecida e nenhum fator foi removido.
- [x] Corrigir a inscrição de autenticador adicional para não reutilizar nome de fator que possa colidir com um vínculo existente, preservando o fluxo de confirmação local e os fatores já verificados. **A271:** a inscrição passou a utilizar rótulo único por tentativa e o QR foi gerado e confirmado após a correção.
- [x] Salvar checkpoint funcional A271 após a validação, documentação e os artefatos saneados, sem publicar ou alterar dados reais. **A271:** checkpoint funcional registrado após restauração de leitura MFA, correção visual de bloqueio, vinculação adicional confirmada, validação integral e artefatos saneados; não houve publicação ou mutação de dados materiais.
- [x] Salvar checkpoint de consolidação A271 após registrar o checkpoint funcional no checklist. **A271:** consolidação registrada sem publicação ou alteração de Lote, área, preço, política, disponibilidade, venda, contrato ou financeiro.

## Correção de edição por Lote: dados físicos e referência interna
- [x] Mapear cada campo exibido no cartão de Lote e seu caminho atual de edição, distinguindo atributos físicos editáveis, referência interna por m², total referencial derivado e campos intencionalmente somente de leitura. **A272:** físicos seguem na ficha; valor por m² segue em condição interna governada; total é derivado e não aceita digitação independente.
- [x] Tornar a entrada de edição física completa e explícita para os atributos físicos permitidos, sem alterar Lote real durante o desenvolvimento e sem misturar preço, disponibilidade, venda, contrato ou financeiro. **A272:** cartão preserva acesso direto à ficha com todos os atributos físicos permitidos e a orientação explícita da fronteira de domínio.
- [x] Tornar o ajuste da referência interna por m² um caminho direto, compreensível e governado a partir do cartão e da ficha, preservando política, MFA, contexto, alçada, correlação, idempotência, evidência e auditoria; não gravar valor real sem autorização material específica. **A272:** CTA autenticado seleciona somente Quadra/Lote, política e escopo individual, leva ao campo focado de valor/m² e o mantém vazio até ação humana autorizada.
- [x] Cobrir a jornada, testar em sessão autenticada sem mutação material, revisar desktop/tablet/celular, gerar ZIP/HTML saneados e salvar checkpoints somente após comprovar a edição por domínio. **A272:** CTA de preço autenticado selecionou escopo individual, preservou o valor vazio e focou o campo correto; cartões e ficha física mantiveram domínios separados. A suíte completa aprovou 226 arquivos/615 testes, tipagem, build e diff; artefatos saneados foram verificados. Checkpoints seguem como etapas separadas.
- [x] Salvar checkpoint funcional A272 após a validação, documentação e artefatos saneados, sem publicar ou alterar valores reais. **A272:** checkpoint funcional registrado após teste autenticado de seleção, rolagem e foco do ajuste interno, sem digitação, preparação ou salvamento de valor real.
- [x] Salvar checkpoint de consolidação A272 após registrar o checkpoint funcional no checklist. **A272:** consolidação registrada sem publicar, preparar condição, gravar preço, editar Lote ou alterar disponibilidade, venda, contrato ou financeiro.

## Editor visível de valor por m² no contexto do Lote
- [x] Substituir o salto distante por uma área de edição expansível e visível dentro da própria ficha do Lote, com campo de valor por m², resumo do total exclusivamente derivado e explicação de domínio interno. **A273:** o editor aparece abaixo da ficha física selecionada, sem duplicar o formulário legado ou misturar atributos físicos com preço.
- [x] Preservar a preparação governada de condição com MFA, contexto, alçada, política, vigência, motivo, respaldo, correlação, idempotência e auditoria; não gravar valor real durante o desenvolvimento nem tornar preço interno em preço comercial. **A273:** a unidade selecionada prepara somente o contexto individual e mantém valor vazio até ação humana autorizada.
- [x] Cobrir abertura, pré-preenchimento contextual, cálculo derivado, bloqueios de MFA e ausência de mutação; validar em sessão autenticada e em desktop/tablet/celular, gerar ZIP/HTML saneados e checkpoints somente após comprovação prática. **A273:** teste autenticado selecionou uma unidade sem mutação e confirmou o editor visível; 227 arquivos/617 testes, tipagem, build, diff e artefatos saneados aprovados. Checkpoints seguem como etapas separadas.
- [x] Salvar checkpoint funcional A273 após validação, documentação e artefatos saneados, sem publicar ou alterar valor real. **A273:** checkpoint funcional registrado após validação autenticada do editor visível, testes, build, artefatos saneados e ausência de digitação, preparação ou salvamento de valor real.
- [x] Salvar checkpoint de consolidação A273 após registrar o checkpoint funcional no checklist. **A273:** consolidação registrada sem publicar, digitar preço, preparar condição, gravar valor, editar Lote ou alterar disponibilidade, venda, contrato ou financeiro.

## Correção de persistência MFA na leitura de valores internos
- [x] Diagnosticar por que uma confirmação MFA recém-concluída não permanece refletida de modo consistente nas consultas protegidas de valor por m² e total referencial, sem expor segredo, sessão, valor individual ou dados privados. **A274:** o token atualizado chegava ao servidor, mas a tela preservava a falha de consulta anterior até atualização manual.
- [x] Corrigir a invalidação/sincronização de estado entre confirmação MFA, consultas de preço e cartões/editor contextual, preservando a expiração real, os guardas server-side e a ausência de mutação de preço. **A274:** transição para MFA recente, montagem e retorno de foco agora renovam as consultas protegidas; a expiração verdadeira continua server-side e fail-closed.
- [x] Cobrir a sessão reforçada, a expiração verdadeira, cartões, ficha e editor contextual; validar em navegador autenticado sem digitar ou salvar preço, gerar ZIP/HTML saneados e checkpoints somente após comprovação prática. **A274:** leitura autenticada persistiu após recarregamento da jornada; 228 arquivos/619 testes, tipagem, build, diff e artefatos saneados aprovados. Checkpoints seguem como etapas separadas.
- [x] Salvar checkpoint funcional A274 após validação, documentação e artefatos saneados, sem publicar ou alterar preço real. **A274:** checkpoint funcional registrado após sincronização de MFA e consultas protegidas, persistência de leitura após recarregamento, validação integral e artefatos saneados; não houve publicação ou mutação de preço.
- [x] Salvar checkpoint de consolidação A274 após registrar o checkpoint funcional no checklist. **A274:** consolidação registrada sem publicar, digitar preço, preparar condição, gravar valor, editar Lote ou alterar política, disponibilidade, venda, contrato ou financeiro.

## Leitura interna vinculada à sessão autenticada
- [x] Mapear todas as consultas de leitura interna que hoje exigem recência TOTP por operação, distinguindo valores internos, dados físicos, perfis operacionais e comandos materiais/privilegiados. **A275:** somente referência interna por Lote e perfil operacional eram leituras sob recência; os demais usos revisados pertencem a mutações materiais.
- [x] Permitir leitura interna autorizada durante a sessão autenticada em que MFA foi confirmado no login ou na revalidação, mantendo o servidor como fonte de verdade e sem conceder alçada adicional pela interface. **A275:** referências internas e perfis operacionais passaram a usar sessão, contexto e autoridade existentes; a leitura autenticada foi confirmada no navegador sem MFA repetido.
- [x] Preservar MFA/reauthenticação, contexto, alçada, correlação, idempotência e auditoria para mutações de preço, permissões, estrutura, contratos, financeiro e demais comandos materiais. **A275:** testes estáticos confirmam guarda MFA em perfil, condição de preço e ficha física; nenhuma mutação foi executada.
- [x] Cobrir token de sessão válido, logout, expiração real, leitura contínua e bloqueio de comandos; validar em navegador autenticado sem mutações, gerar ZIP/HTML saneados e checkpoints após comprovação prática. **A275:** leitura autenticada confirmada; 229 arquivos/621 testes, tipagem, build e diff aprovados. Artefatos e checkpoints seguem como etapas separadas.
- [x] Gerar ZIP/HTML saneados A275 sem publicar ou executar mutação material. **A275:** artefatos gerados e verificados quanto a presença, exclusões e saneamento; não houve publicação.
- [x] Salvar checkpoint funcional A275 após validação, documentação e artefatos saneados, sem publicar ou alterar dados materiais. **A275:** checkpoint funcional registrado após liberar leituras internas na sessão autenticada, preservar MFA em mutações e validar a jornada sem alteração material.
- [x] Salvar checkpoint de consolidação A275 após registrar o checkpoint funcional no checklist. **A275:** consolidação registrada sem publicação ou mudança de preço, Lote, perfil, política, condição, disponibilidade, venda, contrato ou financeiro.

## MFA somente na autenticação da sessão
- [x] Mapear todos os comandos materiais que ainda exigem MFA recente e confirmar que permanecem protegidos por sessão autenticada, organização, contexto, papel, escopo, finalidade, correlação, idempotência e auditoria. **A276:** todos os guardas de Loteamentos e anexos foram inventariados; o helper comum preserva organização, contexto, escopo, finalidade, correlação, idempotência e auditoria.
- [x] Substituir a exigência de MFA repetida por comprovação de sessão autenticada com MFA válido no login ou revalidação, sem conceder permissões adicionais pela interface e sem permitir uso após logout ou expiração real. **A276:** AAL2/TOTP validado no token de sessão sustenta comandos permitidos; logout, expiração e invalidação do provedor bloqueiam novamente.
- [x] Atualizar a orientação do CRM para declarar MFA no login e sessão autorizada para ações permitidas, mantendo bloqueios por contexto, alçada e política. **A276:** Segurança/MFA, estúdio e painel operacional mostram a sessão autorizada; nenhum componente concede alçada pela interface.
- [x] Cobrir sessões válidas e inválidas, comandos autorizados e recusas por escopo/contexto/logout; validar no navegador sem executar mutação material, gerar ZIP/HTML saneados e checkpoints após comprovação. **A276:** sessão AAL2/TOTP é válida sem recência por comando, sessões fracas ou inválidas continuam recusadas, e servidor preserva contexto/escopo/alçada. Segurança/MFA e Loteamentos foram validados no navegador sem mutação; 230 arquivos/623 testes, tipagem, build, diff e artefatos saneados aprovados.
- [x] Salvar checkpoint funcional A276 após validação, documentação e artefatos saneados, sem publicar ou executar mutação material. **A276:** checkpoint funcional registrado após validar MFA no login e sessão autorizada, com comandos permitidos sem desafio repetido e sem mutação material durante a revisão.
- [x] Salvar checkpoint de consolidação A276 após registrar o checkpoint funcional no checklist. **A276:** consolidação registrada sem publicação ou alteração de preço, Lote, perfil, estrutura, permissão, contrato ou financeiro.

## Pré-preenchimento de referência interna por Lote
- [x] Diagnosticar por que o valor por m² exibido no cartão não preenche o editor contextual do mesmo Lote, preservando valor vazio apenas para a exceção intencionalmente sem referência. **A277:** a referência autorizada era exibida no cartão, mas não era copiada ao rascunho do editor selecionado.
- [x] Pré-preencher o valor por m² atual no editor do Lote selecionado, recalcular o total referencial para conferência e manter edição governada por sessão, contexto, política, vigência, motivo, respaldo, correlação, idempotência e auditoria. **A277:** o editor recebe a referência atual da mesma unidade e mostra total derivado sem campo de edição independente; a exceção vazia continua vazia.
- [x] Atualizar o cartão somente após alteração material autorizada e confirmada pelo servidor, sem criar mutação durante testes e sem mostrar preço comercial, disponibilidade, venda, contrato ou financeiro. **A277:** as respostas autorizadas invalidam as leituras do cartão; nenhuma mutação foi executada no teste.
- [x] Cobrir seleção, pré-preenchimento, exceção vazia, cálculo derivado e atualização pós-sucesso; validar no navegador sem editar valor real, gerar ZIP/HTML saneados e checkpoints após comprovação prática. **A277:** seleção autenticada mostrou o valor atual e o total derivado no editor; 230 arquivos/623 testes, tipagem, build, diff e artefatos saneados aprovados. Checkpoints seguem como etapas separadas.
- [x] Salvar checkpoint funcional A277 após validação, documentação e artefatos saneados, sem publicar ou alterar preço real. **A277:** checkpoint funcional registrado após confirmar em sessão autenticada o valor por m² pré-preenchido e o total derivado no editor, sem digitação ou salvamento de preço.
- [x] Salvar checkpoint de consolidação A277 após registrar o checkpoint funcional no checklist. **A277:** consolidação registrada sem publicação, digitação de preço, preparação de condição, gravação de valor, edição de Lote ou alteração de disponibilidade, venda, contrato ou financeiro.

## Referências operacionais curtas em Loteamentos
- [x] Inventariar referências internas visíveis em cartões, editores, formulários, listas e mensagens, preservando os identificadores técnicos completos apenas no servidor, na auditoria e nas chaves de idempotência. **A278:** foram identificadas as superfícies de editor, condições, políticas, lista e seletores; valores técnicos continuam usados somente como estado, chave ou comando interno.
- [x] Definir e aplicar uma apresentação operacional curta, consistente e legível para referências de preço e demais fluxos de Loteamentos, sem modificar referências já registradas ou seus vínculos técnicos. **A278:** ajustes, políticas, condições e seletores passaram a usar rótulos curtos; identificadores técnicos de novas preparações são gerados automaticamente, sem campo de digitação para o operador.
- [x] Cobrir compatibilidade, legibilidade, responsividade e ausência de alteração material; gerar ZIP/HTML saneados e checkpoints após validação integral. **A278:** 232 arquivos de teste e 628 testes aprovados, tipagem, build e diff íntegros; revisão de tela ampla e celular e saneamento de ZIP/HTML concluídos. Nenhuma alteração material foi executada.
- [x] Salvar checkpoint funcional A278 após registrar a padronização visual, validação integral e artefatos saneados. **A278:** checkpoint funcional salvo após a substituição integral de referências técnicas visíveis por rótulos curtos, testes, build, revisão responsiva e saneamento; nenhuma referência persistida ou dado material foi modificado.
- [x] Salvar checkpoint de consolidação A278 após registrar o checkpoint funcional no checklist. **A278:** consolidação salva com referências operacionais curtas em todas as superfícies visíveis de Loteamentos; identificadores técnicos seguem internos e nenhum dado, vínculo, cálculo, preço, política ou condição foi alterado.

## Clientes Loteadora: cadastro-base, qualificação e prontidão documental — A279
- [x] Consolidar o estudo externo, inventário preservativo e estratégia saneada para contato mínimo, perfil cadastral, pendências condicionais e preferências de contato, mantendo fora de escopo crédito, venda, contrato, registro e financeiro. **A279:** fontes institucionais confrontadas; decisão registrada em `docs/estrategia-clientes-loteadora-a279.md`, sem dados pessoais, fontes privadas em artefatos ou operações materiais.
- [x] Definir e testar contratos fechados para perfil cadastral, requisitos condicionais e permissões de contato, rejeitando campos, estados e vínculos de outros domínios. **A279:** contratos estritos e testes focais aprovados; o conjunto rejeita campos de renda/crédito, estados comerciais, finalidade indevida e identificadores de perfil fora da entrada contextual.
- [x] Implementar persistência privada e RPCs aditivas com RLS fail-closed, autoridade contextual, idempotência e auditoria redigida, sem criar dados pessoais reais. **A279:** três tabelas privadas e sete funções protegidas foram aplicadas no Supabase via migração PostgreSQL versionada; a conferência agregada confirmou contagens iniciais iguais a zero.
- [x] Compor a nova experiência dentro de Clientes Loteadora, preservando o cadastro de comprador existente, intenção privada de anexo, envio opaco, prontidão e exportação redigida. **A279:** a central progressiva foi posicionada após o fluxo opaco de anexo e antes da síntese de prontidão; validações estáticas confirmam a coexistência e a ausência de transição comercial ou exportação individual.
- [x] Validar a jornada inteira, bloqueios, estados vazios e responsividade; gerar ZIP/HTML saneados e salvar checkpoints somente após testes, tipagem, build e integridade aprovados. **A279:** 236 arquivos de teste e 641 testes aprovados; tipagem, build Netlify e `git diff --check` concluídos; página inteira revisada em 1920, 1280, 768 e 375 pixels sem interação material. ZIP e HTML autônomo foram gerados e saneados; a migração continua sem perfis, pendências ou preferências de contato criados.
- [x] Salvar checkpoint funcional A279 após estudo, implementação, validação e artefatos saneados, sem publicar ou criar cadastro pessoal real. **A279:** checkpoint funcional `6435e8bb` registrado; preserva o perfil complementar privado, as pendências condicionais, as preferências por finalidade/canal e a exclusão explícita de fluxos comerciais, financeiros, contratuais e registrais.

## Evolução integral de Clientes Loteadora — A280
- [x] Inventariar integralmente as novas especificações privadas e a estrutura da planilha de clientes, sem reproduzir dados pessoais, identificadores, contatos ou conteúdo de documentos em código, histórico, relatórios ou artefatos. **A280:** as três especificações foram lidas até o fim; a planilha foi analisada localmente apenas por estrutura agregada — uma aba, 111 linhas não vazias e nove colunas, com categorias de identificação, contato, situação, vínculo comercial e campos a classificar. Nenhum valor individual foi registrado no projeto.
- [x] Pesquisar e confrontar referências atuais de CRM imobiliário, experiência operacional, privacidade e segurança documental para separar padrões úteis de funcionalidades que pertencem a venda, crédito, contrato, registro e financeiro. **A280:** foram confrontadas referências públicas de Salesforce, HubSpot, Pipedrive, Microsoft, OWASP, ANPD e NIST, além de duas demonstrações públicas de CRM. Os achados de produto e limites foram mantidos exclusivamente em registro privado fora do repositório.
- [x] Definir uma estratégia de evolução por camadas para lista operacional, busca minimizada, ficha em página única, checklist condicional, histórico redigido, notas governadas e anexos privados, preservando todos os fluxos A279. **A280:** estratégia saneada registrada em `docs/estrategia-evolucao-integral-clientes-loteadora-a280.md`; notas livres e central documental por categoria foram conscientemente postergadas para não ampliar tratamento de dados antes de controles próprios.
- [x] Implementar somente camadas cadastrais e documentais autorizadas, com contratos estritos, RLS fail-closed, contexto, autoridade, idempotência e auditoria redigida, sem criar transições comerciais ou dados pessoais reais. **A280:** diretório paginado, busca mínima, painel de ficha, atalho contínuo para A279 e linha do tempo redigida foram adicionados por duas RPCs privadas; nenhuma tabela de dados pessoais, nota livre, download, visualização de arquivo, venda, crédito, contrato, registro ou financeiro foi criada.
- [x] Preparar testes sintéticos e uma jornada de validação sem mutação para confirmar lista, filtros, ficha, checklist, anexos, histórico, notas, responsividade e preservação das jornadas existentes. **A280:** testes focais e integrais aprovaram 240 arquivos e 652 testes; tipagem, build, integridade do diff e revisão de página inteira em 1920, 1280, 768 e 375 pixels foram concluídos sem criar, editar, anexar, importar ou consultar dados individuais. A leitura autenticada no navegador foi tentada, mas expirou sem executar ação e sem produzir efeito material.
- [x] Avaliar a planilha apenas na fase final de migração controlada; antes de qualquer inserção, exigir autorização material específica, confirmação de contexto e revisão da prévia privada sem dados expostos. **A282:** a fonte foi reclassificada pelo proprietário como sintética e foi usada somente em jornadas pontuais autorizadas; não entrou no projeto, no pacote ou em relatórios.
- [x] Registrar a decisão, validar, gerar ZIP/HTML saneados e salvar checkpoints sem publicação; somente depois considerar cadastro manual, pessoa a pessoa, pela jornada aprovada. **A282/A283:** decisões, validações integrais, checkpoints e artefatos saneados foram concluídos; o cadastro individual remanescente foi posteriormente substituído por preparação prática preservativa.

## Correção de visibilidade e execução material solicitada — A280
- [x] Sincronizar a prévia com a central contextual A280 e confirmar em jornada autenticada que ela aparece entre o relatório redigido e o cadastro-base, sem ocultar as seções anteriores ou posteriores. **A281:** a central ficou visível na sessão autenticada, exibindo cadastro direto, indicadores, busca, lista e painel de ficha em continuidade com os fluxos anteriores.
- [x] Validar a leitura contextual da lista e da ficha nova antes de qualquer gravação, mantendo busca, histórico redigido, perfil A279, intenção de anexo e prontidão na sequência contínua. **A282:** a seleção da lista foi corrigida e confirmada na sessão autenticada; a ficha acompanha o cartão selecionado e apresenta somente informações permitidas no contexto.
- [x] Cadastrar manualmente os registros privados autorizados, um a um, somente pelos campos de cadastro-base aprovados e no contexto vigente; interromper diante de divergência, duplicidade ou campo fora de escopo. **Superado por ordem posterior do proprietário:** o setor foi aceito como pronto e a etapa individual foi substituída por preparação prática, auditoria agregada, ciclo reversível e busca escalável, sem inferência, duplicidade ou preenchimento automático.
- [x] Criar uma entrada direta de cliente comprador, com nome declarado e natureza de pessoa, para que o cadastro manual ocorra na própria jornada de Clientes Loteadora sem depender de um papel prévio invisível ao operador. **A281:** formulário de entrada direta composto na central A280, com nome declarado, natureza PF/PJ e retorno imediato para lista, ficha e perfil; nenhuma informação de contato, documento, venda ou financeiro entra nessa criação.
- [x] Garantir por contratos, RPCs, RLS, validações e testes que a entrada direta não duplica `Party`, não cria dados financeiros/comerciais e não permite campos ou relações fora do contexto autorizado. **A281:** comando único server-side liga Party, papel temporal e cliente comprador de modo idempotente; migração aplicada sem criar registros e testes focais, tipagem, build e integridade aprovados.

## Correção estrutural de Clientes Loteadora — A282
- [x] Remover da experiência visível os rótulos de comprador, compra e rascunho, substituindo-os por Cliente Loteadora e Cadastro de Cliente, sem alterar os vínculos internos preservados nem criar fluxo de venda. **A282:** a central, a ficha e a jornada de criação passaram a usar a terminologia de Cliente Loteadora; os vínculos internos compatíveis permanecem restritos ao servidor.
- [x] Modelar e implementar ficha cadastral privada editável para meios de contato, CPF, RG e condição documental, com campos opcionais, finalidade clara, acesso contextual, auditoria redigida e nenhum dado financeiro ou comercial. **A282:** telefone, WhatsApp, e-mail, CPF/CNPJ e RG/documento são exibidos exclusivamente na ficha autorizada e entram somente por edição deliberada; o diretório não lista esses dados.
- [x] Reorganizar e tornar visível a jornada de documentos privados, preservando o fluxo de anexo já existente e sem expor arquivos, URLs, chaves, download ou visualização fora da autoridade autorizada. **A282:** o atalho de documentos privados ficou disponível na ficha e preserva o envio opaco de arquivo único sem ampliar visualização ou compartilhamento.
- [x] Implementar exclusão controlada, auditada e reversível do cadastro ativo, com confirmação explícita e sem apagar rastreabilidade, para testar a jornada antes do recadastro manual autorizado. **A282:** a exclusão agora arquiva o item por confirmação acessível, mantém a auditoria redigida e oferece restauração no mesmo contexto.
- [x] Adicionar testes de contrato, contexto, RLS, rejeição, edição, exclusão/restauração, interface e responsividade antes de executar operações sobre a lista privada. **A282:** contratos, serviços, migração, seleção contextual, confirmação acessível, recuperação de sessão e interface receberam cobertura; a validação integral, tipagem, build e integridade foram aprovados antes do teste sintético.
- [x] Executar a exclusão e a restauração como teste sintético de ponta a ponta; em seguida, limpar somente os registros sintéticos já cadastrados e recadastrar a lista sintética atualizada manualmente, sem usar campos de venda, crédito, contrato ou financeiro. **A282:** arquivamento, restauração e reativação idempotente foram validados na interface. Por autorização posterior do proprietário, o recadastro individual remanescente foi substituído por preparação prática preservativa; nenhum registro foi limpo, duplicado, inferido ou preenchido automaticamente.
- [x] Corrigir a hierarquia visual dos controles da ficha, com botões separados, rótulos legíveis, foco acessível e confirmação embutida; eliminar qualquer dependência de diálogo nativo que possa prender a automação de teste. **A282:** os controles foram separados em ações primária, secundária e destrutiva, e o arquivamento passou a usar confirmação acessível dentro da própria ficha.
- [x] Verificar que seleção, ficha, edição, documentos, arquivamento e restauração permanecem operáveis após recarga, sem confundir indisponibilidade da sessão remota com erro do CRM. **A282:** as jornadas contextualizadas recuperaram o estado autorizado após recarga, abriram ficha única, confirmaram edição, arquivamento e restauração. A cobertura documental confirma retorno opaco, sem `storageKey` ou URL; a interface não oferece visualização nem download. Timeouts posteriores da ponte remota foram tratados como limitação da automação, sem inferir falha no CRM nem executar novos comandos.
- [x] Corrigir a propagação do cliente selecionado entre a central e o editor, para que “Editar dados cadastrais” abra a ficha completa já vinculada ao mesmo Cliente Loteadora. **A282:** central, editor, documentos e prontidão passaram a usar a mesma fonte autorizada; a seleção é preservada localmente durante atualizações concorrentes de consulta.
- [x] Garantir paridade entre ficha-resumo e editor: todo campo apresentado como editável deve ter controle, validação, estado vazio, salvar/cancelar e atualização confirmada pelo servidor. **A282:** o editor inclui natureza, situação, CPF/CNPJ, RG/documento, e-mail, telefone, WhatsApp, situação civil e representação, com descarte local e persistência somente após retorno autorizado do servidor.
- [x] Diagnosticar a causa da instabilidade de navegação e carregamento da área protegida após recarga, rolagem ou ação na central, separando sessão válida, ponte do navegador, invalidação de consultas e estado visual. **A282:** a investigação isolou uma combinação de importação tardia da rota crítica, refetchs de identidade em montagem/reconexão e leituras concorrentes da sessão; o servidor e seus controles permaneceram preservados.
- [x] Corrigir a causa e adicionar testes que provem que a tela não entra em carregamento permanente nem perde seleção após recarga, navegação, edição, arquivamento ou restauração. **A282:** a rota crítica é carregada estaticamente, a ponte de sessão usa leitura única/cache curto e o editor preserva a seleção local; a jornada autenticada confirmou recarga, edição, arquivamento e restauração sem carregamento persistente.
- [x] Compor o editor completo diretamente no painel da ficha do Cliente Loteadora selecionado, preservando o editor de página existente como jornada compatível e eliminando a dependência de rolagem para editar dados básicos. **A282:** a ficha contextual passou a abrir o mesmo editor completo no cartão selecionado; a seção original continua disponível como caminho compatível.
- [x] Corrigir a rolagem interna da lista da Central de Clientes e oferecer uma forma inequívoca de alcançar qualquer cadastro retornado, sem afetar seleção, edição, arquivamento, restauração ou os demais setores. **A282:** a lista deixou de ter uma rolagem aninhada e passou a acompanhar a rolagem natural da página; a ficha permanece fixa e rolável apenas quando seu próprio conteúdo excede a altura útil. Testes, tipagem, build e integridade foram aprovados.
- [x] Corrigir o salvamento da ficha contextual para que o comando emita a mutação protegida, crie ou atualize o perfil correspondente e reflita os campos permitidos no resumo somente após confirmação do servidor. **A282:** a mutação devolve a ficha confirmada pelo servidor e atualiza o cache contextual somente então; uma jornada sintética autorizada salvou uma ficha sem acrescentar dados pessoais e a Central passou a indicar o perfil organizado somente após a resposta de sucesso.
- [x] Remover dos novos comandos cadastrais a exigência indevida de MFA recente por operação e alinhá-los à política A276 de sessão AAL2 válida, preservando identidade, contexto, membership, grant, papel, escopo, finalidade, correlação, idempotência e auditoria no servidor. **A282:** os comandos de perfil, pendência, preferência, arquivamento e restauração usam a atestação de sessão AAL2 vigente; a cobertura confirma recusa fora desse estado e a jornada de salvamento autorizada não exigiu novo desafio por comando.
- [x] Conciliar a leitura da Central de Clientes com os vínculos compatíveis existentes para que a busca localize todos os cadastros ativos autorizados e nunca apresente ausência falsa antes de edição ou recadastro. **A282:** a auditoria agregada e a recarga integral da prévia confirmaram 24 cadastros ativos devolvidos pela leitura contextual, com os arquivados mantidos em lista reversível separada; o primeiro carregamento anterior de 18 foi estado anterior da prévia, não ausência de registros. A página inicial agora usa o limite permitido, reseta paginação e seleção apenas em troca real de contexto e preserva paginação para crescimento.
- [x] Restringir a consulta legada de resumos, que retorna acesso negado após recarga, ao setor compatível que ainda a exige; ficha, lista e indicadores de Clientes Loteadora devem depender somente das consultas contextuais autorizadas. **A282:** a consulta antiga agora é limitada à compatibilidade de Vendas de Lotes; após reinício e recarga, a Central recuperou seu contexto e a listagem autorizada sem disparar o bloqueio legado.
- [x] Impedir que a redefinição do callback de seleção pela página setorial limpe o cartão recém-selecionado na Central antes de abrir a ficha contextual. **A282:** o callback de seleção da página foi estabilizado; a seleção persiste, abre a ficha completa e só é limpa por troca real de contexto ou ciclo de vida confirmado.
- [x] Adicionar uma âncora acessível à lista de cadastros arquivados para que a restauração reversível seja alcançável sem depender de rolagem longa ou frágil. **A282:** a Central apresenta atalho fixo, `aria-controls` e âncora de arquivados; a lista expande no mesmo contexto e restauração pode ser executada também por teclado.
- [x] Corrigir a inicialização prematura da ponte de sessão para aguardar o token de acesso e reexecutar as consultas de contexto quando ele chegar, evitando que a Central de Clientes permaneça sem contexto após recarga. **A282:** uma recarga integral iniciou no estado seguro de carregamento, recuperou o contexto autorizado e devolveu a leitura agregada completa da Central sem novo login, intervenção manual ou erro de contexto.
- [x] Preservar o último contexto autorizado e a área autenticada durante refetchs transitórios, limitando invalidações por troca real de token e impedindo que foco, seleção ou recarga desmontem a Central de Clientes. **A282:** `lastAuthorizedContext` só é reutilizado para a mesma organização durante refetch, `retainAuthorizedSelection` protege as seleções enquanto a leitura fica temporariamente indefinida e as invalidações ficaram restritas às consultas contextuais. A recarga autenticada recuperou a Central e a nova regressão cobre a retenção sem permitir contexto de outra organização.
- [x] Abrir o editor cadastral completo diretamente ao selecionar um cartão da Central de Clientes, com resumo recolhível e ações de salvar ou descartar no mesmo painel, sem retirar o editor compatível de página. **A282:** a seleção do cartão abre imediatamente o editor contextual do mesmo Cliente Loteadora; o resumo e a página de edição compatível permanecem disponíveis.

- [x] Mover o foco programaticamente para a ficha contextual recém-aberta, sem rolagem brusca, para que os campos permitidos permaneçam alcançáveis após selecionar um cartão. **A282:** o formulário contextual recebe foco uma única vez para cada seleção já confirmada, com `preventScroll`; a regressão cobre o comportamento e a tipagem foi aprovada.

- [x] Garantir que o editor contextual recém-focado também seja trazido a uma posição visível, com movimento respeitoso e sem alterar o foco de quem já está editando. **A282:** a primeira abertura de cada ficha selecionada desloca o formulário ao início visível com movimento suave, ou imediato quando a preferência de redução de movimento estiver ativa; as edições subsequentes não perdem foco.

- [x] Consolidar a ficha completa em uma única instância contextual na Central, preservando a página compatível sem montar outro formulário com os mesmos IDs, âncora e foco na mesma rota. **A282:** a Central monta somente o editor embutido do cartão selecionado; a ficha detalhada compatível passou a existir em rota própria, aberta explicitamente. A cobertura da Central, perfil e página setorial foi aprovada com tipagem e integridade do diff.

- [x] Registrar a rota compatível da ficha detalhada no roteador principal para que a abertura explícita não resulte em página inexistente. **A282:** a rota detalhada é resolvida antes da rota geral de Clientes Loteadora e sua cobertura específica, a Central, o perfil e a tipagem foram aprovados.

- [x] Reaplicar a âncora do seletor da ficha após o carregamento contextual, para que o acesso direto ao editor permaneça visível mesmo quando a sessão recupera dados após a navegação inicial. **A282:** o efeito espera o workspace autorizado e a lista contextual antes de rolar para a âncora; a rota compatível e sua cobertura confirmam a recuperação segura.

- [x] Estabilizar as referências primitivas do contexto e restringir as invalidações do diretório ao input contextual afetado, impedindo refetchs concorrentes que deixem a Central em carregamento transitório. **A282:** o objeto de contexto depende somente de organização e finalidade primitivas; cadastro, arquivamento e restauração invalidam exclusivamente a primeira leitura contextual do diretório. As regressões de sessão, Central, contexto e tipagem foram aprovadas.

- [x] Reaplicar a âncora do seletor da ficha quando o contexto terminar de carregar e invalidar somente a leitura contextual correspondente após salvar um perfil, evitando refetchs globais na rota compatível. **A282:** a ficha reaplica a âncora do seletor somente depois de o contexto estar pronto e o salvamento invalida apenas a primeira página do diretório do mesmo contexto; a cobertura de perfil, Central, página e tipagem foi aprovada.
...
- [x] Substituir o recadastro individual restante, por autorização posterior do proprietário, por uma preparação sintética prática e idempotente que apenas preserve cadastros existentes, perfis já confirmados e trilhas de auditoria, sem inferir ou preencher campos ausentes. **A282:** auditoria agregada confirmou 24 ativos, 3 arquivados, zero vínculos temporais duplicados, três perfis em três cadastros distintos e apenas os canais já confirmados. Nenhum campo ausente foi inferido, inserido ou associado a outra ficha.

- [x] Gerar e inspecionar o ZIP de código e o HTML autônomo saneados do marco A282. **A282:** os dois artefatos foram gerados a partir do build aprovado e verificados contra inclusão de segredos, endpoints, arquivos de ambiente, logs, documentos, planilhas, leitores privados e dados auxiliares.

- [x] Redigir caminhos locais eventualmente incluídos pelo bundle no HTML autônomo A283 e regenerar os artefatos antes da entrega. **A283:** o empacotador agora redige referências locais; o ZIP e o HTML regenerados foram validados contra arquivos privados, caminhos locais, endpoints, credenciais, URLs de banco e funções privilegiadas.

## Evolução de escala da Central e da Ficha de Clientes Loteadora

- [x] Substituir o seletor linear da ficha compatível por uma busca contextual escalável, com filtro autorizado por nome declarado, telefone, WhatsApp, e-mail ou referência de identificação e resultados paginados em área com rolagem própria. **A283:** a busca usa função protegida, paginação de 25 resultados, termo mínimo, lista minimizada e área rolável. Os campos declarados participam apenas do filtro no servidor e não retornam na projeção de resultados.
- [x] Permitir editar o nome declarado do Cliente Loteadora na ficha contextual por comando protegido, com validação, deduplicação no servidor, confirmação de retorno e atualização da busca e do resumo apenas após êxito. **A284:** migração aditiva aplicada, RPC protegido e correlacionado retornando o alvo confirmado, atualização local apenas após resposta autorizada e cobertura de identidade, correlação e confirmação aprovada.
- [x] Reorganizar a ficha para destacar CPF/CNPJ como identificação principal e manter RG/documento somente como referência complementar opcional, sem tornar dados cadastrais ausentes obrigatórios sem finalidade. **A284:** o campo de CPF/CNPJ aparece antes dos demais dados de cadastro, com identificação principal explícita, orientação de finalidade e resumo de presença; RG/documento permanece preservado como campo complementar.
- [x] Revogar execução direta por papéis anônimos e autenticados da leitura de perfil cadastral, preservando o acesso exclusivo pelo servidor com os guardas contextuais existentes. **A285:** migração aditiva aplicada e verificação de segurança posterior confirmou a ausência da função de perfil entre avisos de execução direta. O adaptador de serviço, os guardas contextuais e a leitura privada foram preservados.
- [x] Mapear e preparar a qualificação de cônjuge, parente, amigo ou outro cointeressado como pessoa relacionada declarada, com finalidade cadastral, consentimento e revisão humana, sem criar vínculo de venda, crédito, proposta, contrato, registro ou financeiro. **A284:** estratégia saneada preserva entidade distinta, relação fechada, finalidade explícita, revisão humana e auditoria redigida; não houve migração, registro individual ou expansão comercial nesta preparação.
- [x] Registrar os princípios de minimização, finalidade, transparência, segurança e revisão humana para futura qualificação de pessoas relacionadas, sem criar registros individuais ou domínio comercial. **A284:** decisão saneada documentada a partir de referências institucionais de proteção de dados; a implementação permanece separada e exige autorização material própria.
- [x] Manter a ficha completa, inclusão, edição confirmada, arquivamento e restauração reversíveis acessíveis diretamente a partir do resultado selecionado, sem abrir venda, lote, crédito, proposta, contrato, registro, cobrança, pagamento ou financeiro. **A283:** um resultado contextual abre a ficha única editável; os comandos de ciclo de vida e inclusão continuam no atalho da Central, sem novo domínio de negócio.
- [x] Remover a duplicação visual e a linguagem de “jornada compatível” da área de busca, apresentando-a como estoque cadastral com busca, seleção e edição em uma única experiência operacional. **A283:** a rota passou a chamar-se “Busca e edição de Clientes Loteadora”; o seletor linear e a lista de vínculo não são montados nesse caminho.
- [x] Recolher o vínculo compatível residual da Central em uma ação avançada, mantendo o fluxo preservado sem deixar seus cartões lineares expandirem a página em diretórios grandes. **A283:** o vínculo temporal continua disponível em uma divulgação avançada recolhida e sua listagem linear foi removida da página. A Central mantém busca, cadastro direto, edição, arquivamento e restauração como ações principais.

## Estoque operacional de prontidão de Clientes Loteadora

- [x] Converter a leitura de prontidão em uma lista operacional com rolagem própria, paginação limitada e estado vazio claro, sem renderizar o diretório inteiro na página. **A286:** a leitura protegida retorna somente uma janela de 25 cartões e apresenta uma barra de rolagem visível, trilho estável, paginação e estado vazio explícito.
- [x] Exibir em cada cartão operacional somente nome declarado, telefone, WhatsApp e situação de cadastro quando o contexto autorizar, mantendo e-mail, identificação, documentos e detalhes dentro da ficha protegida. **A286:** a projeção dedicada minimiza o retorno e usa contatos apenas quando já registrados; campos ausentes aparecem como não informados, sem inferência.
- [x] Permitir abrir diretamente a ficha completa do mesmo cliente a partir de cada cartão operacional, preservando busca, edição confirmada, inclusão, arquivamento e restauração na Central. **A286:** a ação de edição foi validada navegando à ficha contextual selecionada; o fluxo não criou ou alterou dados durante a validação.

## Cadastro prático de dados sintéticos autorizados

- [x] Auditar a planilha sintética fora do repositório e medir somente campos, linhas válidas, duplicidades internas e cobertura de dados sem expor valores individuais. **A287:** o lote contém 110 linhas com dados, 109 CPF/CNPJ e 109 nomes canônicos distintos, com uma repetição canônica consolidável; há WhatsApp em 109 e telefone em 76. A planilha permaneceu fora do projeto e nenhuma linha individual foi registrada no checklist.
- [x] Preparar e executar o cadastro idempotente dos registros sintéticos na camada de Clientes Loteadora, preservando vazios de origem e proibindo domínio comercial, financeiro, contratual ou registral. **A287:** migração aditiva aplicada; a operação processou 110 linhas e confirmou 110 operações cadastrais/perfis, consolidando-as em 109 identidades únicas pelo CPF/CNPJ declarado, sem inventar e-mail, contatos ou outros campos ausentes.
- [x] Conciliar o resultado por métricas agregadas, conferir a visibilidade no estoque rolável e registrar a execução saneada antes da entrega. **A287:** a leitura posterior confirmou 109 clientes ativos, 109 perfis e 109 referências fiscais; 76 telefones e 108 canais de mensageria refletem somente a fonte. Em desktop, a Central carregou uma página de 25 cartões, com rolagem própria e abertura não material da ficha; em 375 px, a rota preservou estrutura responsiva antes da seleção de contexto.
- [x] Reativar, de forma idempotente e auditada, cadastros do lote sintético que já existiam arquivados quando a referência fiscal coincidir, para que o estoque ativo represente o conjunto autorizado completo. **A288:** aplicada e executada com zero coincidências arquivadas por referência fiscal, portanto não houve reativação adicional. A auditoria A287 demonstrou que três reativações diretas haviam ocorrido antes da releitura de estado; **A289** foi aplicada para corrigir prospectivamente a métrica de reativação, sem repetir a importação nem alterar o lote já conciliado.
- [x] Executar cobertura focal e integral, gerar ZIP e HTML saneados e remover envelopes, utilitários e captura autenticada privados da etapa. **A287–A289:** sete testes focais, suíte integral, checagem de tipos, build Netlify e integridade do diff foram aprovados; artefatos de entrega foram inspecionados contra planilha, dados individuais, logs, leitores privados, caminhos locais, credenciais e URLs reais. Permanecem apenas avisos não bloqueantes de chunk de bundle acima do limiar recomendado.

## Correção de clareza da paginação do estoque de Clientes Loteadora

- [x] Corrigir os indicadores e a paginação da Central para apresentar o total cadastrado autorizado separado da primeira janela de 25 itens, sem duplicar, ocultar, alterar ou recadastrar clientes já existentes. **A290:** a contagem agregada protegida foi aplicada e a Central passou a indicar 109 cadastros ativos no contexto, com a primeira janela explicitamente marcada como 25 itens; o único registro não materializado como nova identidade é a repetição canônica já conciliada na fonte de 110 linhas.
- [x] Destacar visualmente a ação de cadastrar novo Cliente Loteadora, com hierarquia, texto e foco inequívocos para o operador, preservando o formulário, a autorização contextual e os demais controles existentes. **A290:** o formulário ganhou faixa de ação principal, contraste próprio e botão “Cadastrar novo Cliente Loteadora”, sem mudar validação, finalidade, sessão, autorização ou qualquer registro existente.
- [x] Cobrir o contrato de total, a cópia da interface, o avanço de página, a rolagem e a responsividade; validar no CRM e salvar checkpoint saneado sem nova importação ou mutação cadastral. **A290:** testes focal e integral aprovaram 255 arquivos e 703 testes, além de tipagem, build Netlify e integridade do diff. A sessão autenticada confirmou o total agregado, a janela 1–25 e a ação de próxima página; a tela móvel foi revisada em estado seguro. ZIP/HTML foram regenerados e saneados; o checkpoint restaurável foi salvo na conclusão deste marco.

## Habilidade reutilizável de cadastro sintético cadastral

- [x] Criar e validar uma habilidade reutilizável para operações de cadastro sintético de Clientes Loteadora: escopo cadastral, deduplicação, conciliação agregada, paginação clara, proteção de dados, testes e entrega saneada, sem incluir dados individuais, credenciais ou utilitários privados. A habilidade `subdivision-synthetic-client-registry` foi criada fora do repositório do CRM e aprovada pelo validador estrutural; contém apenas orientações gerais e modelo de evidência saneada.

## Programa de unificação — Central de Vendas

- [x] Estudar integralmente os materiais fornecidos, a arquitetura atual, referências regulatórias e os impactos entre Clientes Loteadora, Vendas de Lotes, Loteamentos, contratos, financeiro, documentos, agenda, alertas, relatórios e auditoria, sem mover, ocultar, apagar ou iniciar operações financeiras. A pesquisa e os materiais foram consolidados em documentação interna saneada, com separação explícita entre controle interno, emissão bancária e baixa financeira.
- [x] Produzir uma estratégia de arquitetura preservativa para renomear a experiência de Clientes Loteadora como Central de Vendas e incorporar progressivamente a jornada de Vendas de Lotes, mantendo a rota compatível e o acesso individual existente até a migração estar validada. A estratégia estabeleceu a Central de Vendas como orquestradora das fontes preservadas e a migração em marcos M1–M6, sem fusão de tabelas ou apagamento de rotas.
- [x] Definir o fluxo transacional governado de seleção de lote, identificação ou cadastro de cliente, negociação flexível, contrato, geração de títulos, atualização de estoque, alertas e sincronização, com limites de autorização, concorrência, idempotência, reversibilidade e auditoria antes de qualquer implementação material. A arquitetura-alvo formaliza estados separados, bloqueio concorrente do lote, versões imutáveis após aprovação e agenda interna sem boleto, banco ou baixa simulada.
- [x] Implementar, testar, validar em desktop e mobile, documentar, sanear e criar checkpoints por marco para a Central de Vendas, sem publicação automática e sem integração bancária ou envio de cobrança até que cada camada esteja expressamente autorizada e pronta. **M1–M7:** a Central unificou clientes, preparação de venda, formalização interna, agenda de parcelas, aprovação comercial, estoque, dossiê privado, lote interno no Financeiro e lembretes ao operador; todos os marcos foram validados e mantêm a proibição de emissão, mensagens externas, banco, baixa e pagamento automático.

### Marco M1 — superfície e rotas compatíveis

- [x] Renomear a experiência de Clientes Loteadora para Central de Vendas, preservando busca, cadastro, ficha, documentos privados, arquivamento, restauração, estoque rolável e links compatíveis. **M1:** a navegação lateral e o cabeçalho agora apresentam Central de Vendas, enquanto o caminho de Clientes e sua ficha compatível foram preservados.
- [x] Reposicionar a jornada atual de Vendas de Lotes como uma área interna da Central de Vendas, sem apagar `subdivision_sale_drafts`, co-compradores ou estados de trabalho e sem criar efeitos comerciais, contratuais ou financeiros neste marco visual. **M1:** a Central recebeu as áreas locais Clientes e documentos e Preparação de venda; a nomenclatura provisória visível foi substituída por preparação operacional sem mudar modelos, estados ou comandos existentes.
- [x] Manter `/loteadora/vendas` como rota compatível que abre a Central de Vendas na jornada de vendas, remover somente o item de navegação individual após a confirmação visual e cobrir os redirecionamentos, acessibilidade e responsividade. **M1:** a rota compatível abre a área Preparação de venda, sem item lateral individual; a sessão autenticada confirmou contexto, navegação e formulário em modo não material. Cobertura focal e integral aprovaram 255 arquivos e 705 testes, tipagem, build Netlify, integridade do diff e revisão visual. ZIP/HTML saneados foram gerados sem dados individuais, credenciais, URLs, caminhos locais, planilhas ou capturas autenticadas.

### Marco M2 — caso comercial e termos de negociação

- [x] Criar caso de venda idempotente por lote e organização, preservando as preparações internas existentes, impedindo concorrência ativa do mesmo lote e respeitando reservas físicas, contexto, sessão AAL2, alçada e auditoria redigida. **A291:** adicionou casos com estados de preparação/revisão, índice único parcial por lote e organização, validação da reserva física e comandos de abertura/listagem/termos protegidos; a abertura do caso não modifica estoque, contrato, cobrança, boleto, pagamento ou baixa.
- [x] Exibir na Central de Vendas a referência física e de preço já autorizada do lote escolhido, localizar ou vincular cliente existente e registrar termos flexíveis em preparação sem criar reserva, contrato, boleto, pagamento ou baixa. **A291/A292:** a jornada conecta loteamento, quadra, lote, referência econômica autorizada e cliente cadastrado; a consulta por CPF/CNPJ é correlacionada, minimizada e nunca vai à auditoria. Os termos revisáveis aceitam total, entrada, parcelas, primeiro vencimento e dia de vencimento, sempre como preparação interna sem gerar título financeiro.
- [x] Cobrir a concorrência, idempotência, regras de estado, campos monetários e jornada visual do caso de venda em desktop e mobile; validar de forma integral e registrar artefatos saneados antes de avançar para contrato ou recebíveis. **M2:** cobertura focal aprovou 20 testes e a validação integral aprovou 256 arquivos e 708 testes, além de tipagem, build Netlify, integridade do diff e revisão visual desktop/móvel. A292 foi aplicada; os avisos de RLS sem política são informativos de tabelas fail-closed com execução revogada ao público/autenticado e concedida só ao serviço protegido. ZIP/HTML foram gerados e inspecionados saneados.

### Marco M3 — contrato interno, agenda de recebíveis e alertas

- [x] Criar a etapa de revisão e formalização interna do contrato a partir de termos preparados, com versão imutável, documentos opacos, aprovação segregada, arquivamento reversível e sem emissão bancária ou assinatura automática. **A293:** a formalização transforma somente o caso com termos em revisão interna, preserva o contrato como preparação auditada e não cria assinatura, arquivo público, boleto ou efeito bancário.
- [x] Gerar de forma atômica uma agenda interna de parcelas a partir dos termos aprovados, com vencimentos flexíveis, estado inicial “aguardando emissão bancária”, idempotência, conciliação e sem criar boletos, cobrar ou dar baixa. **A293:** a agenda deriva da entrada e do parcelamento negociados, suporta quantidade e dia de vencimento configuráveis, e permanece como referência interna sem código de barras, remessa, baixa ou pagamento.
- [x] Implementar a leitura interna de atenção de proximidade e anterioridade de parcelas para operadores autorizados, sem enviar mensagens externas, cobrar cliente, inferir pagamento ou integrar banco. **A294:** a projeção sinaliza somente totais por preparação para revisão humana e deixa explícito que ausência de conciliação não confirma atraso ou pagamento.
- [x] Criar e ativar alertas periódicos internos, idempotentes, observáveis, desativáveis e restritos a operadores autorizados. **A297/A301/A303:** após checkpoint e publicação, a antecedência de quatro dias foi persistida, o cron diário interno foi ativado e sua existência foi confirmada somente por métrica agregada. A rotina permanece sem mensagem externa, emissão, banco, baixa ou pagamento; pausa, retomada e remoção continuam governadas no Financeiro.
- [x] Sincronizar o estado do lote apenas na confirmação material aprovada, com bloqueio concorrente, evento auditado e reversão governada; validar contratos, agenda, alertas, estoque e acessibilidade antes de qualquer emissão bancária futura. **A295/A296:** a confirmação aprovada atualiza um estado comercial separado para “Vendido”; o mapa de Loteamentos mostra essa camada sem alterar matriz física ou inventário. A solicitação de reversão muda apenas para revisão humana, sem hard delete ou retorno automático à disponibilidade.

### Marco M4 — aprovação material e estado comercial do lote

- [x] Adicionar um estado comercial separado e auditável ao lote para distinguir estrutura física, preparação de negociação, revisão contratual e lote vendido, preservando a matriz, as reservas físicas e os estados de inventário existentes. **A295:** adicionou uma camada comercial independente com os estados preparação, revisão contratual, vendido e revisão de reversão.
- [x] Criar uma confirmação material explícita, idempotente e segregada para aprovar o contrato interno, marcar o lote como vendido e bloquear concorrência; não emitir boleto, cobrar, baixar, registrar pagamento ou integrar banco. **A295:** a RPC exige sessão AAL2 vigente, autoridade contextual, contrato em revisão, bloqueio transacional por lote e auditoria redigida antes de promover o caso e a situação comercial; nenhum boleto ou pagamento é criado.
- [x] Permitir reversão governada por novo comando auditado, sem hard delete e sem voltar o lote a disponível automaticamente até a revisão humana; validar concorrência, permissão, transição e apresentação no estoque. **A295/A296:** a reversão muda o estado comercial para revisão, mantém o caso aprovado bloqueando nova concorrência e é projetada no mapa somente para leitura. A cobertura integral aprovou 267 arquivos e 732 testes, tipagem, build Netlify, integridade e revisões visuais; ZIP/HTML foram saneados.
...
- [x] Publicar o controle contextual de ativação, pausa, retomada e remoção do lembrete diário por identificador seguro. **A301/M7:** o Financeiro da Loteadora concentra esse controle por organização, sem depender de venda selecionada; a interface preserva a ativação explícita e os controles posteriores de pausa, retomada e remoção.
...
- [x] Exibir e configurar no Financeiro a antecedência interna antes da ativação, usando leitura contextual persistida em vez de estado local ou erro genérico. **A303:** a antecedência de quatro dias foi persistida na sessão autorizada e confirmada pela interface antes da ativação.
- [x] Criar após publicação o agendamento diário autenticado da rotina interna de atenção, com identidade de cron, execução idempotente e possibilidade de pausa, retomada e remoção segura; não enviar mensagens, emitir cobranças, acessar banco, dar baixa ou registrar pagamentos. **A301:** a rotina foi ativada após a sessão publicada; o comando contextual exige autenticação e autoridade server-side, mas não exige MFA específico do Financeiro por comando.
- [x] Executar uma verificação controlada da rotina publicada e confirmar apenas métricas agregadas e ausência de efeitos externos antes de marcar a ativação como concluída. **Ativação operacional:** a interface passou a expor pausa e remoção, e a conciliação agregada confirmou um vínculo de lembrete interno ativo, sem consulta a clientes, parcelas ou identificadores.

### Marco M6 — dossiê privado e revisão de documentação comercial

- [x] Manter a ordem obrigatória do programa: cadastro/estoque, preparação de negociação, formalização e agenda internos, confirmação comercial do lote, dossiê privado e somente depois lembretes ao operador; não antecipar, fundir ou executar efeitos de etapa posterior. **M6:** o dossiê foi implementado depois da agenda e da confirmação comercial; os lembretes foram ativados somente na etapa posterior, após publicação e validação contextual.
- [x] Vincular, de forma opaca, intenções de documento privadas ao caso de venda e à preparação contratual, reutilizando o armazenamento governado e sem expor bytes, URL, chave, download público ou conteúdo em auditoria. **A298/A300:** cada intenção tem categoria interna e estado opaco; o envio privado aceita somente PDF, JPEG ou PNG até 2 MB, valida assinatura do arquivo, sessão, MFA vigente, contexto, finalidade e correlação. Nenhum conteúdo ou metadado sensível é devolvido à tela ou à auditoria.
- [x] Criar uma lista de conferência documental e cadastral exclusivamente interna, revisável e auditada, que nunca bloqueie por inferência automática nem substitua análise humana, jurídica ou registral. **A298:** a Central mostra apenas contagens de intenção/arquivo e estados de revisão; pendências usam códigos internos, sem inferir documento, situação civil, efeito jurídico ou registral.
- [x] Exigir confirmação humana do dossiê antes da aprovação material do contrato, com motivo de pendência e reversão governada, sem assinatura eletrônica, registro imobiliário ou alteração financeira automática. **A299:** a aprovação exige ao menos um arquivo privado registrado e revisão humana “pronta para aprovação”; pendência requer motivo interno. A validação integral aprovou 276 arquivos e 748 testes, tipagem, build Netlify e integridade do diff; ZIP/HTML foram saneados.

### Marco M7 — lote interno de parcelas no Financeiro da Loteadora

- [x] Mapear a agenda A293, a venda aprovada, a superfície Financeiro da Loteadora e as rotas compatíveis antes de criar qualquer novo lote interno. **M7:** a agenda do contrato aprovado é a única fonte de parcelas; a superfície Financeiro preserva a rota e passa a exibir somente projeções autorizadas de lotes internos.
- [x] Definir e documentar o lote interno de boletos como agrupamento operacional de parcelas aprovadas, sem código de barras, registro bancário, emissão, remessa, mensagem, baixa ou pagamento. **M7:** o documento `docs/financeiro-loteadora-lote-interno.md` fixa o modelo de controle interno, seus limites e sua transição reversível para revisão humana.
- [x] Criar a liberação explícita, contextual, idempotente e auditada do lote interno somente para casos de venda aprovados com dossiê revisado e agenda existente. **A302:** a aprovação comercial passa a criar, na mesma transação, um único lote interno por contrato e seus itens derivados da agenda; a reversão move o lote para revisão humana sem apagar, reemitir ou liberar automaticamente.
- [x] Exibir o lote liberado na Central de Vendas e no Financeiro da Loteadora, preservando a agenda e distinguindo prazo de confirmação de pagamento. **M7:** a Central informa a liberação e oferece recuperação idempotente para aprovações históricas; o Financeiro foi liberado apenas para lotes internos autorizados, totais e contagens, mantendo explícita a ausência de emissão, mensagem, banco, baixa e pagamento.
- [x] Validar autorização, concorrência, idempotência, rollback, desktop, mobile, suíte integral, build, artefatos saneados e checkpoint restaurável. **M7:** A302 aplicada; 281 arquivos de teste e 758 testes aprovados, além de tipagem, build Netlify, integridade do diff, revisão desktop/móvel e ZIP/HTML saneados. O checkpoint restaurável será salvo com este registro factual.

### Auditoria E2E — venda conjunta, cálculos e estoques da Loteadora

- [x] Mapear a implementação e as coberturas de múltiplos proponentes, lotes, contratos, agenda, lote interno, alertas e todos os estoques envolvidos. **Auditoria E2E:** o fluxo reúne caso de venda, partes, termos, contrato interno, agenda, lote comercial, lote interno e configuração de atenção, com leituras contextuais protegidas.
- [x] Confirmar que a venda conjunta vincula mais de um cliente de forma contextual e auditada, sem duplicar pessoas nem expor documentos privados. **A304:** criou vínculo de partes do caso, com exatamente um proponente principal, proponentes conjuntos idempotentes, remoção restrita ao estado de preparação e invalidação de revisão documental quando a composição muda.
- [x] Validar cálculos de entrada, saldo, quantidade de parcelas, valor por parcela, vencimento inicial e recorrência, com rejeição de somas inconsistentes e arredondamento explícito. **A304:** o servidor exige que entrada mais parcelas componham exatamente o total; a interface calcula parcela igual quando o saldo divide exatamente, bloqueia divisão residual e mostra a prévia mensal com ajuste ao último dia de meses menores.
- [x] Validar que a venda aprovada cria contrato interno, atualiza o estado comercial do lote, libera lote interno de parcelas e mantém estoques de clientes, lotes, contratos e parcelas sincronizados. **A302/A304:** testes cobrem a transação e a conciliação agregada confirmou zero inconsistências atuais. Não há caso de venda, contrato, agenda ou lote interno materializado no ambiente nesta data, portanto nenhum registro individual foi criado para teste manual.
- [x] Validar que alertas internos usam a data e a antecedência configuradas, permanecendo apenas como lembrete de cobrança manual ao operador. **A297/A301/A303/A304:** a rotina habilitada usa a antecedência persistida e as datas de agenda; não emite cobrança, não envia mensagem, não acessa banco, não dá baixa e não registra pagamento.
- [x] Corrigir quaisquer lacunas encontradas na origem, ampliar testes e concluir com validação integrada, revisão visual, artefatos saneados e checkpoint restaurável. **Auditoria E2E:** A304 foi aplicada; 284 arquivos e 768 testes aprovados, além de tipagem, build Netlify, integridade, revisão desktop/móvel e conciliação agregada. ZIP/HTML serão regenerados e o checkpoint salvo com este registro.

### Modalidades graduais de negociação e composição interna

- [x] Mapear os termos, contrato, agenda, lote interno, alerta e estoques atuais antes de estender a composição negociada. **A305:** a base A304 foi revisada antes da mudança; contrato, agenda e lote interno continuam derivados do mesmo caso de venda protegido.
- [x] Modelar componentes internos de negociação para entrada à vista, entrada parcelada, parcelas simultâneas, venda à vista, complemento de valor e bem entregue, sem criar emissão, mensagem, banco, baixa ou pagamento automático. **A305:** a preparação agora registra modalidade, entrada única, entrada parcelada, série regular, valor à vista, complemento e crédito declarado de bem com campos próprios e datas.
- [x] Exigir que todos os componentes monetários componham exatamente o valor total negociado, preservando arredondamento explícito, datas declaradas e calendário reconciliado. **A305:** contrato compartilhado, interface e RPC impõem composição exata, modo compatível, datas obrigatórias, ajuste de meses menores e limite global de 480 itens mensais.
- [x] Registrar bens entregues somente como controle interno categorizado e declarado, sem avaliação automática, transferência de propriedade, registro, financeiro externo ou pagamento. **A305:** o bem exige crédito, data, categoria e descrição declarados; a interface e a migração explicitam que não há avaliação, transferência ou confirmação de recebimento.
- [x] Integrar as modalidades aprovadas ao contrato interno, agenda, lote interno, lembretes e estoques de contratos, lotes, clientes e parcelas. **A305:** a formalização cria itens por componente, inclusive calendários simultâneos; a aprovação e a liberação idempotente do lote interno copiam tipo e sequência determinística. Estoques continuam alterados somente pela aprovação comercial humana.
- [x] Validar casos à vista, entrada parcelada, cronogramas simultâneos, complemento de valor e bem entregue em testes, interface desktop/móvel, conciliação agregada, artefatos saneados e checkpoint restaurável. **A305:** a matriz de contratos cobre venda à vista, entrada parcelada simultânea a parcelas regulares, total divergente, modo incompatível e bem sem metadados; a suíte integral aprovou 285 arquivos e 772 testes. Tipagem, build Netlify e integridade do diff foram aprovados; a revisão visual em desktop e celular manteve a jornada contextual; a conciliação agregada confirmou zero contratos, agendas, lotes internos, órfãos ou cronogramas acima do limite. ZIP/HTML saneados foram inspecionados e o checkpoint restaurável A305 foi salvo.

### Sócios e Parceiros — participações internas por loteamento, lote, venda e parcela

- [x] Mapear os vínculos atuais de sócio, parceiro e terrenista, regras econômicas existentes, Central de Vendas, agenda, lote interno e Financeiro antes de ampliar o domínio. **A306/A307:** vínculos temporais e regras econômicas anteriores foram preservados; os novos registros iniciam sem alterar estoque, contrato, agenda, lote interno ou venda preexistente.
- [x] Definir participação versionada por loteamento com vigência, papel, base de cálculo, escopo de lote e componentes elegíveis, sem conta bancária, pagamento, repasse ou split externo. **A306:** versões têm vigência, rascunho, ativação, histórico substituído, papel interno já vinculado, componentes da negociação e RLS fail-closed.
- [x] Permitir atribuição detalhada de partes já cadastradas por busca contextual, incluindo regra percentual, valor fixo, valor por lote ou por componente, limites e abrangência total ou seletiva de lotes. **A306:** a busca fiscal é correlacionada e limitada ao loteamento; as regras suportam percentual por componente, valor fixo por componente e teto por lote, com escopo total ou lote a lote.
- [x] Congelar um snapshot auditável da regra vigente na confirmação comercial da venda, sem permitir que mudanças futuras alterem vendas, contratos, agendas ou projeções já confirmadas. **A307:** a aprovação materializa uma fotografia idempotente antes de liberar o lote interno; reversão somente a move para revisão humana e não apaga nenhum histórico.
- [x] Gerar somente projeções internas de participação por componente da agenda e consolidá-las no Financeiro, sem emitir ordem, remessa, cobrança, pagamento, baixa, transferência ou conciliação bancária. **A307:** as projeções respeitam limite por item, teto, integralidade opcional e sobrealocação; o Financeiro mostra somente totais, quantidades, saldo sem participação e estado da projeção.
- [x] Exibir alertas internos para ausência de regra, vigência incompatível, sobreposição, composição inválida e alteração posterior, preservando revisão humana e histórico sem exclusão física. **A306/A307:** ausência de política ativa é registrada como estado explícito na venda aprovada; sobreposição e integralidade inválida bloqueiam a materialização, e mudanças posteriores exigem nova versão sem reescrever snapshots.
- [x] Validar autorização, cálculo, imutabilidade, idempotência, sincronização agregada, interface desktop/móvel, artefatos saneados e checkpoint restaurável. **A306/A307:** a matriz cobre identidade ausente, busca fiscal contextual, versão em rascunho, incompatibilidade de método, snapshot, integralidade, sobrealocação e resumo agregado do Financeiro. As migrações foram aplicadas; a conciliação agregada confirmou zero versões, regras, snapshots, projeções ou órfãos materiais existentes. A revisão desktop/móvel confirmou a jornada de contexto sem ações materiais. A suíte integral aprovou 289 arquivos e 781 testes; tipagem, build Netlify e integridade do diff foram aprovados, com aviso não bloqueante de tamanho de chunk. ZIP/HTML foram saneados e o checkpoint restaurável A306/A307 foi salvo.

### Habilidade reutilizável — participações internas de Sócios e Parceiros

- [x] Consolidar as etapas, limites, precondições e critérios de validação da atualização A306/A307 em uma habilidade reutilizável. **Habilidade `subdivision-internal-participation-governance`:** reúne modelagem versionada, escopo, regras percentuais/fixas/com teto, snapshots de aprovação, projeções internas por agenda, reversão humana, controles de acesso e matriz de validação.
- [x] Criar e validar a habilidade sem incluir dados privados, credenciais, identificadores de ambiente ou instruções de execução financeira externa. **Validação concluída:** a estrutura foi gerada pelo fluxo oficial, exemplos de inicialização não utilizados foram removidos e a validação rápida foi aprovada. A habilidade delimita explicitamente que não há conta, chave, pagamento, split, transferência, remessa, cobrança, baixa ou confirmação de recebimento.

### Acesso individual — login Google para colaboradores BHL Imóveis

- [x] Mapear o login atual, convites, identidade, sessões, revogação e permissões antes de alterar o provedor de autenticação. **Mapeamento:** o aplicativo preserva autenticação de plataforma separada da sessão Supabase de contexto; esta última é a identidade resolvida pelo servidor antes de consultar memberships, grants, módulo, finalidade, escopo e vigência.
- [x] Definir o fluxo de acesso individual por Google, com cadastro prévio ou convite, associação segura de identidade e bloqueio de conta não autorizada. **Decisão:** Google autentica somente a sessão Supabase de contexto e não concede alçada por conta própria; o acesso continua exigindo vínculo interno prévio e válido. A arquitetura, configuração externa e critérios de aceite foram documentados sem credenciais.
- [x] Configurar o provedor e implementar a jornada visual de login Google, preservando contexto organizacional, papéis e auditoria. **Implementação:** a sessão de contexto agora prioriza “Continuar com Google”, usa a origem atual e destino interno validado no retorno OAuth, mantém a contingência por senha recolhida e não altera alçadas. **Configuração externa:** cliente OAuth Web, origem autorizada, callback do Supabase, provedor habilitado e público de teste foram configurados fora do repositório; nenhum ID ou segredo foi registrado no CRM.
- [ ] Validar login, conta não provisionada, suspensão, revogação, logout, sessão e isolamento entre organizações, além de desktop, mobile, testes, build e checkpoint.

### Correção de acesso individual — eliminar bloqueio prévio da plataforma

- [x] Mapear todas as dependências atuais de `ctx.user` e da sessão de plataforma que bloqueiam o login Google antes da validação do contexto. **Diagnóstico:** as rotas operacionais já usam o sujeito Supabase nas consultas de contexto, mas `protectedProcedure`, `auth.me` e o layout global ainda exigem a sessão da plataforma. Bootstrap e administração privilegiada mantêm guardas próprios.
- [x] Substituir, de forma compatível, a identidade da plataforma por uma identidade mínima derivada exclusivamente da sessão Supabase/Google válida. **A308:** após validar o token Supabase no servidor, a sessão cria idempotentemente apenas o sujeito canônico de identidade, sem conceder organização, papel ou alçada.
...
### Sessão Google — dispensa de MFA repetido por comando

- [x] Mapear todas as rotas operacionais que ainda exigem atestação MFA adicional apesar de uma sessão Google autenticada e válida. **Mapeamento:** Loteamentos, preços, Clientes Loteadora, Sócios e Parceiros, Central de Vendas, documentos privados e importação ainda possuíam guardas MFA por comando; comandos de bootstrap e administração de plataforma já eram separados.
- [x] Remover o desafio MFA repetido das rotas operacionais, preservando identidade, sessão, contexto, membership, grant, papel, módulo, finalidade, escopo, vigência, correlação, idempotência e auditoria. **Aplicação:** as rotas operacionais agora dependem apenas da sessão Supabase/Google válida e das validações contextuais já existentes; anexos privados não exigem mais usuário de plataforma ou MFA repetido antes de sua validação opaca.
- [x] Manter controles de bootstrap e privilégios exclusivos de plataforma separados da jornada normal de colaborador Google. **Preservado:** os comandos de bootstrap e alçadas exclusivas continuam guardados por `bootstrapOwnerProcedure` ou `platformActiveProcedure`; a sessão Google não os amplia.
- [ ] Validar que a sessão Google autenticada executa somente comandos já autorizados, enquanto ausência, expiração, logout, suspensão, revogação ou falta de alçada continuam bloqueando o servidor.
