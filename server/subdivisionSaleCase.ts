...
export async function saveSubdivisionSaleCaseTerms(subjectId: string | undefined, rawInput: SaveSubdivisionSaleCaseTermsInput, client: RpcClient = getSupabaseAdminClient()): Promise<{ saleCaseId: string; termsVersion: number; state: "preparation" }> {
  ...
  const { data, error } = await client.rpc("subdivision_save_sale_case_terms", { p_actor_user_id: actorUserId, p_organization_id: input.organizationId, p_module: input.module, p_purpose_code: input.purposeCode, p_sale_case_id: input.saleCaseId, p_negotiated_total_cents: input.negotiatedTotalCents, p_entry_amount_cents: input.entryAmountCents, p_entry_due_date: input.entryDueDate, p_entry_installment_count: input.entryInstallmentCount, p_entry_installment_amount_cents: input.entryInstallmentAmountCents, p_entry_first_due_date: input.entryFirstDueDate, p_entry_due_day: input.entryDueDay, p_installment_count: input.installmentCount, p_installment_amount_cents: input.installmentAmountCents, p_first_due_date: input.firstDueDate, p_due_day: input.dueDay, p_settlement_mode: input.settlementMode, p_cash_settlement_amount_cents: input.cashSettlementAmountCents, p_cash_settlement_due_date: input.cashSettlementDueDate, p_supplemental_amount_cents: input.supplementalAmountCents, p_supplemental_due_date: input.supplementalDueDate, p_trade_in_credit_cents: input.tradeInCreditCents, p_trade_in_due_date: input.tradeInDueDate, p_trade_in_category: input.tradeInCategory, p_trade_in_description: input.tradeInDescription, p_correlation_id: input.correlationId });
...
export async function listSubdivisionSaleCases(subjectId: string | undefined, rawContext: unknown, client: RpcClient = getSupabaseAdminClient()): Promise<SubdivisionSaleCaseSummary[]> {
  ...
  return data.map((row) => {
    ...
    const settlementMode = row.settlement_mode === "cash" ? "cash" : "structured";
    return { saleCaseId: row.sale_case_id, lotId: row.lot_id, buyerClientId: row.buyer_client_id, state: state as SaleCaseState, termsVersion: numericOrNull(row.terms_version), negotiatedTotalCents: numericOrNull(row.negotiated_total_cents), entryAmountCents: numericOrNull(row.entry_amount_cents), entryDueDate: row.entry_due_date == null ? null : String(row.entry_due_date), entryInstallmentCount: Number(row.entry_installment_count ?? 0), entryInstallmentAmountCents: numericOrNull(row.entry_installment_amount_cents), entryFirstDueDate: row.entry_first_due_date == null ? null : String(row.entry_first_due_date), entryDueDay: numericOrNull(row.entry_due_day), installmentCount: Number(row.installment_count), installmentAmountCents: numericOrNull(row.installment_amount_cents), firstDueDate: row.first_due_date === null ? null : String(row.first_due_date), dueDay: numericOrNull(row.due_day), settlementMode, cashSettlementAmountCents: numericOrNull(row.cash_settlement_amount_cents), cashSettlementDueDate: row.cash_settlement_due_date == null ? null : String(row.cash_settlement_due_date), supplementalAmountCents: numericOrNull(row.supplemental_amount_cents), supplementalDueDate: row.supplemental_due_date == null ? null : String(row.supplemental_due_date), tradeInCreditCents: numericOrNull(row.trade_in_credit_cents), tradeInDueDate: row.trade_in_due_date == null ? null : String(row.trade_in_due_date), tradeInCategory: row.trade_in_category == null ? null : String(row.trade_in_category), tradeInDescription: row.trade_in_description == null ? null : String(row.trade_in_description), createdAt: String(row.created_at), updatedAt: String(row.updated_at) };
  });
}
