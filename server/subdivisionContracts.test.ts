...
import { draftSubdivisionBlockInputSchema, draftSubdivisionDevelopmentInputSchema, draftSubdivisionLotInputSchema, subdivisionContextSchema } from "../shared/subdivisionContracts";
...
describe("subdivision foundation contracts", () => {
  ...
  it("limits an explicit draft lot to one of one hundred positions in its block", () => {
    expect(draftSubdivisionLotInputSchema.safeParse({ organizationId, module: "loteadora", purposeCode: "CADASTRO_INICIAL", correlationId, blockId, lotNumber: 100 }).success).toBe(true);
    expect(draftSubdivisionLotInputSchema.safeParse({ organizationId, module: "loteadora", purposeCode: "CADASTRO_INICIAL", correlationId, blockId, lotNumber: 101 }).success).toBe(false);
  });
});
