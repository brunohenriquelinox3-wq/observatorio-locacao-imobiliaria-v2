...
| --- | --- | --- | --- |
| **Painel Loteadora** *(primeira tela interna)* | Alertas e poucos indicadores executivos de elegibilidade, reserva, contrato, dossiê, carteira, direitos, repasses e restrições. | Abre o setor/caso fonte com o mesmo recorte/policy; não edita fatos. | **Decisão aprovada:** é a primeira tela após entrar na coluna Loteadora, sem alterar a ordem global SUPER ADM → ADM → LOTEADORA → VENDAS URBANAS → LOCAÇÃO. |
...
Ao selecionar o cliente pelo CPF/CNPJ protegido na venda de um lote, o CRM usa a ficha existente e mostra os dados/documentos vinculados, com origem, versão, validade e estado. Documentos anexados na ficha ou na venda entram no dossiê canônico da pessoa/empresa e são associados à checklist daquela negociação; o contrato registra quais versões foram usadas no seu snapshot. Portanto, reutilização economiza trabalho, mas não oculta pendências nem declara aprovação automática.

A lente de **Contratos Realizados** fica dentro de Vendas e Contratos, ligada ao lote `Qn · Ln`, e alerta dossiê incompleto, assinatura/versão pendente, agenda/carteira, divergência entre lote e contrato, cessão ou distrato em análise. Ela não é estoque duplicado: o Estoque/Mapa continua autoridade do lote e suas restrições.
