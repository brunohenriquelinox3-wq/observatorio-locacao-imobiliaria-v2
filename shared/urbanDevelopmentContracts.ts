...
export const urbanDevelopmentStructureKindSchema = z.enum(["tower", "block"]);
...
export const draftUrbanDevelopmentStructureInputSchema = urbanSalesContextSchema.extend({
  correlationId: z.string().uuid(),
  developmentId: z.string().uuid(),
  internalReference: z.string().trim().toUpperCase().regex(/^[A-Z][A-Z0-9_]{2,79}$/),
  structureKind: urbanDevelopmentStructureKindSchema,
});

export type DraftUrbanDevelopmentStructureInput = z.infer<typeof draftUrbanDevelopmentStructureInputSchema>;
