...
## Versionamento e ambientes de teste

O GitHub é a fonte externa de versionamento e revisão do projeto. O repositório deve permanecer privado, sem segredos, tokens, credenciais, dados reais ou arquivos de ambiente versionados. O Netlify é o ambiente padrão de teste: cada execução deve instalar dependências com lockfile, executar tipagem, testes e build Netlify antes de publicar somente um deploy de rascunho/preview; produção nunca deve ser publicada automaticamente por esta rotina. Devem existir pelo menos duas execuções agendadas por dia no GitHub Actions, além das execuções por push e pull request, sempre bloqueando o deploy quando a validação falhar. Render permanece opcional e desativado até que o Netlify não atenda a uma necessidade concreta do backend; nenhum deploy em Render deve ser criado por suposição. A configuração dos agendamentos deve usar horários fora do início exato da hora, registrar o resultado e manter os segredos apenas nos cofres do GitHub/Netlify.
