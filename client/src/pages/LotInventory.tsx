...
import LotStructureMatrix from "@/components/LotStructureMatrix";
...
export default function LotInventory() {
  ...
  const inventoryStates = trpc.subdivisionFoundation.listDraftLotInventoryStates.useQuery(context, { enabled: contextReady, retry: false });
  const commercialStates = trpc.subdivisionFoundation.listLotCommercialStates.useQuery(context, { enabled: contextReady, retry: false });
  const events = trpc.subdivisionFoundation.listDraftLotInventoryEvents.useQuery(context, { enabled: contextReady, retry: false });
  const selectedBlock = useMemo(() => blocks.data?.find((block) => block.blockId === blockId) ?? null, [blockId, blocks.data]);
  useEffect(() => {
  ...
  const create = trpc.subdivisionFoundation.createDraftLot.useMutation({ onSuccess() { setLotNumber(1); void utils.subdivisionFoundation.listDraftLots.invalidate(lotsInput); toast.success("Lote em preparação registrado"); } });
  const inventory = trpc.subdivisionFoundation.upsertDraftLotInventoryState.useMutation({ onSuccess() { void utils.subdivisionFoundation.listDraftLotInventoryStates.invalidate(context); void events.refetch(); toast.success("Situação interna registrada"); } });
