...
const economicRuleSetWorkingStates = {
  draft_internal: "Rascunho interno",
  review_required: "Revisão necessária",
} as const;

  const economicRuleComponentCodes = { entry_reference: "Referência de entrada", installment_reference: "Referência de parcela", intermediate_reference: "Referência intermediária", final_result_reference: "Referência de resultado final" } as const;
...
export default function SubdivisionFoundation() {
  ...
  const [economicRuleSetDevelopmentId, setEconomicRuleSetDevelopmentId] = useState("");
  const [economicRuleSetVersionReference, setEconomicRuleSetVersionReference] = useState("");
  const [economicRuleSetWorkingState, setEconomicRuleSetWorkingState] = useState<keyof typeof economicRuleSetWorkingStates>("draft_internal");
  const [economicRuleComponentRuleSetId, setEconomicRuleComponentRuleSetId] = useState("");
  const [economicRuleComponentCode, setEconomicRuleComponentCode] = useState<keyof typeof economicRuleComponentCodes>("entry_reference");
  const [economicRuleComponentWorkingState, setEconomicRuleComponentWorkingState] = useState<keyof typeof economicRuleSetWorkingStates>("draft_internal");
  const [economicRuleComponentIdForRoleReference, setEconomicRuleComponentIdForRoleReference] = useState("");
  ...
  const economicRuleSetsQuery = trpc.subdivisionFoundation.listEconomicRuleSets.useQuery(context, { enabled: isWorkspaceReady, retry: false });
  const economicRuleComponentsQuery = trpc.subdivisionFoundation.listEconomicRuleComponents.useQuery(context, { enabled: isWorkspaceReady, retry: false });
  const economicRuleComponentRoleReferencesQuery = trpc.subdivisionFoundation.listEconomicRuleComponentRoleReferences.useQuery(context, { enabled: isWorkspaceReady, retry: false });
  ...
  const operationalSectors = [
    ...
    { code: "03", title: "Sócios e Parceiros", value: subdivisionOperationalValue({ contextReady: isWorkspaceReady, loading: internalPartyRolesQuery.isLoading, count: internalPartyRolesQuery.data?.length, pendingLabel: "Aguardando leitura" }), description: "Papéis internos temporais não definem participação, valor ou recebimento.", target: "#subdivision-parties" },
    { code: "04", title: "Clientes Loteadora", value: subdivisionOperationalValue({ contextReady: isWorkspaceReady, loading: buyerClientsQuery.isLoading, count: buyerClientsQuery.data?.length, pendingLabel: "Aguardando leitura" }), description: "A jornada do comprador permanece separada de venda, contrato e documentos.", target: "#subdivision-buyers" },
    { code: "05", title: "Rascunho de Venda", value: subdivisionOperationalValue({ contextReady: isWorkspaceReady, loading: saleDraftsQuery.isLoading, count: saleDraftsQuery.data?.length, pendingLabel: "Aguardando leitura" }), description: "Organiza vínculos internos; não cria reserva, contrato ou financeiro.", target: "#subdivision-sales" },
    { code: "06", title: "Regras Internas", value: subdivisionOperationalValue({ contextReady: isWorkspaceReady, loading: economicRuleSetsQuery.isLoading, count: economicRuleSetsQuery.data?.length, pendingLabel: "Aguardando leitura" }), description: "Referências versionadas sem valores, percentuais, cálculo ou pagamento.", target: "#subdivision-economic" },
  ];
  ...
  const createEconomicRuleSetMutation = trpc.subdivisionFoundation.createEconomicRuleSet.useMutation({ onSuccess() { setEconomicRuleSetVersionReference(""); toast.success("Regra interna versionada", { description: "O registro não aceita valores, percentuais, parcelas, cobrança, pagamento, repasse ou integração externa." }); void utils.subdivisionFoundation.listEconomicRuleSets.invalidate(context); }, onError() { toast.error("Regra interna não registrada", { description: "O servidor exige loteamento no mesmo contexto autorizado e uma referência de versão válida." }); } });
  const createEconomicRuleComponentMutation = trpc.subdivisionFoundation.createEconomicRuleComponent.useMutation({ onSuccess() { setEconomicRuleComponentRuleSetId(""); toast.success("Componente interno registrado", { description: "O registro não aceita beneficiário, valores, percentuais, cálculo, cobrança, pagamento, repasse ou integração externa." }); void utils.subdivisionFoundation.listEconomicRuleComponents.invalidate(context); }, onError() { toast.error("Componente não registrado", { description: "O servidor exige uma regra interna do mesmo contexto autorizado." }); } });
  const addEconomicRuleComponentRoleReferenceMutation = trpc.subdivisionFoundation.addEconomicRuleComponentRoleReference.useMutation({ onSuccess() { setEconomicRuleComponentIdForRoleReference(""); setInternalPartyRoleLinkIdForReference(""); toast.success("Referência interna vinculada", { description: "O vínculo não cria direito econômico, beneficiário financeiro, valor, percentual, cobrança, pagamento ou repasse." }); void utils.subdivisionFoundation.listEconomicRuleComponentRoleReferences.invalidate(context); }, onError() { toast.error("Referência não vinculada", { description: "O servidor exige componente e papel interno do mesmo loteamento e contexto autorizado." }); } });
  ...
  return (
    <DashboardLayout navigationItems={navigationItems} navigationTitle="Núcleo CRM" accessGate={subdivisionAccessGate}>
      <main className="subdivision-foundation-page">
        ...
        <section className="subdivision-foundation-overview" aria-labelledby="subdivision-overview-title">
          <div className="subdivision-foundation-overview__heading"><div><p className="subdivision-foundation-eyebrow">PAINEL LOTEADORA · VISÃO DE TRABALHO</p><h2 id="subdivision-overview-title">Setores separados para uma operação que cresce com controle.</h2></div><p>Os indicadores mostram somente leituras do contexto autorizado. Estados vazios não são registros e não revelam informações de outro loteamento.</p></div>
          <nav className="subdivision-foundation-overview__grid" aria-label="Setores operacionais da Loteadora">
            {operationalSectors.map((sector) => <a key={sector.code} href={sector.target}><span>{sector.code}</span><strong>{sector.title}</strong><b>{sector.value}</b><small>{sector.description}</small><em>Ver setor</em></a>)}
          </nav>
        </section>
        ...
        <section id="subdivision-parties" className="subdivision-foundation-workspace" aria-labelledby="subdivision-role-title">
        ...
        <section id="subdivision-sales" className="subdivision-foundation-workspace" aria-labelledby="subdivision-sale-draft-title">
          ...
          {isWorkspaceReady && saleDraftsQuery.data && saleDraftsQuery.data.length > 0 && <form className="subdivision-foundation-card" onSubmit={(event) => { event.preventDefault(); addSaleDraftCoBuyerMutation.mutate({ ...context, correlationId: crypto.randomUUID(), saleDraftId: saleDraftIdForCoBuyer, buyerClientId: coBuyerClientId }); }}><div className="subdivision-foundation-card__title"><UsersRound size={19} /><h3>Vincular co-comprador interno</h3></div><p>Use somente para organizar participantes internos do rascunho. Não define proponente contratual, titularidade, percentual, preço, reserva ou obrigação financeira.</p><label htmlFor="subdivision-sale-draft-co-buyer-item">Rascunho interno de venda<select id="subdivision-sale-draft-co-buyer-item" value={saleDraftIdForCoBuyer} onChange={(event) => setSaleDraftIdForCoBuyer(event.target.value)} disabled={addSaleDraftCoBuyerMutation.isPending} required><option value="">Selecione um rascunho autorizado</option>{saleDraftsQuery.data.map((draft) => <option key={draft.saleDraftId} value={draft.saleDraftId}>Rascunho · {draft.saleDraftId.slice(0, 8)}</option>)}</select></label><label htmlFor="subdivision-sale-draft-co-buyer-client">Cliente comprador em rascunho<select id="subdivision-sale-draft-co-buyer-client" value={coBuyerClientId} onChange={(event) => setCoBuyerClientId(event.target.value)} disabled={addSaleDraftCoBuyerMutation.isPending} required><option value="">Selecione um cliente autorizado</option>{buyerClientsQuery.data?.map((client) => <option key={client.buyerClientId} value={client.buyerClientId}>Vínculo de cliente · {client.buyerClientId.slice(0, 8)}</option>)}</select></label><button type="submit" disabled={!saleDraftIdForCoBuyer || !coBuyerClientId || addSaleDraftCoBuyerMutation.isPending}>{addSaleDraftCoBuyerMutation.isPending ? "Vinculando co-comprador" : "Vincular co-comprador interno"}</button></form>}
        </section>
        <section id="subdivision-economic" className="subdivision-foundation-workspace" aria-labelledby="subdivision-economic-rule-title">
          <div className="subdivision-foundation-heading"><div><p className="subdivision-foundation-eyebrow">09 · REGRAS INTERNAS</p><h2 id="subdivision-economic-rule-title">Prepare versões de regra sem armazenar qualquer dado financeiro.</h2></div><p>Esta fundação organiza o loteamento e a referência de versão para revisão humana posterior. Valores, percentuais, parcelas, pagamentos, cobranças, repasses e integrações externas seguem bloqueados.</p></div>
          <form className="subdivision-foundation-card" onSubmit={(event) => { event.preventDefault(); createEconomicRuleSetMutation.mutate({ ...context, correlationId: crypto.randomUUID(), developmentId: economicRuleSetDevelopmentId, versionReference: economicRuleSetVersionReference, workingState: economicRuleSetWorkingState }); }}><label htmlFor="subdivision-economic-rule-development">Loteamento em rascunho<select id="subdivision-economic-rule-development" value={economicRuleSetDevelopmentId} onChange={(event) => setEconomicRuleSetDevelopmentId(event.target.value)} disabled={!isWorkspaceReady} required><option value="">Selecione um loteamento autorizado</option>{developmentsQuery.data?.map((development) => <option key={development.developmentId} value={development.developmentId}>{development.internalReference}</option>)}</select></label><label htmlFor="subdivision-economic-rule-version">Referência interna da versão<input id="subdivision-economic-rule-version" value={economicRuleSetVersionReference} onChange={(event) => setEconomicRuleSetVersionReference(event.target.value.toUpperCase())} placeholder="REGRAS_001" maxLength={80} disabled={!isWorkspaceReady} required /></label><label htmlFor="subdivision-economic-rule-state">Estado de trabalho<select id="subdivision-economic-rule-state" value={economicRuleSetWorkingState} onChange={(event) => setEconomicRuleSetWorkingState(event.target.value as keyof typeof economicRuleSetWorkingStates)} disabled={!isWorkspaceReady}>{Object.entries(economicRuleSetWorkingStates).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><button type="submit" disabled={!isWorkspaceReady || !economicRuleSetDevelopmentId || !economicRuleSetVersionReference || createEconomicRuleSetMutation.isPending}>{createEconomicRuleSetMutation.isPending ? "Registrando regra" : "Registrar regra interna"}</button></form>
          {isWorkspaceReady && economicRuleSetsQuery.data?.length === 0 && <div className="subdivision-foundation-empty"><Workflow size={18} /><p>Nenhuma regra interna foi devolvida para este contexto.</p></div>}
          {isWorkspaceReady && economicRuleSetsQuery.data && economicRuleSetsQuery.data.length > 0 && <div className="subdivision-foundation-list__rows">{economicRuleSetsQuery.data.map((ruleSet) => { const components = economicRuleComponentsQuery.data?.filter((component) => component.economicRuleSetId === ruleSet.economicRuleSetId) ?? []; const roleReferenceCount = components.reduce((total, component) => total + (economicRuleComponentRoleReferencesQuery.data?.filter((reference) => reference.economicRuleComponentId === component.economicRuleComponentId).length ?? 0), 0); return <article key={ruleSet.economicRuleSetId}><span>Regra econômica interna</span><h3>{ruleSet.versionReference}</h3><p><b>{economicRuleSetWorkingStates[ruleSet.workingState]}</b> · referência de trabalho sem valores, percentuais, parcelas, pagamentos, cobrança, repasses ou integração externa.</p><p><b>{components.length} componente(s) interno(s)</b> · {components.length ? components.map((component) => economicRuleComponentCodes[component.componentCode]).join(" · ") : "nenhum componente registrado"}.</p><p><b>{roleReferenceCount} referência(s) de papel interno</b> · não cria direito econômico, beneficiário financeiro, valor, percentual, cálculo, cobrança, pagamento ou repasse.</p><code>{ruleSet.economicRuleSetId}</code></article>; })}</div>}
          {isWorkspaceReady && economicRuleSetsQuery.data && economicRuleSetsQuery.data.length > 0 && <form className="subdivision-foundation-card" onSubmit={(event) => { event.preventDefault(); createEconomicRuleComponentMutation.mutate({ ...context, correlationId: crypto.randomUUID(), economicRuleSetId: economicRuleComponentRuleSetId, componentCode: economicRuleComponentCode, workingState: economicRuleComponentWorkingState }); }}><div className="subdivision-foundation-card__title"><Workflow size={19} /><h3>Adicionar componente de referência</h3></div><p>Este componente organiza a futura regra, mas não identifica beneficiário, não recebe valor ou percentual e não executa qualquer cálculo, cobrança, pagamento, repasse ou integração externa.</p><label htmlFor="subdivision-economic-component-rule-set">Regra interna versionada<select id="subdivision-economic-component-rule-set" value={economicRuleComponentRuleSetId} onChange={(event) => setEconomicRuleComponentRuleSetId(event.target.value)} disabled={createEconomicRuleComponentMutation.isPending} required><option value="">Selecione uma regra autorizada</option>{economicRuleSetsQuery.data.map((ruleSet) => <option key={ruleSet.economicRuleSetId} value={ruleSet.economicRuleSetId}>{ruleSet.versionReference}</option>)}</select></label><label htmlFor="subdivision-economic-component-code">Componente de referência<select id="subdivision-economic-component-code" value={economicRuleComponentCode} onChange={(event) => setEconomicRuleComponentCode(event.target.value as keyof typeof economicRuleComponentCodes)} disabled={createEconomicRuleComponentMutation.isPending}>{Object.entries(economicRuleComponentCodes).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label htmlFor="subdivision-economic-component-state">Estado de trabalho<select id="subdivision-economic-component-state" value={economicRuleComponentWorkingState} onChange={(event) => setEconomicRuleComponentWorkingState(event.target.value as keyof typeof economicRuleSetWorkingStates)} disabled={createEconomicRuleComponentMutation.isPending}>{Object.entries(economicRuleSetWorkingStates).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><button type="submit" disabled={!economicRuleComponentRuleSetId || createEconomicRuleComponentMutation.isPending}>{createEconomicRuleComponentMutation.isPending ? "Registrando componente" : "Registrar componente interno"}</button></form>}
          {isWorkspaceReady && economicRuleComponentsQuery.data && economicRuleComponentsQuery.data.length > 0 && <form className="subdivision-foundation-card" onSubmit={(event) => { event.preventDefault(); addEconomicRuleComponentRoleReferenceMutation.mutate({ ...context, correlationId: crypto.randomUUID(), economicRuleComponentId: economicRuleComponentIdForRoleReference, internalPartyRoleLinkId: internalPartyRoleLinkIdForReference }); }}><div className="subdivision-foundation-card__title"><UsersRound size={19} /><h3>Referenciar papel interno</h3></div><p>Este vínculo conecta somente referências internas do mesmo loteamento. Não cria direito econômico, beneficiário financeiro, titularidade, valor, percentual, cálculo, cobrança, pagamento ou repasse.</p><label htmlFor="subdivision-economic-role-reference-component">Componente interno<select id="subdivision-economic-role-reference-component" value={economicRuleComponentIdForRoleReference} onChange={(event) => setEconomicRuleComponentIdForRoleReference(event.target.value)} disabled={addEconomicRuleComponentRoleReferenceMutation.isPending} required><option value="">Selecione um componente autorizado</option>{economicRuleComponentsQuery.data.map((component) => <option key={component.economicRuleComponentId} value={component.economicRuleComponentId}>{economicRuleComponentCodes[component.componentCode]}</option>)}</select></label><label htmlFor="subdivision-economic-role-reference-role">Papel interno temporal<select id="subdivision-economic-role-reference-role" value={internalPartyRoleLinkIdForReference} onChange={(event) => setInternalPartyRoleLinkIdForReference(event.target.value)} disabled={addEconomicRuleComponentRoleReferenceMutation.isPending} required><option value="">Selecione um papel autorizado</option>{internalPartyRolesQuery.data?.map((role) => <option key={role.linkId} value={role.linkId}>{role.role}</option>)}</select></label><button type="submit" disabled={!economicRuleComponentIdForRoleReference || !internalPartyRoleLinkIdForReference || addEconomicRuleComponentRoleReferenceMutation.isPending}>{addEconomicRuleComponentRoleReferenceMutation.isPending ? "Vinculando referência" : "Vincular referência interna"}</button></form>}
        </section>
      </main>
    </DashboardLayout>
  );
