  ...
  return (
    <DashboardLayout navigationItems={crmNavigationItems} navigationTitle="Núcleo CRM" accessGate={subdivisionAccessGate}>
      <main className="subdivision-foundation-page">
		...
		{isCentralSalesJourney && <section id="subdivision-sales" className="subdivision-foundation-workspace" aria-labelledby="subdivision-sale-draft-title">
		  <div className="subdivision-foundation-heading"><div><p className="subdivision-foundation-eyebrow">08 · PREPARAÇÃO DE VENDA</p><h2 id="subdivision-sale-draft-title">Lote e cliente podem ser conectados com segurança antes da negociação.</h2></div><p>Esta preparação preserva o estoque: não é reserva, proposta, contrato, preço, cobrança, boleto, comissão, repasse ou financeiro.</p></div>
		  <form className="subdivision-foundation-card" onSubmit={(event) => { event.preventDefault(); createSaleDraftMutation.mutate({ ...context, correlationId: crypto.randomUUID(), lotId: saleLotId, buyerClientId: saleBuyerClientId }); }}><label htmlFor="subdivision-sale-development">Loteamento<select id="subdivision-sale-development" value={saleDevelopmentId} onChange={(event) => { setSaleDevelopmentId(event.target.value); setSaleBlockId(""); setSaleLotId(""); }} disabled={!isWorkspaceReady} required><option value="">Selecione um loteamento autorizado</option>{developmentsQuery.data?.map((development, index) => <option key={development.developmentId} value={development.developmentId}>{formatSubdivisionDevelopmentLabel(index)}</option>)}</select></label><label htmlFor="subdivision-sale-block">Quadra<select id="subdivision-sale-block" value={saleBlockId} onChange={(event) => { setSaleBlockId(event.target.value); setSaleLotId(""); }} disabled={!isWorkspaceReady || !saleDevelopmentId} required><option value="">Selecione uma Quadra autorizada</option>{saleBlocksQuery.data?.map((block) => <option key={block.blockId} value={block.blockId}>Quadra {block.blockNumber}</option>)}</select></label><label htmlFor="subdivision-sale-lot">Lote<select id="subdivision-sale-lot" value={saleLotId} onChange={(event) => setSaleLotId(event.target.value)} disabled={!isWorkspaceReady || !saleBlockId} required><option value="">Selecione um Lote autorizado</option>{saleLotsQuery.data?.map((lot) => <option key={lot.lotId} value={lot.lotId}>Lote {lot.lotNumber}</option>)}</select></label><label htmlFor="subdivision-sale-buyer">Cliente<select id="subdivision-sale-buyer" value={saleBuyerClientId} onChange={(event) => setSaleBuyerClientId(event.target.value)} disabled={!isWorkspaceReady || buyerClientsQuery.isLoading} required><option value="">{!isWorkspaceReady ? "Defina um contexto autorizado" : buyerClientsQuery.isLoading ? "Carregando clientes autorizados" : buyerClientsQuery.isError ? "Leitura de clientes não liberada" : buyerClientsQuery.data?.length ? "Selecione um cliente autorizado" : "Nenhum cliente elegível neste contexto"}</option>{buyerClientsQuery.data?.map((client) => <option key={client.buyerClientId} value={client.buyerClientId}>{buyerClientSelectionLabel(client, partyRolesQuery.data ?? [])}</option>)}</select></label><button type="submit" disabled={!isWorkspaceReady || !saleLotId || !saleBuyerClientId || createSaleDraftMutation.isPending}>{createSaleDraftMutation.isPending ? "Vinculando preparação" : "Iniciar preparação de venda"}</button></form>
