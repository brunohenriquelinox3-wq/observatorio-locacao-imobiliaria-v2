...
import { SubdivisionPartnerParticipationWorkspace } from "@/components/SubdivisionPartnerParticipationWorkspace";
...
const sectorPresentation: Record<LoteadoraSector, { index: string; title: string; description: string }> = {
  ...
  partners: { index: "Setor 03", title: "Sócios e Parceiros", description: "Vincule papéis internos, versões de participação e previsões operacionais por loteamento, sem executar repasse financeiro." },
  finance: { index: "Setor 04", title: "Financeiro", description: "Controle interno dos lotes de parcelas liberados após vendas aprovadas, sem emissão ou integração bancária." },
};
...
export default function SubdivisionFoundation() {
  ...
  const [saleBuyerSearchTerm, setSaleBuyerSearchTerm] = useState("");
  ...
  const buyerClientsQuery = trpc.subdivisionFoundation.listDraftBuyerClients.useQuery(context, { enabled: false, retry: false });
  ...
  const saleBuyerDirectoryQueryInput = useMemo(() => ({ ...context, searchTerm: saleBuyerSearchTerm.trim() || null, pageSize: 25, pageOffset: 0 }), [context, saleBuyerSearchTerm]);
  const saleBuyerDirectoryQuery = trpc.subdivisionFoundation.listDraftBuyerClientDirectory.useQuery(saleBuyerDirectoryQueryInput, { enabled: isWorkspaceReady && isCentralSalesJourney && Boolean(saleBuyerSearchTerm.trim()), retry: false });
  const saleDirectoryBuyerClients = saleBuyerSearchTerm.trim() ? saleBuyerDirectoryQuery.data : directoryBuyerClients;
  ...
  const saleCasePartiesQuery = trpc.subdivisionFoundation.listSaleCaseParties.useQuery(context, { enabled: isWorkspaceReady && isCentralSalesJourney, retry: false });
  ...
  const internalReceivableBatchesQuery = trpc.subdivisionFoundation.listInternalReceivableBatches.useQuery(context, { enabled: isWorkspaceReady && (isCentralSalesJourney || activeSector === "finance"), retry: false });
  const internalReceivableAlertConfigurationQuery = trpc.subdivisionFoundation.getInternalReceivableAlertConfiguration.useQuery(context, { enabled: isWorkspaceReady && activeSector === "finance", retry: false });
  ...
  useEffect(() => {
    ...
    setSaleBuyerClientId((value) => retainAuthorizedSelection(value, directoryBuyerClients, (client) => client.buyerClientId));
    setCoBuyerClientId((value) => retainAuthorizedSelection(value, directoryBuyerClients, (client) => client.buyerClientId));
  }, [buyerDirectoryQuery.isFetching, directoryBuyerClients, profileBuyerClients]);
  useEffect(() => {
  ...
  const addSaleCaseJointProponentMutation = trpc.subdivisionFoundation.addSaleCaseJointProponent.useMutation({ onSuccess() { toast.success("Proponente conjunto vinculado", { description: "A preparação mantém os clientes distintos e a composição exigirá nova revisão humana antes da aprovação." }); void utils.subdivisionFoundation.listSaleCaseParties.invalidate(context); }, onError() { toast.error("Proponente não vinculado", { description: "O servidor exige preparação disponível, cliente distinto, sessão autorizada e mesmo contexto." }); } });
  const removeSaleCaseJointProponentMutation = trpc.subdivisionFoundation.removeSaleCaseJointProponent.useMutation({ onSuccess() { toast.success("Proponente conjunto removido", { description: "A composição voltou para revisão humana; nenhum cliente, contrato, lote ou parcela foi apagado." }); void utils.subdivisionFoundation.listSaleCaseParties.invalidate(context); }, onError() { toast.error("Proponente não removido", { description: "A remoção só é permitida enquanto a preparação ainda não foi formalizada." }); } });
  ...
  const approveSaleCaseMutation = trpc.subdivisionFoundation.approveSaleCase.useMutation({ onSuccess(result) { setSaleCaseId(result.saleCaseId); toast.success("Venda aprovada e lote interno liberado", { description: result.internalBatchReleased ? "O lote foi marcado como vendido e as parcelas negociadas foram liberadas somente para controle interno do operador." : "O lote foi marcado como vendido. Nenhuma emissão bancária, cobrança, baixa ou pagamento foi executado." }); void utils.subdivisionFoundation.listSaleCases.invalidate(context); void utils.subdivisionFoundation.listInternalSaleContracts.invalidate(context); void utils.subdivisionFoundation.listInternalReceivableBatches.invalidate(context); }, onError() { toast.error("Venda não aprovada", { description: "A confirmação exige sessão AAL2, contrato interno em revisão, agenda e dossiê revisado no mesmo contexto autorizado." }); } });
  ...
  const configureInternalReceivableAlertsMutation = trpc.subdivisionFoundation.configureInternalReceivableAlerts.useMutation({ onSuccess(result) { toast.success("Lembretes internos preparados", { description: `A antecedência de ${result.leadDays} dia(s) foi registrada sem ativar agendamento, mensagem externa, cobrança ou baixa.` }); void utils.subdivisionFoundation.getInternalReceivableAlertConfiguration.invalidate(context); }, onError() { toast.error("Lembretes internos não preparados", { description: "O servidor exige sessão AAL2, contexto autorizado e antecedência entre 1 e 14 dias." }); } });
  const releaseInternalReceivableBatchMutation = trpc.subdivisionFoundation.releaseInternalReceivableBatch.useMutation({ onSuccess(result) { setSaleCaseId(result.saleCaseId); toast.success("Lote interno de parcelas liberado", { description: `${result.scheduledItemCount} parcela(s) foram disponibilizadas apenas para controle interno. Nenhuma emissão bancária, cobrança, mensagem, baixa ou pagamento foi executado.` }); void utils.subdivisionFoundation.listInternalReceivableBatches.invalidate(context); }, onError() { toast.error("Lote interno não liberado", { description: "A liberação exige venda e contrato aprovados, agenda existente, sessão AAL2 e contexto autorizado." }); } });
    ...
    <DashboardLayout navigationItems={crmNavigationItems} navigationTitle="Núcleo CRM" accessGate={subdivisionAccessGate}>
      <main className="subdivision-foundation-page">
        ...
        {activeSector === "partners" && <section id="subdivision-parties" className="subdivision-foundation-workspace" aria-labelledby="subdivision-role-title">
          ...
          <SubdivisionPartnerParticipationWorkspace context={context} isReady={isWorkspaceReady} developments={(developmentsQuery.data ?? []).map((development, index) => ({ developmentId: development.developmentId, label: formatSubdivisionDevelopmentLabel(index, development.displayName, development.internalReference) }))} partyRoles={eligibleInternalPartyRoles.map((role) => ({ ...role, displayName: subdivisionPartyRoleSelectionLabel(role) }))} />
        </section>}
		...
		{isCentralSalesJourney && <section id="subdivision-sales" className="subdivision-foundation-workspace" aria-label="Jornada de venda da Central de Vendas">
		  <SubdivisionSaleCaseWorkspace isWorkspaceReady={isWorkspaceReady} developmentReadState={developmentsQuery.isLoading ? "loading" : developmentsQuery.isError ? "error" : "ready"} blockReadState={saleBlocksQuery.isLoading ? "loading" : saleBlocksQuery.isError ? "error" : "ready"} lotReadState={saleLotsQuery.isLoading ? "loading" : saleLotsQuery.isError ? "error" : "ready"} developments={(developmentsQuery.data ?? []).map((development, index) => ({ id: development.developmentId, label: formatSubdivisionDevelopmentLabel(index, development.displayName, development.internalReference) }))} blocks={(saleBlocksQuery.data ?? []).map((block) => ({ id: block.blockId, label: `Quadra ${block.blockNumber}` }))} lots={(saleLotsQuery.data ?? []).map((lot) => ({ id: lot.lotId, label: `Lote ${lot.lotNumber}` }))} buyers={(saleDirectoryBuyerClients ?? []).map((client) => ({ id: client.buyerClientId, label: client.displayName }))} buyerReadState={saleBuyerSearchTerm.trim() ? saleBuyerDirectoryQuery.isLoading ? "loading" : saleBuyerDirectoryQuery.isError ? "error" : "ready" : buyerDirectoryQuery.isLoading ? "loading" : buyerDirectoryQuery.isError ? "error" : "ready"} buyerSearchTerm={saleBuyerSearchTerm} selectedDevelopmentId={saleDevelopmentId} selectedBlockId={saleBlockId} selectedLotId={saleLotId} selectedBuyerClientId={saleBuyerClientId} selectedSaleCaseId={saleCaseId} fiscalReference={saleFiscalReference} lookupState={saleLookupState} priceContext={saleLotPriceContextQuery.data} saleCases={saleCasesQuery.data ?? []} saleCaseParties={saleCasePartiesQuery.data ?? []} openingCase={openSaleCaseMutation.isPending} savingTerms={saveSaleCaseTermsMutation.isPending} addingJointProponent={addSaleCaseJointProponentMutation.isPending} removingJointProponent={removeSaleCaseJointProponentMutation.isPending} formalizingCase={formalizeSaleCaseMutation.isPending} approvingCase={approveSaleCaseMutation.isPending} requestingReversal={requestSaleReversalMutation.isPending} configuringAlerts={configureInternalReceivableAlertsMutation.isPending} managingAlertSchedule={manageInternalReceivableAlertScheduleMutation.isPending} alertScheduleState={alertScheduleState} internalContracts={internalSaleContractsQuery.data ?? []} internalAttention={internalReceivableAttentionQuery.data ?? []} internalBatches={internalReceivableBatchesQuery.data ?? []} releasingInternalBatch={releaseInternalReceivableBatchMutation.isPending} onDevelopmentChange={(value) => { setSaleDevelopmentId(value); setSaleBlockId(""); setSaleLotId(""); setSaleCaseId(""); }} onBlockChange={(value) => { setSaleBlockId(value); setSaleLotId(""); setSaleCaseId(""); }} onLotChange={(value) => { setSaleLotId(value); setSaleCaseId(""); }} onBuyerChange={setSaleBuyerClientId} onSearchBuyer={(value) => { setSaleBuyerSearchTerm(value); setSaleBuyerClientId(""); }} onSaleCaseChange={setSaleCaseId} onFiscalReferenceChange={(value) => { setSaleFiscalReference(value); setSaleLookupCorrelationId(crypto.randomUUID()); setSaleLookupState("idle"); }} onLookupBuyer={lookupSaleBuyerByFiscalReference} onOpenCase={() => openSaleCaseMutation.mutate({ ...context, correlationId: crypto.randomUUID(), lotId: saleLotId, buyerClientId: saleBuyerClientId })} onAddJointProponent={(buyerClientId) => addSaleCaseJointProponentMutation.mutate({ ...context, correlationId: crypto.randomUUID(), saleCaseId, buyerClientId })} onRemoveJointProponent={(buyerClientId) => removeSaleCaseJointProponentMutation.mutate({ ...context, correlationId: crypto.randomUUID(), saleCaseId, buyerClientId })} onSaveTerms={(terms) => saveSaleCaseTermsMutation.mutate({ ...context, correlationId: crypto.randomUUID(), saleCaseId, ...terms })} onFormalizeCase={() => formalizeSaleCaseMutation.mutate({ ...context, correlationId: crypto.randomUUID(), saleCaseId })} onApproveCase={() => approveSaleCaseMutation.mutate({ ...context, correlationId: crypto.randomUUID(), saleCaseId })} onRequestReversal={() => requestSaleReversalMutation.mutate({ ...context, correlationId: crypto.randomUUID(), saleCaseId })} onConfigureAlerts={(leadDays) => configureInternalReceivableAlertsMutation.mutate({ ...context, correlationId: crypto.randomUUID(), leadDays })} onManageAlertSchedule={(action) => manageInternalReceivableAlertScheduleMutation.mutate({ ...context, correlationId: crypto.randomUUID(), action })} onReleaseInternalBatch={() => releaseInternalReceivableBatchMutation.mutate({ ...context, correlationId: crypto.randomUUID(), saleCaseId })} />
        ...
        {activeSector === "finance" && <SubdivisionInternalReceivableBatchPanel isReady={isWorkspaceReady} batches={internalReceivableBatchesQuery.data} isLoading={internalReceivableBatchesQuery.isLoading} isError={internalReceivableBatchesQuery.isError} alertScheduleState={alertScheduleState} managingAlertSchedule={manageInternalReceivableAlertScheduleMutation.isPending} onManageAlertSchedule={(action) => manageInternalReceivableAlertScheduleMutation.mutate({ ...context, correlationId: crypto.randomUUID(), action })} alertConfiguration={internalReceivableAlertConfigurationQuery.data} alertConfigurationLoading={internalReceivableAlertConfigurationQuery.isLoading} alertConfigurationError={internalReceivableAlertConfigurationQuery.isError} configuringAlerts={configureInternalReceivableAlertsMutation.isPending} onConfigureAlerts={(leadDays) => configureInternalReceivableAlertsMutation.mutate({ ...context, correlationId: crypto.randomUUID(), leadDays })} />}
      </main>
    </DashboardLayout>
  );
}
