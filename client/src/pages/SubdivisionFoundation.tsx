    ...
    <DashboardLayout navigationItems={navigationItems} navigationTitle="Núcleo CRM" accessGate={subdivisionAccessGate}>
      <main className="subdivision-foundation-page">
        ...
        <section className="subdivision-foundation-workspace" aria-labelledby="subdivision-sale-draft-title">
          ...
          {isWorkspaceReady && saleDraftsQuery.data && saleDraftsQuery.data.length > 0 && <div className="subdivision-foundation-list__rows">{saleDraftsQuery.data.map((draft) => { const coverage = saleDraftAttachmentCoverageQuery.data?.find((item) => item.saleDraftId === draft.saleDraftId); const workState = saleDraftWorkStatesQuery.data?.find((item) => item.saleDraftId === draft.saleDraftId); const coBuyerCount = saleDraftCoBuyersQuery.data?.filter((item) => item.saleDraftId === draft.saleDraftId).length ?? 0; return <article key={draft.saleDraftId}><span>Rascunho interno de venda</span><h3>Vínculo de lote e cliente comprador</h3><p><b>{coverage ? saleDraftAttachmentCoverageStates[coverage.attachmentCoverageState] : "Cobertura de anexo não disponível"}</b> · estado opaco, sem documento, nome, URL, download ou visualização.</p><p><b>{workState ? saleDraftWorkPhases[workState.workPhase] : "Sem classificação interna"}</b> · organização de trabalho, sem efeito comercial.</p><p><b>{coBuyerCount} co-comprador(es) interno(s)</b> · sem titularidade, percentual, preço ou obrigação financeira.</p><p>Criado em {new Date(draft.createdAt).toLocaleString("pt-BR")}. Sem reserva, proposta, contrato, cobrança ou financeiro.</p><code>{draft.saleDraftId}</code></article>; })}</div>}
