...
const sectorPresentation: Record<LoteadoraSector, { index: string; title: string; description: string }> = {
  ...
  clients: { index: "Setor 02", title: "Clientes Loteadora", description: "Organize cadastro, contatos, identificação e documentos privados sem duplicar dados ou iniciar venda." },
...
export default function SubdivisionFoundation() {
  ...
  useEffect(() => {
    ...
    setBuyerClientIdForProfile((value) => retainAuthorizedSelection(value, buyerClientsQuery.data, (client) => client.buyerClientId));
  ...
  return (
    <DashboardLayout navigationItems={crmNavigationItems} navigationTitle="Núcleo CRM" accessGate={subdivisionAccessGate}>
      <main className="subdivision-foundation-page">
        ...
        <ReportExportActions report={{ title: "Resumo de Clientes Loteadora", scopeLabel: "Clientes em organização no contexto selecionado", rows: [{ section: "Cadastro", indicator: "Clientes em organização", status: buyerClientsQuery.data ? `${buyerClientsQuery.data.length} registro(s) autorizado(s)` : "Leitura pendente ou bloqueada" }, { section: "Anexos", indicator: "Cobertura", status: "Estado privado e redigido" }, { section: "Segurança", indicator: "Contexto", status: isWorkspaceReady ? "Confirmado pelo servidor" : "Não selecionado" }] }} isAuthorized={isWorkspaceReady} description="Exporte um resumo redigido da prontidão de clientes; nomes, documentos e identificadores não são incluídos." />
        <SubdivisionBuyerClientDirectory context={context} isContextReady={isContextReady} isWorkspaceReady={isWorkspaceReady} selectedBuyerClientId={buyerClientIdForProfile} onSelectBuyerClient={setBuyerClientIdForProfile} />
        <section id="subdivision-buyers" className="subdivision-foundation-workspace" aria-labelledby="subdivision-buyer-title">
          <div className="subdivision-foundation-heading"><div><p className="subdivision-foundation-eyebrow">07 · CADASTRO POR VÍNCULO EXISTENTE</p><h2 id="subdivision-buyer-title">Mantenha disponível o vínculo de cliente já existente.</h2></div><p>Use este caminho apenas quando o Cliente Loteadora já possuir um papel temporal elegível no contexto. Para novo cadastro, use a Central de Clientes acima. Esta etapa não cria venda, lote, contrato, cobrança ou valor.</p></div>
          ...
          {isWorkspaceReady && buyerClientsQuery.data?.length === 0 && <div className="subdivision-foundation-empty"><UsersRound size={18} /><p>Nenhum Cliente Loteadora foi devolvido para este contexto.</p></div>}
          {isWorkspaceReady && buyerClientsQuery.data && buyerClientsQuery.data.length > 0 && <div className="subdivision-foundation-list__rows">{buyerClientsQuery.data.map((client) => <article key={client.buyerClientId}><span>Cliente Loteadora</span><h3>{buyerClientSelectionLabel(client, partyRolesQuery.data ?? [])}</h3><p>Registrado em {new Date(client.createdAt).toLocaleString("pt-BR")}. Nenhum identificador técnico é exibido.</p></article>)}</div>}
        </section>
        ...
        <SubdivisionBuyerClientProfile context={context} isContextReady={isContextReady} isWorkspaceReady={isWorkspaceReady} buyerClients={buyerClientsQuery.data} partyRoles={partyRolesQuery.data} selectedBuyerClientId={buyerClientIdForProfile} onSelectBuyerClient={setBuyerClientIdForProfile} />
