...
export default function SubdivisionFoundation() {
  ...
  const buyerClientsQuery = trpc.subdivisionFoundation.listDraftBuyerClients.useQuery(context, { enabled: isWorkspaceReady && activeSector === "sales", retry: false });
    ...
    <DashboardLayout navigationItems={crmNavigationItems} navigationTitle="Núcleo CRM" accessGate={subdivisionAccessGate}>
      <main className="subdivision-foundation-page">
        ...
        {isBuyerProfilePage && <section id="buyer-profile-standalone" className="subdivision-foundation-workspace" aria-labelledby="buyer-profile-standalone-title">
          <div className="subdivision-foundation-heading"><div><p className="subdivision-foundation-eyebrow">06 · FICHA CADASTRAL</p><h2 id="buyer-profile-standalone-title">Edição completa do Cliente Loteadora</h2></div><p>Esta é a jornada compatível da ficha detalhada. Ela preserva o mesmo contexto autorizado e não abre venda, lote, crédito, contrato ou financeiro.</p></div>
          ...
          <SubdivisionBuyerClientProfile context={context} isContextReady={isContextReady} isWorkspaceReady={isWorkspaceReady} buyerClients={directoryBuyerClients} selectedBuyerClientId={buyerClientIdForProfile} onSelectBuyerClient={selectBuyerClient} />
        </section>}
        <section id="subdivision-buyers" className="subdivision-foundation-workspace" aria-labelledby="subdivision-buyer-title">
          ...
          {isWorkspaceReady && directoryBuyerClients && directoryBuyerClients.length > 0 && <div className="subdivision-foundation-list__rows">{directoryBuyerClients.map((client) => <article key={client.buyerClientId}><span>Cliente Loteadora</span><h3>{client.displayName}</h3><p>Disponível no diretório autorizado. Nenhum identificador técnico é exibido.</p></article>)}</div>}
        </section>
