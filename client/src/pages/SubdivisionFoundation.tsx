    ...
    <DashboardLayout navigationItems={navigationItems} navigationTitle="Núcleo CRM" accessGate={subdivisionAccessGate}>
      <main className="subdivision-foundation-page">
        ...
        <section className="subdivision-foundation-workspace" aria-labelledby="subdivision-economic-rule-title">
          ...
          {isWorkspaceReady && economicRuleSetsQuery.data && economicRuleSetsQuery.data.length > 0 && <div className="subdivision-foundation-list__rows">{economicRuleSetsQuery.data.map((ruleSet) => <article key={ruleSet.economicRuleSetId}><span>Regra econômica interna</span><h3>{ruleSet.versionReference}</h3><p><b>{economicRuleSetWorkingStates[ruleSet.workingState]}</b> · referência de trabalho sem valores, percentuais, parcelas, pagamentos, cobrança, repasses ou integração externa.</p><code>{ruleSet.economicRuleSetId}</code></article>)}</div>}
        </section>
      </main>
    </DashboardLayout>
  );
