...
type Lesson = {
  ...
  avoid: string;
  link?: string;
  screen: "context" | "lot" | "sale" | "finance" | "partners" | "client" | "permutant";
};
...
const lessons: Lesson[] = [
  ...
  { id: "permutant", number: "07", title: "Usar o painel do Permutante", audience: "Permutante · administrativo", duration: "6 min", description: "O painel do Permutante concentra somente o acompanhamento que foi autorizado para a sua participação. Ele não substitui contrato, banco ou reunião de conferência.", steps: ["Entre com a conta Google vinculada ao seu acesso.", "Abra o painel de Sócios e Parceiros destinado à sua participação.", "Confira empreendimento, escopo e vigência exibidos antes de interpretar qualquer informação.", "Leia as projeções como acompanhamento interno, nunca como ordem de pagamento.", "Se houver diferença, envie a solicitação ao responsável com a referência do empreendimento e do período."], observe: "Cada acesso mostra apenas o escopo autorizado. O que não aparece não deve ser inferido nem solicitado por atalhos.", avoid: "Não compartilhe capturas, acesso ou dados de outra participação; o painel não autoriza pagamento, transferência ou alteração de regras.", link: "/loteadora/socios-parceiros", screen: "permutant" },
];

const firstLessonByTrack: Record<RoleKey, string> = {
  admin: "context", finance: "finance", sales: "sale", broker: "lot", legal: "client", permutant: "permutant", partners: "partners",
};
...
function ScreenMockup({ type }: { type: Lesson["screen"] }) {
  const content = {
    ...
    permutant: { label: "PAINEL DO PERMUTANTE · ACOMPANHAMENTO", title: "Participação autorizada", fields: [["Empreendimento", "Vista do Sol"], ["Vigência", "Em acompanhamento"], ["Visão", "Projeção interna"]], accent: "permutant" },
  }[type];
...
export default function OperatorManual() {
  ...
  return <DashboardLayout navigationTitle="Núcleo CRM" accessGate={accessGate}>
    <main className="operator-manual">
      ...
      <section className="operator-manual__section operator-manual__gallery"><div className="operator-manual__section-heading"><div><p className="operator-manual__eyebrow">GALERIA DE PRINTS GUIADOS</p><h2>Reconheça as telas antes de operar.</h2></div><p>Todos os exemplos abaixo usam informações fictícias. Clique em uma tela para abrir a aula correspondente e ver o passo a passo.</p></div><div className="operator-manual__gallery-grid">{lessons.map((lesson) => <button key={lesson.id} type="button" className={activeLesson === lesson.id ? "is-active" : ""} onClick={() => setActiveLesson(lesson.id)}><ScreenMockup type={lesson.screen} /><span><b>AULA {lesson.number}</b>{lesson.title}<small>Ver instruções <ArrowRight size={12} /></small></span></button>)}</div></section>
