...
export default function OwnWorkforceAccess() {
  ...
  const requestMutation = trpc.administration.requestOwnWorkforceAccess.useMutation({
    onSuccess() { toast.success("Solicitação registrada", { description: "Nenhum acesso foi criado. Aguarde a preparação por um ADM e aceite apenas sua própria delegação em uma sessão válida." }); setOrganizationReference(""); void requests.refetch(); },
  ...
  const acceptMutation = trpc.administration.acceptOwnWorkforceAccess.useMutation({
    ...
    onError() { toast.error("Aceite não concluído", { description: "Sessão e preparação válidas continuam obrigatórias. Nenhum acesso foi ativado." }); },
  });
