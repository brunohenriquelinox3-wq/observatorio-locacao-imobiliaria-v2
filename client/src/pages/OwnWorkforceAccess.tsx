...
export default function OwnWorkforceAccess() {
  ...
  return <main className="workforce-self">
    <section className="workforce-self__shell" aria-labelledby="workforce-self-title">
      ...
      <aside className="workforce-self__status" aria-live="polite"><div className="workforce-panel__subheading"><ShieldCheck size={18} aria-hidden="true" /><h2>Minhas delegações</h2></div>{!isAuthenticated ? <p>Conecte sua sessão da plataforma para iniciar a confirmação de identidade.</p> : identity.isLoading ? <p>Confirmando a sessão Supabase antes de liberar a solicitação.</p> : !hasSupabaseSubject ? <p>Conecte a sessão Supabase em <a href="/entrar?destino=%2Facesso-equipe">/entrar</a>. A sessão da plataforma, isoladamente, não libera solicitação nem leitura.</p> : requests.isLoading ? <p>Confirmando o contexto seguro.</p> : (requests.data?.length ?? 0) === 0 ? <p>Nenhuma solicitação devolvida. Isto não revela solicitações de outras pessoas.</p> : <ol className="workforce-panel__request-list">{requests.data?.map((request, index) => <li key={request.requestId}><span>Minha solicitação {String(index + 1).padStart(2, "0")}</span><strong>{profileLabel[request.profile]}</strong><small>{request.organizationLabel} · {request.state === "prepared" ? "Aguardando meu aceite com MFA" : request.state === "active" ? "Acesso ativo" : "Aguardando preparação"}</small>{request.state === "prepared" && <button type="button" onClick={() => acceptMutation.mutate({ requestId: request.requestId, correlationId: crypto.randomUUID() })} disabled={acceptMutation.isPending}>{acceptMutation.isPending ? "Validando aceite" : "Aceitar minha delegação"}</button>}</li>)}</ol>}</aside></div>
      <footer><ShieldCheck size={17} aria-hidden="true" /><p>O aceite é pessoal e não altera papel, módulos ou vigência preparados. O servidor confirma identidade, MFA e policy antes de ativar qualquer acesso.</p></footer>
    </section>
  </main>;
}
