...
export default function SupabaseLogin() {
  ...
  const showSignInForm = state === "missing";
  const showActiveSession = state === "ready";
    ...
    <main className="min-h-screen bg-[#f4f5ee] px-4 py-10 text-[#14343e] sm:px-6 lg:py-16">
      <section className="mx-auto grid w-full max-w-5xl overflow-hidden rounded-[24px] border border-[#d7e2df] bg-white shadow-[0_24px_70px_rgba(9,44,52,0.10)] lg:grid-cols-[0.9fr_1.1fr]">
        <aside className="bg-[#103d49] p-7 text-[#eef9f7] sm:p-10">
          <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#5bd9bd] text-[#103d49]">
        ...
        <div className="p-7 sm:p-10">
          <div className="flex items-start gap-3 rounded-xl border border-[#c8e8df] bg-[#edf9f5] p-4 text-sm leading-6 text-[#205e54]">
            ...
            <p><strong>Entrada separada e mínima.</strong> A sessão Google é criada diretamente pelo provedor. Ela não substitui vínculo, alçada ou validação de contexto no servidor.</p>
          </div>

          {state === "checking" ? <div className="mt-10 flex items-center gap-3 text-sm text-[#5c7478]"><Loader2 className="h-5 w-5 animate-spin" aria-hidden="true" /> Verificando se existe uma sessão de contexto neste dispositivo.</div> : null}

          {state === "unavailable" ? <div className="mt-10"><p className="text-xs font-extrabold tracking-[0.16em] text-[#1d8775]">SESSÃO INDISPONÍVEL</p><h2 className="mt-3 text-2xl font-bold tracking-[-0.03em]">A validação de contexto não está disponível nesta prévia.</h2><p className="mt-3 max-w-lg text-sm leading-6 text-[#5c7478]">Nenhum acesso foi liberado. Revise a configuração de ambiente antes de tentar novamente.</p></div> : null}

          {showSignInForm ? <section className="mt-10 max-w-lg" aria-labelledby="supabase-context-title"><p className="text-xs font-extrabold tracking-[0.16em] text-[#1d8775]">ENTRADA INDIVIDUAL</p><h2 className="mt-3 text-2xl font-bold tracking-[-0.03em]" id="supabase-context-title">Entre no contexto autorizado.</h2><p className="mt-3 text-sm leading-6 text-[#5c7478]">Use a conta Google de trabalho já ativada para este CRM. A autenticação não cria acesso: o servidor continuará validando seu vínculo, módulo e alçada.</p><button aria-label="Continuar com Google" className="mt-7 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-lg border border-[#b8d3ce] bg-white px-4 py-2 text-sm font-bold text-[#164a45] transition hover:bg-[#f2f8f6] active:scale-[0.97] focus:outline-none focus-visible:ring-4 focus-visible:ring-[#5bd9bd]/50 disabled:cursor-wait disabled:opacity-70" disabled={isStartingGoogle || isSubmitting} onClick={signInWithGoogle} type="button">{isStartingGoogle ? <><Loader2 className="h-4 w-4 animate-spin" aria-hidden="true" /> Direcionando para o Google</> : <><Chrome size={18} aria-hidden="true" /> Continuar com Google</>}</button><div className="my-7 flex items-center gap-3 text-xs font-semibold text-[#71888a]" aria-hidden="true"><span className="h-px flex-1 bg-[#d8e3e0]" />OU<span className="h-px flex-1 bg-[#d8e3e0]" /></div><details className="rounded-xl border border-[#d7e2df] bg-[#f9fbfa] p-4"><summary className="cursor-pointer text-sm font-bold text-[#31565b]">Usar acesso de contingência com senha</summary><p className="mt-3 text-sm leading-6 text-[#5c7478]">Mantenha esta alternativa apenas durante a transição autorizada. Em caso de falha, a tela não revela se uma conta existe.</p><form className="mt-5" onSubmit={signIn}><label className="block text-sm font-bold text-[#294e55]" htmlFor="supabase-context-email">E-mail de acesso<input autoComplete="email" className="mt-2 min-h-11 w-full rounded-lg border border-[#bbced0] bg-white px-3 text-[#14343e] outline-none transition placeholder:text-[#7c9295] focus:border-[#1b9c87] focus:ring-4 focus:ring-[#5bd9bd]/25" id="supabase-context-email" inputMode="email" onChange={(event) => setEmail(event.target.value)} required type="email" value={email} /></label><label className="mt-5 block text-sm font-bold text-[#294e55]" htmlFor="supabase-context-password">Senha<input autoComplete="current-password" className="mt-2 min-h-11 w-full rounded-lg border border-[#bbced0] bg-white px-3 text-[#14343e] outline-none transition focus:border-[#1b9c87] focus:ring-4 focus-visible:ring-[#5bd9bd]/25" id="supabase-context-password" onChange={(event) => setPassword(event.target.value)} required type="password" value={password} /></label><button className="mt-5 inline-flex min-h-11 items-center gap-2 rounded-lg bg-[#126f62] px-4 py-2 text-sm font-bold text-white transition hover:bg-[#0b5a50] active:scale-[0.97] focus:outline-none focus-visible:ring-4 focus-visible:ring-[#5bd9bd]/50 disabled:cursor-wait disabled:opacity-70" disabled={isSubmitting || isStartingGoogle} type="submit">{isSubmitting ? <><Loader2 className="h-4 w-4 animate-spin" aria-hidden="true" /> Validando</> : <>Validar contingência <ArrowRight size={17} aria-hidden="true" /></>}</button></form></details>{errorMessage ? <p className="mt-5 flex gap-2 rounded-lg border border-[#f0c4b5] bg-[#fff5f1] p-3 text-sm leading-6 text-[#8a3f28]"><CircleAlert className="mt-0.5 h-5 w-5 shrink-0" aria-hidden="true" />{errorMessage}</p> : null}</section> : null}
