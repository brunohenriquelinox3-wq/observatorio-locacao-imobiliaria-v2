...
const agendaClassificationLabels = { lead_review: "Revisão do lead", context_preparation: "Preparação de contexto", internal_follow_up: "Acompanhamento interno" } as const;

type UrbanSector = "clients" | "properties" | "developments" | "agenda" | "search" | "proposals" | "finance";

const urbanSectorByPath: Record<string, UrbanSector> = {
...
const urbanSectorPresentation: Record<UrbanSector, { index: string; title: string; description: string }> = {
  clients: { index: "Setor 01", title: "Clientes e Leads", description: "Qualificação de interesse e leitura de leads, sem proposta ou compromisso comercial." },
  properties: { index: "Setor 02", title: "Imóveis e Proprietários", description: "Vínculos internos entre ativo urbano e interesse declarado, sem publicação ou disponibilidade comercial." },
  developments: { index: "Setor 03", title: "Empreendimentos e Construtoras", description: "Estrutura reservada para a modelagem própria de empreendimento, torre, condomínio e construtora." },
  ...
  search: { index: "Setor 05", title: "Perfil de Busca", description: "Preferências codificadas que apoiam a triagem sem recomendação automática." },
  proposals: { index: "Setor 06", title: "Propostas, Reservas e Contratos", description: "Setor delimitado, ainda bloqueado para desenvolvimento material." },
...
export default function UrbanPipeline() {
  ...
  const activeUrbanSector = urbanSectorByPath[location] ?? "clients";
  const activeUrbanPresentation = urbanSectorPresentation[activeUrbanSector];
  ...
  const agendaClassificationsQuery = trpc.urbanPipeline.listDraftAgendaClassifications.useQuery(context, { enabled, retry: false });
  ...
  const agendaMutation = trpc.urbanPipeline.createDraftAgenda.useMutation({
    onSuccess() { setAgendaLeadId(""); setScheduledFor(""); setAgendaReason(""); toast.success("Agenda criada", { description: "O compromisso é interno: não integrou calendário, não enviou convite e não confirmou visita." }); void leadsQuery.refetch(); void agendasQuery.refetch(); },
  ...
  const agendaClassificationMutation = trpc.urbanPipeline.upsertDraftAgendaClassification.useMutation({
    onSuccess() {
      setClassificationAgendaId(""); setAgendaClassification("lead_review"); setAgendaInternalCode("");
      ...
      void utils.urbanPipeline.listDraftAgendaClassifications.invalidate(context);
    },
  ...
  return (
    <DashboardLayout navigationItems={crmNavigationItems} navigationTitle="Núcleo CRM" accessGate={urbanAccessGate}>
      <div className="urban-pipeline-page" data-active-sector={activeUrbanSector}>
        <header className="urban-workspace-header"><div><p className="urban-pipeline-eyebrow">VENDAS URBANAS · {activeUrbanPresentation.index.toUpperCase()}</p><h1>{activeUrbanPresentation.title}</h1><p>{activeUrbanPresentation.description}</p></div><div className="urban-workspace-header__status"><ShieldCheck size={18} aria-hidden="true" /><span>Contexto e alçada<br /><b>confirmados pelo servidor</b></span></div></header>
        ...
        {(activeUrbanSector === "developments" || activeUrbanSector === "proposals" || activeUrbanSector === "finance") && <section className="urban-sector-locked" aria-labelledby="urban-sector-locked-title"><CircleSlash2 size={26} aria-hidden="true" /><div><p className="urban-pipeline-eyebrow">SETOR BLOQUEADO</p><h2 id="urban-sector-locked-title">{activeUrbanSector === "developments" ? "A modelagem de empreendimentos será construída como setor próprio." : activeUrbanSector === "proposals" ? "Propostas, reservas e contratos ainda não estão liberados." : "Financeiro ainda não está liberado para desenvolvimento."}</h2><p>{activeUrbanSector === "developments" ? "Não há dados de construtora, empreendimento, torre, condomínio ou unidade nesta etapa." : "Não há valores, percentuais, cálculos, contratos, parcelas, cobrança, pagamentos, repasses ou integrações externas nesta área."}</p></div></section>}

        <section id="urban-leads" data-urban-section="clients-agenda" className="urban-pipeline-workspace" aria-labelledby="urban-workspace-title"><div className="urban-pipeline-heading"><div><p className="urban-pipeline-eyebrow">02 · ORIGEM, ETAPA E AGENDA</p><h2 id="urban-workspace-title">Cada ação produz um fato limitado.</h2></div><p>Party, lead, etapa e agenda são objetos separados. Nenhum formulário captura contatos, documentos, renda, score, ativo específico, preço ou nota livre.</p></div><div className="urban-pipeline-grid">
          <form className="urban-pipeline-card urban-lead-entry" onSubmit={createLead}><div className="urban-pipeline-card__title"><UserRoundPlus size={19} /><h3>Lead de entrada</h3></div><p>Conecte uma Party em rascunho a uma origem codificada e a um interesse declarado.</p><label htmlFor="urban-party">Party autorizada<select id="urban-party" value={partyId} onChange={(event) => setPartyId(event.target.value)} disabled={!enabled || partiesQuery.isLoading} required><option value="">{!enabled ? "Defina um contexto autorizado" : partiesQuery.isLoading ? "Carregando Parties autorizadas" : partiesQuery.isError ? "Leitura de Parties não liberada" : partiesQuery.data?.length ? "Selecione uma Party autorizada" : "Nenhuma Party em rascunho neste contexto"}</option>{partiesQuery.data?.map((party) => <option key={party.partyId} value={party.partyId}>{urbanPartySelectionLabel(party)}</option>)}</select></label><label htmlFor="urban-source">Origem em código</label><input id="urban-source" value={sourceCode} onChange={(event) => setSourceCode(event.target.value.toUpperCase())} placeholder="EX.: OPERADOR" disabled={!enabled} required /><label htmlFor="urban-interest">Interesse declarado</label><select id="urban-interest" value={interestKind} onChange={(event) => setInterestKind(event.target.value as typeof interestKind)} disabled={!enabled}><option value="search_profile">Perfil de busca</option><option value="urban_asset">Ativo urbano</option><option value="unspecified">Não especificado</option></select><button type="submit" disabled={!enabled || !partyId || createLeadMutation.isPending}>{createLeadMutation.isPending ? "Criando lead" : "Criar lead em rascunho"}</button></form>
          <form className="urban-pipeline-card urban-lead-stage" onSubmit={transitionLead}><div className="urban-pipeline-card__title"><Workflow size={19} /><h3>Etapa de trabalho</h3></div><p>A transição é explícita, controlada por sequência e mantém encerramento sem ganho separado de qualquer venda.</p><label htmlFor="urban-stage-lead">Lead autorizado<select id="urban-stage-lead" value={stageLeadId} onChange={(event) => setStageLeadId(event.target.value)} disabled={!enabled || leadsQuery.isLoading} required><option value="">{!enabled ? "Defina um contexto autorizado" : leadsQuery.isLoading ? "Carregando leads autorizados" : leadsQuery.isError ? "Leitura de leads não liberada" : leadsQuery.data?.length ? "Selecione um lead autorizado" : "Nenhum lead em rascunho neste contexto"}</option>{leadsQuery.data?.map((lead) => <option key={lead.leadId} value={lead.leadId}>{urbanLeadSelectionLabel(lead)}</option>)}</select></label><label htmlFor="urban-next-stage">Próxima etapa</label><select id="urban-next-stage" value={nextStage} onChange={(event) => setNextStage(event.target.value as typeof nextStage)} disabled={!enabled}>{Object.entries(stageLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select>{nextStage === "closed_lost" && <><label htmlFor="urban-stage-reason">Motivo em código</label><input id="urban-stage-reason" value={stageReason} onChange={(event) => setStageReason(event.target.value.toUpperCase())} placeholder="EX.: DESISTENCIA" disabled={!enabled} required /></>}<button type="submit" disabled={!enabled || !stageLeadId || transitionMutation.isPending}>{transitionMutation.isPending ? "Atualizando etapa" : "Atualizar etapa"}</button></form>
          <form className="urban-pipeline-card urban-agenda-entry" onSubmit={createAgenda}><div className="urban-pipeline-card__title"><CalendarClock size={19} /><h3>Compromisso interno</h3></div><p>Registre a agenda interna sem integrar calendário de terceiro, convidar participante ou confirmar uma visita.</p><label htmlFor="urban-agenda-lead">Lead autorizado<select id="urban-agenda-lead" value={agendaLeadId} onChange={(event) => setAgendaLeadId(event.target.value)} disabled={!enabled || leadsQuery.isLoading} required><option value="">{!enabled ? "Defina um contexto autorizado" : leadsQuery.isLoading ? "Carregando leads autorizados" : leadsQuery.isError ? "Leitura de leads não liberada" : leadsQuery.data?.length ? "Selecione um lead autorizado" : "Nenhum lead em rascunho neste contexto"}</option>{leadsQuery.data?.map((lead) => <option key={lead.leadId} value={lead.leadId}>{urbanLeadSelectionLabel(lead)}</option>)}</select></label><label htmlFor="urban-agenda-date">Data e hora</label><input id="urban-agenda-date" type="datetime-local" value={scheduledFor} onChange={(event) => setScheduledFor(event.target.value)} disabled={!enabled} required /><label htmlFor="urban-agenda-state">Situação</label><select id="urban-agenda-state" value={agendaState} onChange={(event) => setAgendaState(event.target.value as typeof agendaState)} disabled={!enabled}>{Object.entries(agendaLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select>{["cancelled", "not_held"].includes(agendaState) && <><label htmlFor="urban-agenda-reason">Motivo em código</label><input id="urban-agenda-reason" value={agendaReason} onChange={(event) => setAgendaReason(event.target.value.toUpperCase())} placeholder="EX.: REMARCACAO_SOLICITADA" disabled={!enabled} required /></>}<button type="submit" disabled={!enabled || !agendaLeadId || agendaMutation.isPending}>{agendaMutation.isPending ? "Registrando agenda" : "Registrar agenda"}</button></form>
        </div></section>

        <section id="urban-assets" data-urban-section="properties" className="urban-pipeline-link" aria-labelledby="urban-asset-link-title"><div className="urban-pipeline-heading"><div><p className="urban-pipeline-eyebrow">02A · ATIVO DECLARADO</p><h2 id="urban-asset-link-title">Vincule um ativo somente ao lead interessado em ativo urbano.</h2></div><p>O servidor nega leads de busca ou não especificados e exige ativo em rascunho no módulo Vendas Urbanas. O vínculo não torna o ativo disponível nem cria proposta, reserva ou contrato.</p></div><div className="urban-pipeline-link__grid"><form className="urban-pipeline-card" onSubmit={linkLeadAsset}><div className="urban-pipeline-card__title"><Link2 size={19} /><h3>Vínculo interno de ativo</h3></div><p>Selecione referências já autorizadas no mesmo contexto. A relação é uma referência de qualificação, não uma oferta ou autorização comercial.</p><label htmlFor="urban-asset-link-lead">Lead interessado em ativo<select id="urban-asset-link-lead" value={assetLinkLeadId} onChange={(event) => setAssetLinkLeadId(event.target.value)} disabled={!enabled || leadsQuery.isLoading} required><option value="">{!enabled ? "Defina um contexto autorizado" : leadsQuery.isLoading ? "Carregando leads autorizados" : leadsQuery.isError ? "Leitura de leads não liberada" : leadsQuery.data?.some((lead) => lead.interestKind === "urban_asset") ? "Selecione um lead autorizado" : "Nenhum lead de ativo neste contexto"}</option>{leadsQuery.data?.filter((lead) => lead.interestKind === "urban_asset").map((lead) => <option key={lead.leadId} value={lead.leadId}>{urbanLeadSelectionLabel(lead)}</option>)}</select></label><label htmlFor="urban-asset-link-asset">Ativo urbano autorizado<select id="urban-asset-link-asset" value={assetLinkAssetId} onChange={(event) => setAssetLinkAssetId(event.target.value)} disabled={!enabled || assetsQuery.isLoading} required><option value="">{!enabled ? "Defina um contexto autorizado" : assetsQuery.isLoading ? "Carregando ativos autorizados" : assetsQuery.isError ? "Leitura de ativos não liberada" : assetsQuery.data?.length ? "Selecione um ativo autorizado" : "Nenhum ativo em rascunho neste contexto"}</option>{assetsQuery.data?.map((asset) => <option key={asset.assetId} value={asset.assetId}>{urbanAssetSelectionLabel(asset)}</option>)}</select></label><button type="submit" disabled={!enabled || !assetLinkLeadId || !assetLinkAssetId || assetLinkMutation.isPending}>{assetLinkMutation.isPending ? "Vinculando ativo" : "Vincular ativo em rascunho"}</button></form><aside className="urban-pipeline-link__limits" aria-label="Limites do vínculo de ativo"><ShieldCheck size={19} /><div><h3>Limites preservados</h3><p>Sem preço, endereço detalhado, mídia, matrícula, titularidade, disponibilidade, publicação, proposta, reserva, contrato, comissão, cobrança, repasse ou financeiro.</p></div></aside></div>{!contextReady && <div className="urban-pipeline-empty"><CircleAlert size={18} /><p>Sem contexto não há consulta nem indicação de vínculo de ativo.</p></div>}{contextReady && !isAuthenticated && <div className="urban-pipeline-empty"><ShieldCheck size={18} /><p>O vínculo e a leitura permanecem bloqueados até haver sessão autenticada.</p></div>}{enabled && leadAssetLinksQuery.isLoading && <div className="urban-pipeline-empty"><span className="urban-pipeline-spinner" aria-hidden="true" /><p>Confirmando o contexto antes de solicitar a leitura minimizada dos vínculos.</p></div>}{enabled && leadAssetLinksQuery.isError && <div className="urban-pipeline-empty is-error"><CircleAlert size={18} /><p>A leitura dos vínculos não foi liberada. Revise identidade, membership, grant, vigência e contexto sem tentar inferir ativos externos.</p></div>}{enabled && leadAssetLinksQuery.data?.length === 0 && <div className="urban-pipeline-empty"><Link2 size={18} /><p>Nenhum vínculo de ativo foi devolvido para este contexto. A resposta não revela ativos de outros contextos.</p></div>}{enabled && leadAssetLinksQuery.data && leadAssetLinksQuery.data.length > 0 && <div className="urban-pipeline-list__rows">{leadAssetLinksQuery.data.map((link) => <article key={link.linkId}><span>Ativo em rascunho</span><h3>{link.assetReferenceLabel}</h3><p><b>{assetKindLabels[link.assetKind]}</b> · código {link.assetInternalReference} · vínculo interno em {new Date(link.linkedAt).toLocaleString("pt-BR")}</p></article>)}</div>}</section>

        <section id="urban-search" data-urban-section="search" className="urban-pipeline-link urban-pipeline-search" aria-labelledby="urban-search-profile-title"><div className="urban-pipeline-heading"><div><p className="urban-pipeline-eyebrow">02B · PERFIL DE BUSCA</p><h2 id="urban-search-profile-title">Registre preferências de busca, não condições de compra.</h2></div><p>O perfil exige um lead com interesse em busca e aceita somente tipos canônicos, horizonte e um código interno. Não aceita endereço, preço, crédito, financiamento ou qualquer dado pessoal.</p></div><div className="urban-pipeline-link__grid"><form className="urban-pipeline-card urban-pipeline-search__card" onSubmit={upsertLeadSearchProfile}><div className="urban-pipeline-card__title"><Search size={19} /><h3>Preferências em código</h3></div><p>Selecione tipos urbanos já definidos, indique a janela de busca e, se preciso, informe apenas um código interno de classificação.</p><label htmlFor="urban-search-lead">Lead de perfil de busca<select id="urban-search-lead" value={searchProfileLeadId} onChange={(event) => setSearchProfileLeadId(event.target.value)} disabled={!enabled || leadsQuery.isLoading} required><option value="">{!enabled ? "Defina um contexto autorizado" : leadsQuery.isLoading ? "Carregando leads autorizados" : leadsQuery.isError ? "Leitura de leads não liberada" : leadsQuery.data?.some((lead) => lead.interestKind === "search_profile") ? "Selecione um lead autorizado" : "Nenhum lead de perfil de busca neste contexto"}</option>{leadsQuery.data?.filter((lead) => lead.interestKind === "search_profile").map((lead) => <option key={lead.leadId} value={lead.leadId}>{urbanLeadSelectionLabel(lead)}</option>)}</select></label><fieldset disabled={!enabled}><legend>Tipos de ativo aceitos</legend><div className="urban-pipeline-search__options">{Object.entries(assetKindLabels).map(([kind, label]) => <label key={kind}><input type="checkbox" checked={acceptedAssetKinds.includes(kind as keyof typeof assetKindLabels)} onChange={() => toggleAssetKind(kind as keyof typeof assetKindLabels)} />{label}</label>)}</div></fieldset><label htmlFor="urban-search-timing">Horizonte declarado</label><select id="urban-search-timing" value={searchTiming} onChange={(event) => setSearchTiming(event.target.value as keyof typeof searchTimingLabels)} disabled={!enabled}>{Object.entries(searchTimingLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><label htmlFor="urban-preference-code">Código interno opcional</label><input id="urban-preference-code" value={preferenceCode} onChange={(event) => setPreferenceCode(event.target.value.toUpperCase())} placeholder="EX.: MORADIA_URBANA" disabled={!enabled} minLength={3} maxLength={80} /><button type="submit" disabled={!enabled || !searchProfileLeadId || searchProfileMutation.isPending}>{searchProfileMutation.isPending ? "Registrando perfil" : "Registrar perfil de busca"}</button></form><aside className="urban-pipeline-link__limits" aria-label="Limites do perfil de busca"><ShieldCheck size={19} /><div><h3>Sem decisão automática</h3><p>O perfil não recomenda imóvel, não calcula aderência, não pontua interessado e não substitui análise humana ou critérios de alçada.</p></div></aside></div>{!contextReady && <div className="urban-pipeline-empty"><CircleAlert size={18} /><p>Sem contexto não há consulta nem indicação de perfil de busca.</p></div>}{contextReady && !isAuthenticated && <div className="urban-pipeline-empty"><ShieldCheck size={18} /><p>O perfil e a leitura permanecem bloqueados até haver sessão autenticada.</p></div>}{enabled && leadSearchProfilesQuery.isLoading && <div className="urban-pipeline-empty"><span className="urban-pipeline-spinner" aria-hidden="true" /><p>Confirmando o contexto antes de solicitar a leitura minimizada dos perfis.</p></div>}{enabled && leadSearchProfilesQuery.isError && <div className="urban-pipeline-empty is-error"><CircleAlert size={18} /><p>A leitura dos perfis não foi liberada. Revise identidade, membership, grant, vigência e contexto sem tentar inferir leads externos.</p></div>}{enabled && leadSearchProfilesQuery.data?.length === 0 && <div className="urban-pipeline-empty"><Search size={18} /><p>Nenhum perfil de busca foi devolvido para este contexto. A resposta não revela outros leads ou preferências.</p></div>}{enabled && leadSearchProfilesQuery.data && leadSearchProfilesQuery.data.length > 0 && <div className="urban-pipeline-list__rows">{leadSearchProfilesQuery.data.map((profile) => <article key={profile.profileId}><span>Busca em rascunho</span><h3>{profile.acceptedAssetKinds.map((kind) => assetKindLabels[kind]).join(" · ")}</h3><p><b>{searchTimingLabels[profile.searchTiming]}</b> · {profile.preferencePresent ? "código interno informado" : "sem código interno"} · atualizado em {new Date(profile.updatedAt).toLocaleString("pt-BR")}</p></article>)}</div>}</section>

        <section id="urban-agenda" data-urban-section="agenda" className="urban-pipeline-link" aria-labelledby="urban-agenda-classification-title"><div className="urban-pipeline-heading"><div><p className="urban-pipeline-eyebrow">02C · AGENDA INTERNA</p><h2 id="urban-agenda-classification-title">Classifique a finalidade, não uma ação externa.</h2></div><p>A agenda só recebe uma finalidade interna enquanto ela e seu lead estiverem em rascunho. Não há convite, mensagem, confirmação de visita ou sincronização de calendário.</p></div><div className="urban-pipeline-link__grid"><form className="urban-pipeline-card" onSubmit={upsertAgendaClassification}><div className="urban-pipeline-card__title"><Tag size={19} /><h3>Finalidade interna</h3></div><p>Use uma agenda agendada ou remarcada já criada no mesmo contexto. O código opcional é estruturado e não aceita narrativa livre.</p><label htmlFor="urban-classification-agenda">Agenda interna autorizada<select id="urban-classification-agenda" value={classificationAgendaId} onChange={(event) => setClassificationAgendaId(event.target.value)} disabled={!enabled || agendasQuery.isLoading} required><option value="">{!enabled ? "Defina um contexto autorizado" : agendasQuery.isLoading ? "Carregando agendas autorizadas" : agendasQuery.isError ? "Leitura de agendas não liberada" : agendasQuery.data?.some((agenda) => ["scheduled", "rescheduled"].includes(agenda.state)) ? "Selecione uma agenda autorizada" : "Nenhuma agenda classificável neste contexto"}</option>{agendasQuery.data?.filter((agenda) => ["scheduled", "rescheduled"].includes(agenda.state)).map((agenda) => <option key={agenda.agendaId} value={agenda.agendaId}>{urbanAgendaSelectionLabel(agenda)}</option>)}</select></label><label htmlFor="urban-agenda-classification">Finalidade</label><select id="urban-agenda-classification" value={agendaClassification} onChange={(event) => setAgendaClassification(event.target.value as keyof typeof agendaClassificationLabels)} disabled={!enabled}>{Object.entries(agendaClassificationLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><label htmlFor="urban-agenda-internal-code">Código interno opcional</label><input id="urban-agenda-internal-code" value={agendaInternalCode} onChange={(event) => setAgendaInternalCode(event.target.value.toUpperCase())} placeholder="EX.: EM_REVISAO" disabled={!enabled} minLength={3} maxLength={80} /><button type="submit" disabled={!enabled || !classificationAgendaId || agendaClassificationMutation.isPending}>{agendaClassificationMutation.isPending ? "Classificando agenda" : "Registrar finalidade interna"}</button></form><aside className="urban-pipeline-link__limits" aria-label="Limites da classificação de agenda"><ShieldCheck size={19} /><div><h3>Sem confirmação externa</h3><p>A classificação não cria contato, não compromete participante, não altera data, não registra presença, não cria proposta, reserva, contrato ou efeito financeiro.</p></div></aside></div>{!contextReady && <div className="urban-pipeline-empty"><CircleAlert size={18} /><p>Sem contexto não há consulta nem indicação de classificação de agenda.</p></div>}{contextReady && !isAuthenticated && <div className="urban-pipeline-empty"><ShieldCheck size={18} /><p>A classificação e a leitura permanecem bloqueadas até haver sessão autenticada.</p></div>}{enabled && agendaClassificationsQuery.isLoading && <div className="urban-pipeline-empty"><span className="urban-pipeline-spinner" aria-hidden="true" /><p>Confirmando o contexto antes de solicitar a leitura minimizada das classificações.</p></div>}{enabled && agendaClassificationsQuery.isError && <div className="urban-pipeline-empty is-error"><CircleAlert size={18} /><p>A leitura das classificações não foi liberada. Revise identidade, membership, grant, vigência e contexto sem tentar inferir agendas externas.</p></div>}{enabled && agendaClassificationsQuery.data?.length === 0 && <div className="urban-pipeline-empty"><Tag size={18} /><p>Nenhuma classificação foi devolvida para este contexto. A resposta não revela outras agendas ou finalidades.</p></div>}{enabled && agendaClassificationsQuery.data && agendaClassificationsQuery.data.length > 0 && <div className="urban-pipeline-list__rows">{agendaClassificationsQuery.data.map((item) => <article key={item.classificationId}><span>Agenda interna</span><h3>{agendaClassificationLabels[item.classification]}</h3><p><b>{item.internalCodePresent ? "Código interno informado" : "Sem código interno"}</b> · atualizado em {new Date(item.updatedAt).toLocaleString("pt-BR")}</p></article>)}</div>}</section>

        <section data-urban-section="clients" className="urban-pipeline-list" aria-labelledby="urban-list-title"><div className="urban-pipeline-heading"><div><p className="urban-pipeline-eyebrow">03 · PRÓXIMOS PASSOS</p><h2 id="urban-list-title">Leads no contexto atual.</h2></div><p>A lista é autorizada pelo servidor e mostra somente o nome de exibição necessário, origem, interesse, etapa e próxima agenda.</p></div>{!contextReady && <div className="urban-pipeline-empty"><CircleAlert size={18} /><p>Defina contexto para solicitar a leitura. Sem isso, não há informação sobre existência de leads.</p></div>}{contextReady && leadsQuery.isLoading && <div className="urban-pipeline-empty"><span className="urban-pipeline-spinner" aria-hidden="true" /><p>Verificando contexto e preparando a leitura autorizada.</p></div>}{contextReady && leadsQuery.isError && <div className="urban-pipeline-empty is-error"><CircleAlert size={18} /><p>A leitura não foi liberada. Verifique identidade, membership, grant, vigência e finalidade sem tentar inferir outros registros.</p></div>}{contextReady && leadsQuery.data?.length === 0 && <div className="urban-pipeline-empty"><Workflow size={18} /><p>Nenhum lead em rascunho foi devolvido para este contexto. A resposta não revela qualquer outro funil.</p></div>}{contextReady && leadsQuery.data && leadsQuery.data.length > 0 && <div className="urban-pipeline-list__rows">{leadsQuery.data.map((lead) => <article key={lead.leadId}><span>{stageLabels[lead.stage]}</span><h3>{lead.partyLabel}</h3><p>Origem: {lead.sourceCode} · Interesse: {lead.interestKind} · {lead.nextAgendaFor ? `Próxima agenda: ${new Date(lead.nextAgendaFor).toLocaleString()}` : "Sem agenda aberta"}</p></article>)}</div>}</section>
      </div>
    </DashboardLayout>
  );
}
