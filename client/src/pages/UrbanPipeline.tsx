...
export default function UrbanPipeline() {
  ...
  const operationalSectors = [
    ...
    { code: "03", title: "Ativos Urbanos", value: urbanOperationalValue({ contextReady: enabled, loading: leadAssetLinksQuery.isLoading, count: leadAssetLinksQuery.data?.length, pendingLabel: "Aguardando leitura" }), description: "Vínculos internos não publicam, reservam ou ofertam ativos.", target: "#urban-assets" },
    { code: "04", title: "Perfil de Busca", value: urbanOperationalValue({ contextReady: enabled, loading: leadSearchProfilesQuery.isLoading, count: leadSearchProfilesQuery.data?.length, pendingLabel: "Aguardando leitura" }), description: "Preferências em código não fazem recomendação automática.", target: "#urban-search" },
  ];
  ...
  return (
    <DashboardLayout navigationItems={navigationItems} navigationTitle="Núcleo CRM" accessGate={urbanAccessGate}>
      <div className="urban-pipeline-page">
        ...
        <section className="urban-pipeline-overview" aria-labelledby="urban-overview-title"><div className="urban-pipeline-overview__heading"><div><p className="urban-pipeline-eyebrow">PAINEL VENDAS URBANAS · VISÃO DE TRABALHO</p><h2 id="urban-overview-title">Um funil organizado antes de qualquer compromisso comercial.</h2></div><p>Os indicadores usam somente leituras do contexto autorizado. Um estado vazio não revela atividade de outra organização nem pressupõe uma negociação.</p></div><nav className="urban-pipeline-overview__grid" aria-label="Setores operacionais de Vendas Urbanas">{operationalSectors.map((sector) => <a key={sector.code} href={sector.target}><span>{sector.code}</span><strong>{sector.title}</strong><b>{sector.value}</b><small>{sector.description}</small><em>Ver setor</em></a>)}</nav></section>
