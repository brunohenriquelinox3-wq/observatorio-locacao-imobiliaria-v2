...
export function SubdivisionInternalReceivableBatchPanel({ isReady, batches, isLoading, isError, alertScheduleState, managingAlertSchedule, onManageAlertSchedule }: Props) {
  ...
  return <section className="subdivision-finance-internal" aria-labelledby="subdivision-finance-title">
    ...
    <section className="subdivision-finance-internal__reminders" aria-labelledby="subdivision-finance-reminders-title">
      ...
      <div className="subdivision-finance-internal__reminder-actions" aria-label="Controle do lembrete diário interno">
        {alertScheduleState === "unbound" ? <button type="button" onClick={() => onManageAlertSchedule("activate")} disabled={!isReady || managingAlertSchedule}>{managingAlertSchedule ? "Ativando lembrete" : "Ativar lembrete diário interno"}</button> : <>{alertScheduleState === "active" ? <button type="button" onClick={() => onManageAlertSchedule("pause")} disabled={managingAlertSchedule}>Pausar lembrete interno</button> : <button type="button" onClick={() => onManageAlertSchedule("resume")} disabled={managingAlertSchedule}>Retomar lembrete interno</button>}<button type="button" onClick={() => onManageAlertSchedule("remove")} disabled={managingAlertSchedule}>Remover lembrete interno</button></>}
      </div>
