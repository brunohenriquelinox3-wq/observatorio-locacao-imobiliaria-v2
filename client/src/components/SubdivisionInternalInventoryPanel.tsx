...
export function SubdivisionInternalInventoryPanel({ context, developmentId, blocks, isWorkspaceReady }: {
  ...
  return <section className="subdivision-internal-inventory" id="subdivision-internal-inventory" aria-labelledby="internal-inventory-title">
    <header className="subdivision-internal-inventory__head">
      ...
      <div className="subdivision-internal-inventory__guard"><ShieldAlert size={17} /><span>Leitura e gravação sob MFA</span></div>
    </header>
    ...
    {profilesQuery.isError ? <p className="subdivision-internal-inventory__notice is-warning">A leitura de perfil interno exige sessão com MFA recente. A matriz física permanece inalterada.</p> : <>
    ...
    <form className="subdivision-internal-inventory__form" onSubmit={(event) => { event.preventDefault(); if (!blockId || !lotNumber) return; upsertProfileMutation.mutate({ ...input, blockId, lotNumber: Number(lotNumber), inventoryClassification, reviewState, mapLegend, internalNote, correlationId: crypto.randomUUID() }); }}>
      <div className="subdivision-internal-inventory__form-head"><div><span>FICHA INTERNA DO ESTOQUE</span><b>{profileReadUnavailable ? "Confirme MFA recente para liberar a ficha" : selectedBlock && lotNumber ? `Q${selectedBlock.blockNumber} · L${lotNumber}` : "Selecione uma unidade no mapa"}</b></div><small>Sem efeito comercial</small></div>
      ...
      <button type="submit" disabled={profileControlsDisabled || !blockId || !lotNumber}>{upsertProfileMutation.isPending ? "Registrando perfil" : profileReadUnavailable ? "MFA necessária para salvar" : "Salvar perfil interno"}</button>
    </form>
  </section>;
}
