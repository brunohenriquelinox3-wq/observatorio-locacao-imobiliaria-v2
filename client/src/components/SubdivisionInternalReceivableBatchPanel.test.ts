...
describe("SubdivisionInternalReceivableBatchPanel", () => {
  ...
  it("separa projeção interna de pagamento, repasse e confirmação de recebimento", () => { expect(source).toContain("Projeção interna"); expect(source).toContain("Saldo sem participação"); expect(source).toContain("não há pagamento, ordem de repasse, split bancário, baixa ou confirmação de recebimento"); });
});
