...
export function SubdivisionDevelopmentStudio({ context, isContextReady, isWorkspaceReady }: DevelopmentStudioProps) {
  ...
  const physicalCoverage = [
    ...
    { key: "position", label: "Posição", available: physicalLots.filter((lot) => Boolean(lot.positionCode)).length },
    { key: "typology", label: "Tipologia", available: physicalLots.filter((lot) => Boolean(lot.lotTypology)).length },
  ];
