...
import { formatPriceConditionLabel, formatPricePolicyLabel } from "@/lib/subdivisionOperationalReference";
...
import "../subdivision-lot-price-stability-a263.css";
...
const blockTypologyLabels: Record<string, string> = {
  regular: "Regular",
  mixed: "Mista",
  irregular: "Irregular",
  other: "Outra",
...
function createAutomaticTechnicalReference(prefix: "PC_AJUSTE" | "PB_CORRECAO") {
...
export function SubdivisionDevelopmentStudio({ context, isContextReady, isWorkspaceReady }: DevelopmentStudioProps) {
  ...
  const [lotReservationFilter, setLotReservationFilter] = useState<"all" | "reserved" | "unreserved">("all");
  ...
  const [priceBaseSourcePreview, setPriceBaseSourcePreview] = useState<{ sourceRows: number; importableLineCount: number; exceptionCounts: Record<string, number>; exceptionRows: Array<{ sourceRow: number; code: string }>; ignoredColumns: string[]; auxiliaryFormulaCount: number; reconciledLineCount: number; unreconciledLineCount: number; matrixAreaResolvedLineCount: number } | null>(null);
  ...
  const [manualPriceCorrectionDraft, setManualPriceCorrectionDraft] = useState({
    sourcePolicyId: "",
    versionReference: createAutomaticTechnicalReference("PB_CORRECAO"),
    effectiveFrom: "",
    sourceRow: "",
    blockNumber: "",
    lotNumber: "",
  ...
  const [priceEvidenceDraft, setPriceEvidenceDraft] = useState({
    subjectKind: "price_base_policy" as "price_base_policy" | "price_condition",
    subjectId: "",
    attachmentId: "",
  });
  const [withdrawPriceBaseReason, setWithdrawPriceBaseReason] = useState<"source_superseded" | "governance_review" | "effective_date_reassessment" | "documentary_reconciliation">("governance_review");
  const [priceConditionDraft, setPriceConditionDraft] = useState({
    basePolicyId: "",
    conditionReference: createAutomaticTechnicalReference("PC_AJUSTE"),
  ...
  const createLotConditionReference = (lot: (typeof physicalLots)[number]) => `PC_LOTE_Q${lot.blockNumber}_L${lot.lotNumber}_${crypto.randomUUID().slice(0, 8).toUpperCase()}`;
  const searchedLots = useMemo(() => {
    ...
    return physicalLots.filter((lot) => {
      ...
      const hasPhysicalPending = typeof lot.areaSqm !== "number" || typeof lot.frontageM !== "number" || typeof lot.depthM !== "number" || typeof lot.rearM !== "number" || typeof lot.leftSideM !== "number" || typeof lot.rightSideM !== "number" || lot.positionCode === "not_declared" || lot.lotTypology === "standard";
      ...
      const matchesReservation = lotReservationFilter === "all" || (lotReservationFilter === "reserved" ? lot.reservationPurpose !== null : lot.reservationPurpose === null);
      return matchesBlock && matchesSearch && matchesPhysicalStatus && matchesReservation;
    });
  }, [lotSearch, lotBlockFilter, lotPhysicalStatusFilter, lotReservationFilter, physicalLots]);
  const visibleLotBlocks = useMemo(() => {
    ...
    return groups.sort((left, right) => left.blockNumber - right.blockNumber).map((group) => ({
      ...
      lotsWithPhysicalPending: group.lots.filter((lot) => typeof lot.areaSqm !== "number" || typeof lot.frontageM !== "number" || typeof lot.depthM !== "number" || typeof lot.rearM !== "number" || typeof lot.leftSideM !== "number" || typeof lot.rightSideM !== "number" || lot.positionCode === "not_declared" || lot.lotTypology === "standard").length,
    }));
  }, [searchedLots]);
  ...
  const physicalCoverage = [
    ...
    { key: "rear", label: "Fundos", available: physicalLots.filter((lot) => typeof lot.rearM === "number").length },
    { key: "left-side", label: "Lateral esquerda", available: physicalLots.filter((lot) => typeof lot.leftSideM === "number").length },
    { key: "right-side", label: "Lateral direita", available: physicalLots.filter((lot) => typeof lot.rightSideM === "number").length },
    { key: "depth", label: "Profundidade auxiliar", available: physicalLots.filter((lot) => typeof lot.depthM === "number").length },
  ...
  const selectedOperationalLotInternalReference = useMemo(() => selectedOperationalLot ? internalLotPriceByKey.get(`${selectedOperationalLot.blockId}:${selectedOperationalLot.lotNumber}`) ?? null : null, [internalLotPriceByKey, selectedOperationalLot]);
  const priceConditionTargetLot = useMemo(() => focusedLotPriceTarget ? physicalLots.find((lot) => lot.blockId === focusedLotPriceTarget.blockId && lot.lotNumber === focusedLotPriceTarget.lotNumber) ?? null : null, [focusedLotPriceTarget, physicalLots]);
  const priceConditionPreviewTotal = priceConditionTargetLot && typeof priceConditionTargetLot.areaSqm === "number" && priceConditionDraft.adjustmentKind === "override_per_sqm" && Number.isFinite(Number(priceConditionDraft.amount)) && Number(priceConditionDraft.amount) > 0
    ? priceConditionTargetLot.areaSqm * Number(priceConditionDraft.amount)
    : null;
  ...
  useEffect(() => {
    ...
    setPriceConditionDraft((current) => {
      const alreadySelected = current.scope === "lot"
        && current.basePolicyId === selectedOperationalLotInternalReference.policyId
        && current.blockId === selectedOperationalLot.blockId
        && current.lotNumber === String(selectedOperationalLot.lotNumber);
      if (alreadySelected) return current;
      return {
        ...current,
        basePolicyId: selectedOperationalLotInternalReference.policyId,
        conditionReference: createLotConditionReference(selectedOperationalLot),
        scope: "lot",
  ...
  }, [selectedOperationalLot, selectedOperationalLotInternalReference]);

  function refreshInternalLotPriceReferences() {
    void internalLotPriceReferencesQuery.refetch();
  }
  ...
  const upsertPhysicalReservationMutation = trpc.subdivisionFoundation.upsertDraftLotPhysicalReservation.useMutation({
    onSuccess() {
      toast.success("Finalidade física registrada", { description: "A reserva foi classificada como finalidade interna; não criou disponibilidade, preço, venda, proposta, contrato ou financeiro." });
      setPhysicalReservationDraft((current) => ({ ...current, lotNumber: "" }));
      void utils.subdivisionFoundation.listDraftPhysicalStructure.invalidate(structureInput);
    },
    onError() {
      toast.error("Reserva física não registrada", { description: "O servidor exige MFA recente, contexto autorizado e o Lote selecionado dentro deste loteamento." });
    },
  });
  const upsertOperationalLotMutation = trpc.subdivisionFoundation.upsertDraftLotOperationalProfile.useMutation({
    onSuccess() {
      toast.success("Ficha operacional atualizada", { description: "Atributos físicos e observação interna foram atualizados sem abrir disponibilidade, preço, venda, contrato ou financeiro." });
      void utils.subdivisionFoundation.listDraftPhysicalStructure.invalidate(structureInput);
    },
    onError() {
      toast.error("Ficha operacional não atualizada", { description: "Revise os atributos permitidos, a observação interna, MFA e o contexto autorizado." });
    },
  });
  const upsertOperationalBlockMutation = trpc.subdivisionFoundation.upsertDraftBlockOperationalProfile.useMutation({
    onSuccess() { toast.success("Ficha da Quadra atualizada", { description: "Os atributos físicos e a observação interna foram atualizados sem abrir preço, venda, contrato ou financeiro." }); void utils.subdivisionFoundation.listDraftPhysicalStructure.invalidate(structureInput); },
    onError() { toast.error("Ficha da Quadra não atualizada", { description: "Revise os atributos permitidos, MFA e o contexto autorizado." }); },
  });
  ...
  const approvePriceBasePolicyMutation = trpc.subdivisionFoundation.approvePriceBasePolicy.useMutation({
    ...
    onError(error) {
      const evidenceRequired = error.message.includes("PRICE_BASE_POLICY_EVIDENCE_REQUIRED");
      toast.error("Aprovação bloqueada", { description: evidenceRequired ? "Vincule ou restaure ao menos um respaldo privado ativo antes de aprovar. O conteúdo do anexo permanece protegido." : "A aprovação exige pessoa distinta do preparador, MFA recente, alçada e vigência sem sobreposição." });
    },
  });
  const withdrawPriceBasePolicyMutation = trpc.subdivisionFoundation.withdrawPriceBasePolicy.useMutation({
    onSuccess() {
      toast.success("Política retirada", { description: "A retirada preserva o histórico e não altera matriz, condição, venda, contrato ou financeiro." });
      void utils.subdivisionFoundation.listPriceBasePolicies.invalidate();
      void utils.subdivisionFoundation.getLotPriceContext.invalidate();
    },
    onError() {
      toast.error("Retirada bloqueada", { description: "Somente política encaminhada e ainda não aprovada pode ser retirada, com MFA e alçada válidos." });
    },
  });
  const createPriceConditionMutation = trpc.subdivisionFoundation.createPriceCondition.useMutation({
    onSuccess() {
      ...
      setPriceConditionDraft((current) => ({ ...current, conditionReference: createAutomaticTechnicalReference("PC_AJUSTE"), amount: "", effectiveFrom: "", effectiveUntil: "", documentState: "pending_evidence" }));
  ...
  const submitPriceConditionMutation = trpc.subdivisionFoundation.submitPriceCondition.useMutation({
    ...
    onError(error) {
      const evidenceRequired = error.message.includes("PRICE_CONDITION_EVIDENCE_REQUIRED");
      toast.error("Encaminhamento bloqueado", { description: evidenceRequired ? "Vincule ao menos um respaldo privado registrado antes de encaminhar. O conteúdo do anexo permanece protegido." : "Respaldo declarado completo, política-base aprovada sem exceções, MFA e alçada são obrigatórios." });
    },
  });
  ...
  const linkPriceEvidenceMutation = trpc.subdivisionFoundation.linkPriceEvidence.useMutation({
    onSuccess() {
      toast.success("Respaldo vinculado", { description: "O vínculo privado foi registrado sem expor conteúdo ou alterar preço, aprovação, venda ou contrato." });
      setPriceEvidenceDraft((current) => ({ ...current, attachmentId: "" }));
      void utils.subdivisionFoundation.listPriceEvidenceSummary.invalidate();
    },
    onError() {
      toast.error("Vínculo bloqueado", { description: "Confirme anexo privado registrado, objeto do cadastro selecionado, MFA recente e contexto autorizado." });
    },
  });
  ...
  function linkPriceEvidence(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!selectedDevelopmentId || !priceEvidenceDraft.subjectId || !priceEvidenceDraft.attachmentId) return;
    linkPriceEvidenceMutation.mutate({
      ...context,
      developmentId: selectedDevelopmentId,
      subjectKind: priceEvidenceDraft.subjectKind,
      subjectId: priceEvidenceDraft.subjectId,
      attachmentId: priceEvidenceDraft.attachmentId,
      correlationId: crypto.randomUUID(),
    });
  }
  ...
  function selectLotForPriceAdjustment(lot: (typeof physicalLots)[number], policyId: string) {
    ...
    setPriceConditionDraft((current) => ({
      ...
      conditionReference: createLotConditionReference(lot),
      scope: "lot",
      blockId: lot.blockId,
      lotNumber: String(lot.lotNumber),
      adjustmentKind: "override_per_sqm",
    ...
    window.requestAnimationFrame(() => {
      document.getElementById("inline-lot-price-editor-contextual")?.scrollIntoView({ behavior: "smooth", block: "center" });
      window.setTimeout(() => priceConditionAmountInputRef.current?.focus({ preventScroll: true }), 80);
    });
  }
  ...
  function applyPhysicalSource() {
    ...
    applyPhysicalStructureMutation.mutate({
      ...
      blocks: physicalSourcePreview.blocks.map((block) => ({
        ...
        lots: block.lots.map((lot) => ({ lotNumber: lot.lotNumber, areaSqm: lot.areaSqm, frontageM: null, depthM: null, rearM: null, leftSideM: null, rightSideM: null, lotTypology: "standard" as const, positionCode: "not_declared" as const })),
      })),
    ...
    <section id="subdivision-studio" className="subdivision-studio" aria-labelledby="subdivision-studio-title">
      ...
      <div id="subdivision-development-workspace" className="subdivision-studio__workbench" data-view={studioView}>
        <aside className="subdivision-studio__records" aria-label="Cadastros de loteamentos em estruturação">
          ...
          <label className="subdivision-studio__record-search"><Search size={15} /><span className="sr-only">Buscar cadastro de loteamento</span><input value={recordFilter} onChange={(event) => setRecordFilter(event.target.value)} placeholder="Buscar loteamento" disabled={!isWorkspaceReady} /></label>
          ...
          <div className="subdivision-studio__record-list">
            ...
            {filteredDevelopments.map((development) => <button type="button" key={development.developmentId} onClick={() => selectDevelopment(development.developmentId)} data-active={development.developmentId === selectedDevelopmentId}>
              <span>{development.displayName || "Loteamento autorizado"}</span>
              <small><b>Cadastro</b>Identificação interna preservada</small>
              <em><b>Fase</b>{phaseLabels[development.workingPhase]}<i>Abrir cadastro →</i></em>
            </button>)}
          </div>
          <div className="subdivision-studio__records-foot"><ShieldCheck size={15} /><span>A lista revela somente cadastros autorizados do contexto atual.</span></div>
        </aside>
          ...
          <section className="subdivision-studio__active-module" aria-labelledby={`studio-module-${activeModule}`}>
            ...
            {activeModule === "structure" && <div className="subdivision-studio__structure-module">
              ...
              {selectedDevelopmentId && physicalLots.length > 0 && <section className="subdivision-lot-management" aria-labelledby="lot-management-title">
                <div className="subdivision-lot-management__head"><div><span>GESTÃO FÍSICA POR UNIDADE</span><h5 id="lot-management-title">Matriz detalhada por Quadra e Lote.</h5><p>Use os filtros para localizar um Lote. Cada cartão reúne a ficha física, o preço-base interno por m² e o valor total calculado pela área confirmada. Ajustes seguem para condição governada e nunca criam disponibilidade, venda, contrato ou financeiro.</p></div><div className="subdivision-lot-management__head-mark"><Ruler size={20} /><span>Leitura física e preço interno</span></div></div>
                <div className="subdivision-lot-management__price-read-status" data-state={internalPriceReferenceReadState} aria-live="polite"><div className="subdivision-lot-management__price-read-copy"><span>LEITURA INTERNA DE VALORES</span><b>{internalPriceReferenceReadState === "available" ? "Valores protegidos disponíveis" : internalPriceReferenceReadState === "loading" ? "Atualizando valores protegidos" : internalPriceReferenceReadState === "mfa_required" ? "Confirmação de MFA necessária" : "Consulta interna indisponível"}</b><small>{internalPriceReferenceReadState === "available" ? "Preço-base por m² e total referencial são lidos sob autorização; não criam disponibilidade comercial." : internalPriceReferenceReadState === "loading" ? "A estrutura do cartão permanece estável enquanto a leitura autorizada é atualizada." : internalPriceReferenceReadState === "mfa_required" ? "Confirme a sessão em Segurança e MFA e atualize a leitura. Nenhum preço ou cálculo foi alterado." : "Atualize a leitura para consultar novamente. Nenhum preço, área ou cálculo foi removido."}</small></div><div className="subdivision-lot-management__price-read-actions">{internalPriceReferenceReadState === "mfa_required" && <a className="subdivision-lot-management__renew-mfa" href="/seguranca-mfa"><ShieldCheck size={14} aria-hidden="true" /><span>Confirmar MFA</span></a>}<button type="button" className="subdivision-lot-management__refresh-price" disabled={!isWorkspaceReady || internalLotPriceReferencesQuery.isFetching} onClick={refreshInternalLotPriceReferences}><RefreshCw size={14} aria-hidden="true" /><span>{internalLotPriceReferencesQuery.isFetching ? "Atualizando" : "Atualizar valores"}</span></button></div></div>
                ...
                <div className="subdivision-lot-management__reservation-filter"><label><span>Finalidade física</span><select value={lotReservationFilter} onChange={(event) => setLotReservationFilter(event.target.value as typeof lotReservationFilter)}><option value="all">Todas as finalidades</option><option value="reserved">Somente reservas físicas</option><option value="unreserved">Sem reserva física</option></select></label><small>Este filtro organiza a leitura da matriz; não cria disponibilidade, preço ou venda.</small></div>
                ...
                <section className="subdivision-lot-management__physical-reservation" aria-labelledby="physical-reservation-title"><div><span>FINALIDADE FÍSICA RESERVADA</span><h6 id="physical-reservation-title">Classifique um Lote reservado sem abrir disponibilidade comercial.</h6><p>Use para reserva patrimonial de proprietários da área de origem ou infraestrutura técnica, como poço artesiano e caixa d’água. A seleção não gera preço, venda, proposta, contrato ou financeiro.</p></div><div className="subdivision-lot-management__physical-reservation-controls"><label>Quadra<select value={physicalReservationDraft.blockId} onChange={(event) => setPhysicalReservationDraft((current) => ({ ...current, blockId: event.target.value, lotNumber: "" }))}><option value="">Selecione a Quadra</option>{(physicalStructureQuery.data ?? []).map((block) => <option key={block.blockId} value={block.blockId}>Q{block.blockNumber}</option>)}</select></label><label>Lote<select value={physicalReservationDraft.lotNumber} onChange={(event) => setPhysicalReservationDraft((current) => ({ ...current, lotNumber: event.target.value }))} disabled={!physicalReservationDraft.blockId}><option value="">Selecione o Lote</option>{physicalReservationLots.map((lot) => <option key={lot.lotNumber} value={String(lot.lotNumber)}>L{lot.lotNumber}</option>)}</select></label><label>Finalidade<select value={physicalReservationDraft.reservationPurpose} onChange={(event) => setPhysicalReservationDraft((current) => ({ ...current, reservationPurpose: event.target.value as typeof current.reservationPurpose }))}><option value="landowner_reserve">Reserva de proprietários da área de origem</option><option value="technical_artesian_well">Infraestrutura · poço artesiano</option><option value="technical_water_tank">Infraestrutura · caixa d’água</option><option value="technical_other">Outra infraestrutura técnica</option></select></label><button type="button" disabled={!isWorkspaceReady || !physicalReservationDraft.blockId || !physicalReservationDraft.lotNumber || upsertPhysicalReservationMutation.isPending} onClick={() => upsertPhysicalReservationMutation.mutate({ ...context, developmentId: selectedDevelopmentId, blockId: physicalReservationDraft.blockId, lotNumber: Number(physicalReservationDraft.lotNumber), reservationPurpose: physicalReservationDraft.reservationPurpose, correlationId: crypto.randomUUID() })}>{upsertPhysicalReservationMutation.isPending ? "Registrando finalidade" : "Registrar finalidade física"}</button></div><small>{physicalLots.filter((lot) => lot.reservationPurpose === "landowner_reserve").length} reserva(s) patrimonial(is) · {physicalLots.filter((lot) => lot.reservationPurpose?.startsWith("technical_")).length} reserva(s) técnica(s) registradas.</small></section>
                <div id="physical-lot-cards" className="subdivision-lot-management__list" tabIndex={-1}>{visibleLotBlocks.length === 0 ? <p className="subdivision-lot-management__empty">{emptyLotFilterMessage}</p> : visibleLotBlocks.map((block, index) => <details className="subdivision-lot-management__block" key={block.blockNumber} open={lotBlockFilter !== "all" || Boolean(lotSearch.trim()) || index === 0}><summary><div><span>QUADRA</span><h6>Q{block.blockNumber}</h6></div><dl><div><dt>Lotes exibidos</dt><dd>{block.lots.length}</dd></div><div><dt>Área física</dt><dd>{block.totalAreaSqm.toLocaleString("pt-BR", { maximumFractionDigits: 2 })} m²</dd></div><div><dt>Lotes com pendência</dt><dd>{block.lotsWithPhysicalPending}</dd></div></dl><span className="subdivision-lot-management__toggle">Ver Lotes</span></summary><div className="subdivision-lot-management__lot-grid">{block.lots.map((lot) => { const isFocused = focusedLotPriceTarget?.blockId === lot.blockId && focusedLotPriceTarget.lotNumber === lot.lotNumber; const priceContext = isFocused ? focusedLotPriceContextQuery.data : undefined; const activePrice = priceContext?.state === "active" && typeof priceContext.effectivePricePerSqmBrl === "number" ? priceContext : null; const internalReference = internalLotPriceByKey.get(`${lot.blockId}:${lot.lotNumber}`); const hasInternalPrice = typeof internalReference?.basePricePerSqmBrl === "number" && typeof internalReference.lotTotalBrl === "number"; const missingInternalPrice = internalReference?.referenceState === "missing_base_price"; const pricePlaceholderLabel = internalPriceReferenceReadState === "loading" ? "Consultando" : internalPriceReferenceReadState === "mfa_required" ? "Protegido por MFA" : internalPriceReferenceReadState === "unavailable" ? "Consulta indisponível" : missingInternalPrice ? "Não informado" : "Sem referência"; const totalPlaceholderLabel = internalPriceReferenceReadState === "loading" ? "Aguardando leitura" : internalPriceReferenceReadState === "mfa_required" ? "Aguardando revalidação" : internalPriceReferenceReadState === "unavailable" ? "Atualize a leitura" : missingInternalPrice ? "Não calculado" : "Sem cálculo"; return <article className="subdivision-lot-management__lot" key={`${lot.blockNumber}-${lot.lotNumber}`} tabIndex={0} onMouseEnter={() => setFocusedLotPriceTarget({ blockId: lot.blockId, lotNumber: lot.lotNumber })} onMouseLeave={() => setFocusedLotPriceTarget(null)} onFocus={() => setFocusedLotPriceTarget({ blockId: lot.blockId, lotNumber: lot.lotNumber })} onBlur={() => setFocusedLotPriceTarget(null)}><div className="subdivision-lot-management__lot-head"><div><b>L{lot.lotNumber}</b><em>{lot.lotTypology === "standard" ? "Tipologia pendente" : lotTypologyLabels[lot.lotTypology] ?? "Não informada"}</em></div><button type="button" className="subdivision-lot-management__lot-edit" aria-label={`Editar ficha física da Q${lot.blockNumber}, L${lot.lotNumber}`} title="Editar ficha física deste Lote" onClick={() => selectLotForOperationalEdit(lot)}><PencilLine size={14} /><span>Editar</span></button></div>{lot.reservationPurpose && <span className="subdivision-lot-management__reservation-badge">{lot.reservationPurpose === "landowner_reserve" ? "Reserva patrimonial" : lot.reservationPurpose === "technical_artesian_well" ? "Infraestrutura · poço artesiano" : lot.reservationPurpose === "technical_water_tank" ? "Infraestrutura · caixa d’água" : "Infraestrutura técnica"}</span>}<dl className="subdivision-lot-management__lot-measures"><div><dt>Área física</dt><dd>{typeof lot.areaSqm === "number" ? `${lot.areaSqm.toLocaleString("pt-BR")} m²` : "Pendente"}</dd></div><div><dt>Posição</dt><dd>{lotPositionLabels[lot.positionCode] ?? "Não informada"}</dd></div><div><dt>Frente</dt><dd>{typeof lot.frontageM === "number" ? `${lot.frontageM.toLocaleString("pt-BR")} m` : "Pendente"}</dd></div><div><dt>Fundos</dt><dd>{typeof lot.rearM === "number" ? `${lot.rearM.toLocaleString("pt-BR")} m` : "Pendente"}</dd></div><div><dt>Lateral esquerda</dt><dd>{typeof lot.leftSideM === "number" ? `${lot.leftSideM.toLocaleString("pt-BR")} m` : "Pendente"}</dd></div><div><dt>Lateral direita</dt><dd>{typeof lot.rightSideM === "number" ? `${lot.rightSideM.toLocaleString("pt-BR")} m` : "Pendente"}</dd></div><div><dt>Profundidade auxiliar</dt><dd>{typeof lot.depthM === "number" ? `${lot.depthM.toLocaleString("pt-BR")} m` : "Pendente"}</dd></div></dl><aside className="subdivision-lot-management__price-context" aria-live="polite" data-active={Boolean(activePrice)} data-internal={internalReference?.referenceState ?? "none"} data-read-state={internalPriceReferenceReadState}><div className="subdivision-lot-management__price-context-head"><span>REFERÊNCIA DE PREÇO</span>{internalReference && <button type="button" onClick={() => selectLotForPriceAdjustment(lot, internalReference.policyId)} aria-label={`Editar referência interna por m² da Q${lot.blockNumber}, L${lot.lotNumber}`} title="Editar referência interna por m² deste Lote"><PencilLine size={13} /><span>Editar preço</span></button>}</div><div className="subdivision-lot-management__price-values">{hasInternalPrice ? <><div><span>{activePrice ? "PREÇO POR M² VIGENTE" : "PREÇO-BASE INTERNO POR M²"}</span><b>{(activePrice?.effectivePricePerSqmBrl ?? internalReference!.basePricePerSqmBrl!).toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</b></div><div><span>{activePrice ? "VALOR TOTAL VIGENTE" : "VALOR TOTAL REFERENCIAL"}</span><b>{(activePrice?.effectiveLotTotalBrl ?? internalReference!.lotTotalBrl!).toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</b></div></> : <><div data-placeholder="true"><span>PREÇO-BASE INTERNO POR M²</span><b>{pricePlaceholderLabel}</b></div><div data-placeholder="true"><span>VALOR TOTAL REFERENCIAL</span><b>{totalPlaceholderLabel}</b></div></>}</div>{hasInternalPrice ? <small>{activePrice ? "Referência aprovada e vigente." : internalReference!.referenceState === "internal_prepared" ? "Referência interna preparada. Não gera disponibilidade, venda ou preço contratual." : "Referência interna encaminhada. Aguarda aprovação por segunda pessoa."}</small> : internalPriceReferenceReadState === "mfa_required" ? <small>Confirme a sessão em Segurança e MFA e use Atualizar valores. O cálculo permanece preservado e não foi alterado.</small> : internalPriceReferenceReadState === "unavailable" ? <small>Não foi possível consultar a referência interna agora. Use Atualizar valores; nenhum preço, área ou cálculo foi removido.</small> : missingInternalPrice ? <small>Este Lote permanece sem valor até definição humana respaldada. Nenhum cálculo é estimado.</small> : <small>{isFocused ? lotPriceAvailabilityReasonLabels[(priceContext?.availabilityReason ?? "no_policy") as LotPriceAvailabilityReason] : "Sem referência interna preparada para este Lote."}</small>}</aside></article>; })}</div></details>)}</div>
              </section>}
              {selectedDevelopmentId && physicalStructureQuery.data && physicalStructureQuery.data.length > 0 && <SubdivisionInternalInventoryPanel context={context} developmentId={selectedDevelopmentId} blocks={physicalStructureQuery.data} isWorkspaceReady={isWorkspaceReady} />}
              {selectedDevelopmentId && physicalLots.length > 0 && <section ref={operationalLotProfileRef} className="subdivision-lot-management__physical-reservation subdivision-lot-management__operational-profile" aria-labelledby="operational-profile-title"><div><span>EDIÇÃO FÍSICA POR LOTE</span><h6 id="operational-profile-title">Selecione o Lote, confira as quatro divisas e salve a ficha física.</h6><p>{operationalLotDraft.blockId && operationalLotDraft.lotNumber ? `Você está revisando a unidade física selecionada. A alteração só será enviada após salvar e confirmar os requisitos de segurança.` : "Use o ícone de edição no cartão do Lote ou selecione Quadra e Lote abaixo. A área física é um atributo próprio: ela não é estimada pelas medidas."}</p></div><div className="subdivision-lot-management__profile-cues" aria-label="Como editar a ficha do Lote"><span><b>1</b> Localize a Quadra</span><span><b>2</b> Selecione o Lote</span><span><b>3</b> Revise somente dados físicos</span><span><b>4</b> Salve com MFA</span></div>{selectedOperationalLot && <aside className="subdivision-lot-management__edit-routing" aria-label="Caminhos de edição do Lote selecionado"><div><span>REFERÊNCIA INTERNA POR M²</span><b>Editar preço em jornada governada</b><small>Valor por m² é ajustado separadamente. O total referencial é recalculado pela área física e não é digitado.</small></div>{selectedOperationalLotInternalReference ? <button type="button" onClick={() => selectLotForPriceAdjustment(selectedOperationalLot, selectedOperationalLotInternalReference.policyId)}><PencilLine size={15} /><span>Editar valor interno por m²</span></button> : internalPriceReferenceReadState === "mfa_required" ? <a href="/seguranca-mfa"><ShieldCheck size={15} /><span>Confirmar MFA para consultar valor</span></a> : <small className="subdivision-lot-management__edit-routing-status">Nenhuma referência interna liberada para este Lote.</small>}</aside>}<div className="subdivision-lot-management__physical-reservation-controls"><label>Quadra<small>Local da unidade física</small><select value={operationalLotDraft.blockId} onChange={(event) => setOperationalLotDraft((current) => ({ ...current, blockId: event.target.value, lotNumber: "" }))}><option value="">Selecione a Quadra</option>{(physicalStructureQuery.data ?? []).map((block) => <option key={block.blockId} value={block.blockId}>Q{block.blockNumber}</option>)}</select></label><label>Lote<small>Unidade que será editada</small><select value={operationalLotDraft.lotNumber} disabled={!operationalLotDraft.blockId} onChange={(event) => { const lot = operationalLotOptions.find((candidate) => candidate.lotNumber === Number(event.target.value)); if (lot) selectLotForOperationalEdit(lot); }}><option value="">Selecione o Lote</option>{operationalLotOptions.map((lot) => <option key={lot.lotNumber} value={String(lot.lotNumber)}>L{lot.lotNumber}</option>)}</select></label><label>Área física (m²)<small>Não é calculada por esta ficha</small><input inputMode="decimal" value={operationalLotDraft.areaSqm} onChange={(event) => setOperationalLotDraft((current) => ({ ...current, areaSqm: event.target.value }))} /></label><label>Frente (m)<small>Divisa frontal</small><input inputMode="decimal" value={operationalLotDraft.frontageM} onChange={(event) => setOperationalLotDraft((current) => ({ ...current, frontageM: event.target.value }))} /></label><label>Fundos (m)<small>Divisa oposta à frente</small><input inputMode="decimal" value={operationalLotDraft.rearM} onChange={(event) => setOperationalLotDraft((current) => ({ ...current, rearM: event.target.value }))} /></label><label>Lateral esquerda (m)<small>Olhando da rua para o Lote</small><input inputMode="decimal" value={operationalLotDraft.leftSideM} onChange={(event) => setOperationalLotDraft((current) => ({ ...current, leftSideM: event.target.value }))} /></label><label>Lateral direita (m)<small>Olhando da rua para o Lote</small><input inputMode="decimal" value={operationalLotDraft.rightSideM} onChange={(event) => setOperationalLotDraft((current) => ({ ...current, rightSideM: event.target.value }))} /></label><label>Profundidade auxiliar (m)<small>Opcional · medida complementar</small><input inputMode="decimal" value={operationalLotDraft.depthM} onChange={(event) => setOperationalLotDraft((current) => ({ ...current, depthM: event.target.value }))} /></label><label>Tipologia<small>Formato físico do Lote</small><select value={operationalLotDraft.lotTypology} onChange={(event) => setOperationalLotDraft((current) => ({ ...current, lotTypology: event.target.value as typeof current.lotTypology }))}>{Object.entries(lotTypologyLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label>Posição<small>Localização na Quadra</small><select value={operationalLotDraft.positionCode} onChange={(event) => setOperationalLotDraft((current) => ({ ...current, positionCode: event.target.value as typeof current.positionCode }))}>{Object.entries(lotPositionLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label>Finalidade física<small>Não cria disponibilidade comercial</small><select value={operationalLotDraft.reservationPurpose} onChange={(event) => setOperationalLotDraft((current) => ({ ...current, reservationPurpose: event.target.value as typeof current.reservationPurpose }))}><option value="none">Sem reserva física</option><option value="landowner_reserve">Reserva patrimonial</option><option value="technical_artesian_well">Poço artesiano</option><option value="technical_water_tank">Caixa d’água</option><option value="technical_other">Outra infraestrutura</option></select></label><label className="subdivision-lot-management__profile-note">Observação interna<small>Sem pessoa, preço, contrato ou financeiro</small><textarea value={operationalLotDraft.internalNote} maxLength={280} placeholder="Ex.: acesso, interferência física ou ponto a revisar" onChange={(event) => setOperationalLotDraft((current) => ({ ...current, internalNote: event.target.value }))} /></label><button type="button" disabled={!isWorkspaceReady || !operationalLotDraft.blockId || !operationalLotDraft.lotNumber || upsertOperationalLotMutation.isPending} onClick={() => upsertOperationalLotMutation.mutate({ ...context, developmentId: selectedDevelopmentId, blockId: operationalLotDraft.blockId, lotNumber: Number(operationalLotDraft.lotNumber), areaSqm: operationalLotDraft.areaSqm ? Number(operationalLotDraft.areaSqm) : null, frontageM: operationalLotDraft.frontageM ? Number(operationalLotDraft.frontageM) : null, depthM: operationalLotDraft.depthM ? Number(operationalLotDraft.depthM) : null, rearM: operationalLotDraft.rearM ? Number(operationalLotDraft.rearM) : null, leftSideM: operationalLotDraft.leftSideM ? Number(operationalLotDraft.leftSideM) : null, rightSideM: operationalLotDraft.rightSideM ? Number(operationalLotDraft.rightSideM) : null, lotTypology: operationalLotDraft.lotTypology, positionCode: operationalLotDraft.positionCode, reservationPurpose: operationalLotDraft.reservationPurpose === "none" ? null : operationalLotDraft.reservationPurpose, internalNote: operationalLotDraft.internalNote.trim() || null, correlationId: crypto.randomUUID() })}>{upsertOperationalLotMutation.isPending ? "Salvando ficha" : "Salvar ficha física do Lote"}</button></div></section>}
              {selectedDevelopmentId && physicalLots.length > 0 && <section className="subdivision-lot-management__physical-reservation subdivision-lot-management__operational-profile" aria-labelledby="operational-block-title"><div><span>EDIÇÃO FÍSICA POR QUADRA</span><h6 id="operational-block-title">Selecione a Quadra e revise sua identificação operacional.</h6><p>Esta ficha organiza o conjunto de Lotes: referência setorial, tipologia e observação interna. Ela não altera as medidas individuais, a área da matriz, preço, disponibilidade, venda, contrato ou financeiro.</p></div><div className="subdivision-lot-management__profile-cues" aria-label="Como editar a ficha da Quadra"><span><b>1</b> Selecione a Quadra</span><span><b>2</b> Atualize a identificação física</span><span><b>3</b> Salve a revisão</span></div><div className="subdivision-lot-management__physical-reservation-controls"><label>Quadra<small>Conjunto físico a revisar</small><select value={operationalBlockDraft.blockId} onChange={(event) => { const block = (physicalStructureQuery.data ?? []).find((candidate) => candidate.blockId === event.target.value); setOperationalBlockDraft({ blockId: event.target.value, sectorReference: block?.sectorReference ?? "", blockTypology: block?.blockTypology ?? "regular", internalNote: block?.internalNote ?? "" }); }}><option value="">Selecione a Quadra</option>{(physicalStructureQuery.data ?? []).map((block) => <option key={block.blockId} value={block.blockId}>Q{block.blockNumber}</option>)}</select></label><label>Referência setorial<small>Apelido ou setor interno, se aplicável</small><input value={operationalBlockDraft.sectorReference} maxLength={80} placeholder="Ex.: setor de acesso ou etapa interna" onChange={(event) => setOperationalBlockDraft((current) => ({ ...current, sectorReference: event.target.value }))} /></label><label>Tipologia da Quadra<small>Formato predominante do conjunto</small><select value={operationalBlockDraft.blockTypology} onChange={(event) => setOperationalBlockDraft((current) => ({ ...current, blockTypology: event.target.value as typeof current.blockTypology }))}>{Object.entries(blockTypologyLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label className="subdivision-lot-management__profile-note">Observação interna<small>Sem pessoa, preço, contrato ou financeiro</small><textarea value={operationalBlockDraft.internalNote} maxLength={280} placeholder="Ex.: acesso, marco físico ou item técnico a revisar" onChange={(event) => setOperationalBlockDraft((current) => ({ ...current, internalNote: event.target.value }))} /></label><button type="button" disabled={!isWorkspaceReady || !operationalBlockDraft.blockId || upsertOperationalBlockMutation.isPending} onClick={() => upsertOperationalBlockMutation.mutate({ ...context, developmentId: selectedDevelopmentId, blockId: operationalBlockDraft.blockId, sectorReference: operationalBlockDraft.sectorReference.trim() || null, blockTypology: operationalBlockDraft.blockTypology, internalNote: operationalBlockDraft.internalNote.trim() || null, correlationId: crypto.randomUUID() })}>{upsertOperationalBlockMutation.isPending ? "Salvando ficha" : "Salvar ficha física da Quadra"}</button></div></section>}
              {selectedDevelopmentId && selectedOperationalLot && <>{selectedOperationalLotInternalReference ? <section id="inline-lot-price-editor" className="subdivision-lot-management__inline-price-editor" aria-labelledby="inline-lot-price-editor-title"><div className="subdivision-lot-management__inline-price-editor-head"><div><span>REFERÊNCIA INTERNA POR M²</span><h6 id="inline-lot-price-editor-title">Atualize o valor do Lote selecionado.</h6><p>Informe o novo valor por m² neste ponto de trabalho. O total é recalculado apenas para conferência e não pode ser digitado separadamente.</p></div><span className="subdivision-lot-management__inline-price-editor-lot">Q{selectedOperationalLot.blockNumber} · L{selectedOperationalLot.lotNumber}</span></div><form id="inline-lot-price-form" className="subdivision-lot-management__inline-price-editor-form" onSubmit={preparePriceCondition}><label>Valor por m² (BRL)<small>Referência interna sujeita a MFA e governança</small><input ref={priceConditionAmountInputRef} type="number" min="0.0001" step="0.0001" inputMode="decimal" value={priceConditionDraft.amount} onChange={(event) => setPriceConditionDraft((current) => ({ ...current, amount: event.target.value }))} placeholder="Informe o novo valor por m²" disabled={!isWorkspaceReady || isBusy} required /></label><label>Início da vigência<small>Obrigatório para preparar a atualização interna</small><input type="date" value={priceConditionDraft.effectiveFrom} onChange={(event) => setPriceConditionDraft((current) => ({ ...current, effectiveFrom: event.target.value }))} disabled={!isWorkspaceReady || isBusy} required /></label><label>Motivo interno<small>Justificativa operacional da atualização</small><select value={priceConditionDraft.reasonCode} onChange={(event) => setPriceConditionDraft((current) => ({ ...current, reasonCode: event.target.value as typeof current.reasonCode }))} disabled={!isWorkspaceReady || isBusy}><option value="internal_review">Revisão interna</option><option value="work_progress">Evolução de obras</option><option value="market_response">Resposta de mercado</option><option value="campaign">Campanha interna</option><option value="specific_condition">Condição específica</option><option value="other">Outro motivo interno</option></select></label><label>Referência da atualização<small>Identificação interna única da condição</small><input value={priceConditionDraft.conditionReference} onChange={(event) => setPriceConditionDraft((current) => ({ ...current, conditionReference: event.target.value }))} maxLength={75} disabled={!isWorkspaceReady || isBusy} required /></label><div className="subdivision-lot-management__inline-price-preview"><span>TOTAL REFERENCIAL PARA CONFERÊNCIA</span><b>{inlineLotPricePreviewTotal === null ? "Informe o valor por m²" : inlineLotPricePreviewTotal.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</b><small>Derivado exclusivamente da área física confirmada. Não é valor contratual, disponibilidade ou lançamento financeiro.</small></div></form><div className="subdivision-lot-management__inline-price-editor-actions"><p>Ao preparar, a atualização permanece interna e exige política, contexto, MFA, alçada, vigência, respaldo, correlação, idempotência e aprovação segregada antes de qualquer referência vigente.</p><button type="submit" form="inline-lot-price-form" disabled={!isWorkspaceReady || isBusy || !priceConditionCanPrepare}>{createPriceConditionMutation.isPending ? "Preparando atualização interna" : "Preparar atualização interna"}</button></div></section> : internalPriceReferenceReadState === "mfa_required" ? <section id="inline-lot-price-editor" className="subdivision-lot-management__inline-price-editor" aria-labelledby="inline-lot-price-editor-title"><div className="subdivision-lot-management__inline-price-editor-head"><div><span>REFERÊNCIA INTERNA POR M²</span><h6 id="inline-lot-price-editor-title">Confirme MFA para editar o valor por m².</h6><p>A ficha física permanece disponível; a referência interna só é carregada com sessão reforçada.</p></div></div><a className="subdivision-lot-management__renew-mfa" href="/seguranca-mfa"><ShieldCheck size={15} /><span>Confirmar MFA</span></a></section> : <section id="inline-lot-price-editor" className="subdivision-lot-management__inline-price-editor" aria-labelledby="inline-lot-price-editor-title"><div className="subdivision-lot-management__inline-price-editor-head"><div><span>REFERÊNCIA INTERNA POR M²</span><h6 id="inline-lot-price-editor-title">Referência interna ainda não disponível.</h6><p>Este Lote não recebe valor estimado. Atualize a leitura ou conclua a definição humana respaldada antes de preparar uma atualização.</p></div></div></section>}</>}
              {selectedDevelopmentId && selectedOperationalLot && <InlineLotPriceEditor host={operationalLotProfileRef.current} blockNumber={selectedOperationalLot.blockNumber} lotNumber={selectedOperationalLot.lotNumber} state={selectedOperationalLotInternalReference ? "available" : internalPriceReferenceReadState === "mfa_required" ? "mfa_required" : "unavailable"} draft={{ amount: priceConditionDraft.amount, effectiveFrom: priceConditionDraft.effectiveFrom, reasonCode: priceConditionDraft.reasonCode, conditionReference: priceConditionDraft.conditionReference }} onDraftChange={(patch) => setPriceConditionDraft((current) => ({ ...current, ...patch }))} amountInputRef={priceConditionAmountInputRef} previewTotal={inlineLotPricePreviewTotal} workspaceReady={isWorkspaceReady} busy={isBusy} canPrepare={priceConditionCanPrepare} preparing={createPriceConditionMutation.isPending} onPrepare={preparePriceCondition} />}
              ...
              {selectedDevelopmentId && <section className="subdivision-price-base-policy" aria-labelledby="price-base-policy-title">
                ...
                <details className="subdivision-price-evidence">
                  <summary><span>RESPALDO PRIVADO VINCULADO</span><b>Associar documento já registrado</b><small>O vínculo usa somente metadados opacos; não exibe nome, URL, chave ou conteúdo do arquivo.</small></summary>
                  <form onSubmit={linkPriceEvidence}>
                    <label>Objeto de preço<select value={priceEvidenceDraft.subjectKind} onChange={(event) => setPriceEvidenceDraft((current) => ({ ...current, subjectKind: event.target.value as "price_base_policy" | "price_condition", subjectId: "" }))} disabled={!isWorkspaceReady || isBusy}><option value="price_base_policy">Política de preço-base</option><option value="price_condition">Condição de preço</option></select></label>
                    <label>{priceEvidenceDraft.subjectKind === "price_base_policy" ? "Política" : "Condição"}<select value={priceEvidenceDraft.subjectId} onChange={(event) => setPriceEvidenceDraft((current) => ({ ...current, subjectId: event.target.value }))} disabled={!isWorkspaceReady || isBusy} required><option value="">Selecione o objeto</option>{priceEvidenceDraft.subjectKind === "price_base_policy" ? (priceBasePoliciesQuery.data ?? []).map((policy, index) => <option key={policy.policyId} value={policy.policyId}>{formatPricePolicyLabel(index)} · {priceConditionStateLabels[policy.state] ?? policy.state}</option>) : (priceConditionsQuery.data ?? []).map((condition, index) => <option key={condition.conditionId} value={condition.conditionId}>{formatPriceConditionLabel(index, condition.scope)} · {priceConditionStateLabels[condition.state] ?? condition.state}</option>)}</select></label>
                    <label>Anexo privado registrado<select value={priceEvidenceDraft.attachmentId} onChange={(event) => setPriceEvidenceDraft((current) => ({ ...current, attachmentId: event.target.value }))} disabled={!isWorkspaceReady || isBusy || recordedPrivateAttachments.length === 0} required><option value="">{recordedPrivateAttachments.length ? "Selecione o anexo" : "Nenhum anexo privado registrado"}</option>{recordedPrivateAttachments.map((attachment, index) => <option key={attachment.attachmentId} value={attachment.attachmentId}>Documento privado {String(index + 1).padStart(2, "0")} · {attachmentCategoryLabels[attachment.category]}</option>)}</select></label>
                    <p><ShieldCheck size={15} />O vínculo não declara aprovação documental, não publica o arquivo e não altera preço, vigência ou condição.</p>
                    <button type="submit" disabled={!isWorkspaceReady || isBusy || !priceEvidenceDraft.subjectId || !priceEvidenceDraft.attachmentId}>{linkPriceEvidenceMutation.isPending ? "Vinculando respaldo" : "Vincular respaldo privado"}</button>
                  </form>
                </details>
                ...
                <div className="subdivision-price-base-policy__history" aria-live="polite"><div><span>VERSÕES REGISTRADAS</span><h6>Histórico auditável de preparação e aprovação</h6></div>{priceBasePoliciesQuery.isLoading && <p><LoaderCircle className="subdivision-foundation-spinner" />Carregando políticas autorizadas.</p>}{!priceBasePoliciesQuery.isLoading && (priceBasePoliciesQuery.data ?? []).length === 0 && <p>Nenhuma política formal registrada neste cadastro.</p>}{(priceBasePoliciesQuery.data ?? []).map((policy, index) => { const evidenceCount = priceEvidenceCountFor("price_base_policy", policy.policyId); const policyReadyForSubmission = policy.exceptionCount === 0 && evidenceCount > 0; const policyReadyForApproval = policy.state === "submitted" && evidenceCount > 0; return <article key={policy.policyId} data-state={policy.state}><div><b>{formatPricePolicyLabel(index)}</b><span>Vigência inicial: {new Date(`${policy.effectiveFrom}T00:00:00`).toLocaleDateString("pt-BR")}</span></div><dl><div><dt>Linhas</dt><dd>{policy.lineCount}</dd></div><div><dt>Exceções</dt><dd>{policy.exceptionCount}</dd></div><div><dt>Respaldo privado</dt><dd>{evidenceCount}</dd></div><div><dt>Estado</dt><dd>{policy.state === "prepared" ? "Preparada" : policy.state === "submitted" ? "Encaminhada" : policy.state === "approved" ? "Aprovada" : policy.state === "expired" ? "Expirada" : "Retirada"}</dd></div></dl>{policy.state === "prepared" && !policyReadyForSubmission && <p className="subdivision-price-base-policy__submission-note"><ShieldAlert size={15} />{policy.exceptionCount > 0 ? "Resolva todas as exceções antes de encaminhar." : "Vincule ao menos um respaldo privado antes de encaminhar."}</p>}{policy.state === "submitted" && !policyReadyForApproval && <p className="subdivision-price-base-policy__submission-note"><ShieldAlert size={15} />Mantenha ao menos um respaldo privado ativo antes de aprovar.</p>}<div className="subdivision-price-base-policy__history-actions">{policy.state === "prepared" && <button type="button" className="subdivision-studio__secondary" onClick={() => submitPriceBasePolicyMutation.mutate({ ...context, policyId: policy.policyId, correlationId: crypto.randomUUID() })} disabled={!isWorkspaceReady || isBusy || !policyReadyForSubmission}>Encaminhar</button>}{policy.state === "submitted" && <><button type="button" onClick={() => approvePriceBasePolicyMutation.mutate({ ...context, policyId: policy.policyId, correlationId: crypto.randomUUID() })} disabled={!isWorkspaceReady || isBusy || !policyReadyForApproval}>Aprovar como segunda pessoa</button><label>Motivo da retirada<select value={withdrawPriceBaseReason} onChange={(event) => setWithdrawPriceBaseReason(event.target.value as typeof withdrawPriceBaseReason)} disabled={!isWorkspaceReady || isBusy}><option value="source_superseded">Fonte substituída</option><option value="governance_review">Revisão de governança</option><option value="effective_date_reassessment">Reavaliação de vigência</option><option value="documentary_reconciliation">Reconciliação documental</option></select></label><button type="button" className="subdivision-studio__secondary" onClick={() => withdrawPriceBasePolicyMutation.mutate({ ...context, policyId: policy.policyId, reasonCode: withdrawPriceBaseReason, correlationId: crypto.randomUUID() })} disabled={!isWorkspaceReady || isBusy}>Retirar encaminhamento</button></>}</div></article>; })}</div>
              </section>}
              ...
              {selectedDevelopmentId && <section className="subdivision-price-conditions" aria-labelledby="price-conditions-title">
                ...
                {priceConditionTargetLot && <section className="subdivision-price-conditions__lot-target" aria-label="Lote selecionado para ajuste interno"><div><span>AJUSTE DIRETO DO CARTÃO</span><b>Quadra {priceConditionTargetLot.blockNumber} · Lote {priceConditionTargetLot.lotNumber}</b><small>Altere somente o valor por m² nesta jornada. O total é recalculado pela área física confirmada.</small></div><dl><div><dt>Área física</dt><dd>{typeof priceConditionTargetLot.areaSqm === "number" ? `${priceConditionTargetLot.areaSqm.toLocaleString("pt-BR")} m²` : "Pendente"}</dd></div><div><dt>Novo total referencial</dt><dd>{priceConditionPreviewTotal === null ? "Informe o valor por m²" : priceConditionPreviewTotal.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</dd></div></dl></section>}
                ...
                <form className="subdivision-price-conditions__form" onSubmit={preparePriceCondition}>
                  <label>Política-base<select value={priceConditionDraft.basePolicyId} onChange={(event) => setPriceConditionDraft((current) => ({ ...current, basePolicyId: event.target.value }))} disabled={!isWorkspaceReady || isBusy} required><option value="">Selecione uma política preparada</option>{priceConditionBasePolicies.map((policy, index) => <option key={policy.policyId} value={policy.policyId}>{formatPricePolicyLabel(index)} · {priceConditionStateLabels[policy.state] ?? policy.state}</option>)}</select></label>
                  ...
                  <button type="submit" disabled={!isWorkspaceReady || isBusy || !priceConditionCanPrepare}>{createPriceConditionMutation.isPending ? "Preparando condição" : priceConditionTargetLot ? "Preparar ajuste interno deste Lote" : "Preparar condição"}</button>
                </form>
                ...
                <div className="subdivision-price-conditions__evidence"><span>{activeAttachmentCount} anexo(s) privado(s) no cadastro</span><button type="button" className="subdivision-studio__secondary" onClick={() => { setAttachmentCategory("other"); openModule("documents"); }} disabled={!isWorkspaceReady || isBusy}>Adicionar respaldo privado</button></div>
                <div className="subdivision-price-conditions__history" aria-live="polite"><div><span>CONDIÇÕES REGISTRADAS</span><h6>Histórico por escopo, vigência e estado.</h6></div>{priceConditionsQuery.isLoading && <p><LoaderCircle className="subdivision-foundation-spinner" />Carregando condições autorizadas.</p>}{!priceConditionsQuery.isLoading && (priceConditionsQuery.data ?? []).length === 0 && <p>Nenhuma condição registrada neste cadastro. Use os campos acima apenas quando houver decisão interna e referência documental a organizar.</p>}{(priceConditionsQuery.data ?? []).map((condition, index) => { const basePolicy = priceConditionBasePolicies.find((policy) => policy.policyId === condition.basePolicyId); const evidenceCount = priceEvidenceCountFor("price_condition", condition.conditionId); const canSubmit = condition.state === "prepared" && condition.documentState === "declared_complete" && basePolicy?.state === "approved" && basePolicy.exceptionCount === 0 && evidenceCount > 0; const canApprove = condition.state === "submitted" && evidenceCount > 0; return <article key={condition.conditionId} data-state={condition.state}><div><b>{formatPriceConditionLabel(index, condition.scope)}</b><span>{priceConditionScopeLabels[condition.scope]} · {priceConditionKindLabels[condition.adjustmentKind]}</span></div><dl><div><dt>Vigência</dt><dd>{new Date(`${condition.effectiveFrom}T00:00:00`).toLocaleDateString("pt-BR")}{condition.effectiveUntil ? ` — ${new Date(`${condition.effectiveUntil}T00:00:00`).toLocaleDateString("pt-BR")}` : ""}</dd></div><div><dt>Respaldo</dt><dd>{priceConditionDocumentStateLabels[condition.documentState]}</dd></div><div><dt>Vínculos privados</dt><dd>{evidenceCount} registrado(s)</dd></div><div><dt>Estado</dt><dd>{priceConditionStateLabels[condition.state] ?? condition.state}</dd></div></dl><div className="subdivision-price-conditions__actions">{condition.state === "prepared" && <><button type="button" className="subdivision-studio__secondary" onClick={() => submitPriceConditionMutation.mutate({ ...context, conditionId: condition.conditionId, correlationId: crypto.randomUUID() })} disabled={!isWorkspaceReady || isBusy || !canSubmit}>Encaminhar</button>{evidenceCount === 0 && <p className="subdivision-price-conditions__evidence-required">Vincule um respaldo privado antes de encaminhar.</p>}</>}{condition.state === "submitted" && <><button type="button" onClick={() => approvePriceConditionMutation.mutate({ ...context, conditionId: condition.conditionId, correlationId: crypto.randomUUID() })} disabled={!isWorkspaceReady || isBusy || !canApprove}>Aprovar como segunda pessoa</button>{!canApprove && <p className="subdivision-price-conditions__evidence-required">Mantenha um respaldo privado ativo antes de aprovar.</p>}</>}{(condition.state === "prepared" || condition.state === "submitted") && <><label>Motivo da retirada<select value={withdrawPriceConditionReason} onChange={(event) => setWithdrawPriceConditionReason(event.target.value as typeof withdrawPriceConditionReason)} disabled={!isWorkspaceReady || isBusy}><option value="source_superseded">Fonte substituída</option><option value="governance_review">Revisão de governança</option><option value="effective_date_reassessment">Reavaliação de vigência</option><option value="documentary_reconciliation">Reconciliação documental</option></select></label><button type="button" className="subdivision-studio__secondary" onClick={() => withdrawPriceConditionMutation.mutate({ ...context, conditionId: condition.conditionId, reasonCode: withdrawPriceConditionReason, correlationId: crypto.randomUUID() })} disabled={!isWorkspaceReady || isBusy}>Retirar</button></>}</div></article>; })}</div>
              </section>}
              ...
              {selectedDevelopmentId && <details className="subdivision-studio__matrix-editor" open={activeSavedStructure.length === 0}>
                ...
                {activeSavedStructure.length > 0 && <a className="subdivision-studio__inventory-link" href="/estoque-lotes"><MapPinned size={16} /><span>Abrir mapa interno de Loteamentos</span><small>Revise a matriz Qn · Ln criada neste contexto.</small></a>}
                </form>
              </details>}
            </div>}
        ...
        <aside className="subdivision-studio__overview" aria-label="Visão operacional do loteamento">
          ...
          <div className="subdivision-studio__overview-name"><strong>{mode === "create" ? form.displayName || "Novo loteamento" : selectedDevelopment?.displayName ?? "Loteamento selecionado"}</strong><span>Identificação interna preservada</span></div>
