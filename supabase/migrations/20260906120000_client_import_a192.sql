-- A192 — Importação controlada de clientes por CSV.
-- Importa somente nome de exibição, natureza e papel em rascunho; não aceita identificadores fiscais,
-- contato, endereço, documentos, contratos, valores, cobrança, pagamento ou integrações externas.
...
create table public.client_import_batches (
  ...
  received_row_count integer not null check (received_row_count between 1 and 200),
  created_row_count integer not null default 0 check (created_row_count between 0 and 200),
  duplicate_row_count integer not null default 0 check (duplicate_row_count between 0 and 200),
  ...
  constraint client_import_batches_counts_match check (created_row_count + duplicate_row_count = received_row_count),
  constraint client_import_batches_actor_correlation_unique unique (created_by, correlation_id)
);

create index client_import_batches_context_lookup
  on public.client_import_batches (organization_id, module, created_at desc);

alter table public.client_import_batches enable row level security;
revoke all on table public.client_import_batches from public, anon, authenticated;

create or replace function private.require_client_import_authority(
...
begin
  if not exists (
    ...
    where organization.id = p_organization_id
      ...
      and membership.role in ('organization_admin', 'area_admin')
    ...
    raise exception using errcode = '42501', message = 'CLIENT_IMPORT_CONTEXT_DENIED';
  end if;
end;
$$;
...
begin
  ...
  loop
    ...
    select party.id, role_assignment.id into v_party_id, v_role_id
    ...
    where party.organization_id = p_organization_id
      ...
      and lower(party.display_name) = lower(v_display_name)
...
revoke all on function private.require_client_import_authority(uuid, uuid, public.operating_module, text) from public, anon, authenticated;
...
comment on table public.client_import_batches is 'A192: metadados de lote de importação CSV, sem conteúdo, documento, identificador fiscal, contato, contrato ou financeiro.';
