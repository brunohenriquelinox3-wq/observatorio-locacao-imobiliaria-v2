-- A238: correção manual governada de preço-base. Não cria venda, proposta, contrato, cobrança, pagamento, repasse, receita ou integração externa.

create or replace function public.subdivision_prepare_manual_price_base_correction_v1(
...
begin
  ...
  where event.command_name = 'subdivision_prepare_manual_price_base_correction_v1'
  ...
  values (
    p_correlation_id, p_actor_user_id, p_organization_id, 'subdivision_prepare_manual_price_base_correction_v1', 'allowed',
    ...
    jsonb_build_object('module', p_module::text, 'purpose_code', trim(p_purpose_code), 'source_policy_present', true,
      'manual_correction', true, 'reason_code', p_reason_code, 'document_state', p_document_state,
      'source_row_declared', true, 'area_provenance', 'matrix_physical', 'result', v_result)
  );
  return v_result;
end; $$;

revoke all on function public.subdivision_prepare_manual_price_base_correction_v1(uuid, uuid, public.operating_module, text, uuid, uuid, text, date, integer, integer, integer, numeric, text, text, text, text, uuid) from public, anon, authenticated;
grant execute on function public.subdivision_prepare_manual_price_base_correction_v1(uuid, uuid, public.operating_module, text, uuid, uuid, text, date, integer, integer, integer, numeric, text, text, text, text, uuid) to service_role;
