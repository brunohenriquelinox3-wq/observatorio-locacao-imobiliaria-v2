-- A209: restaura somente Quadras e Lotes arquivados do mesmo rascunho autorizado; não cria Lotes, preço, estoque comercial, venda, contrato ou financeiro.
create or replace function public.subdivision_list_archived_draft_structure_v1(
  p_actor_user_id uuid, p_organization_id uuid, p_module public.operating_module, p_purpose_code text, p_development_id uuid
) returns table (block_id uuid, block_number integer, archived_lot_count integer)
...
begin
  ...
  return query select b.id, b.block_number, count(l.id)::integer from public.subdivision_blocks b left join public.subdivision_lots l on l.block_id = b.id and l.organization_id = p_organization_id and l.state = 'archived'::public.party_lifecycle_state where b.organization_id = p_organization_id and b.development_id = p_development_id and b.state = 'archived'::public.party_lifecycle_state group by b.id, b.block_number order by b.block_number asc;
end; $$;
...
begin
  ...
  insert into public.admin_audit_events (correlation_id, actor_user_id, organization_id, command_name, outcome, target_type, target_id, payload_redacted) values (p_correlation_id, p_actor_user_id, p_organization_id, 'subdivision_restore_draft_block_v1', 'allowed', 'subdivision_block', p_block_id, pg_catalog.jsonb_build_object('module', p_module::text, 'purpose_code', pg_catalog.trim(p_purpose_code), 'development_id_present', true, 'restored_lot_count', v_restored_lot_count, 'result', v_result));
  return v_result;
end; $$;

revoke all on function public.subdivision_list_archived_draft_structure_v1(uuid, uuid, public.operating_module, text, uuid) from public, anon, authenticated;
...
grant execute on function public.subdivision_list_archived_draft_structure_v1(uuid, uuid, public.operating_module, text, uuid) to service_role;
