-- A251: expõe área confirmada e total referencial somente no mesmo contexto em que a política e eventual condição já estejam aprovadas e vigentes.

create or replace function public.subdivision_get_lot_price_context_v4(p_actor_user_id uuid, p_organization_id uuid, p_module public.operating_module, p_purpose_code text, p_development_id uuid, p_block_id uuid, p_lot_number integer)
returns jsonb language plpgsql security definer set search_path = '' as $$
declare v_lot_id uuid; v_area numeric; v_base record; v_condition record; v_effective numeric; v_latest record;
begin
  perform private.require_active_subdivision_draft_authority(p_actor_user_id, p_organization_id, p_module, p_purpose_code);
  select l.id, l.area_sqm into v_lot_id, v_area from public.subdivision_lots l join public.subdivision_blocks b on b.id = l.block_id and b.organization_id = l.organization_id where l.organization_id = p_organization_id and l.block_id = p_block_id and l.lot_number = p_lot_number and b.development_id = p_development_id and l.state = 'draft'::public.party_lifecycle_state and b.state = 'draft'::public.party_lifecycle_state;
  ...
  return jsonb_build_object('state', 'active', 'availability_reason', null, 'policy_reference', v_base.version_reference, 'condition_reference', case when v_condition.id is null then null else v_condition.condition_reference end, 'condition_scope', case when v_condition.id is null then null else v_condition.scope::text end, 'condition_kind', case when v_condition.id is null then null else v_condition.adjustment_kind::text end, 'effective_from', case when v_condition.id is null then null else v_condition.effective_from end, 'effective_until', case when v_condition.id is null then null else v_condition.effective_until end, 'document_state', case when v_condition.id is null then null else v_condition.document_state::text end, 'effective_price_per_sqm_brl', v_effective, 'lot_area_sqm', case when v_area > 0 then v_area else null end, 'effective_lot_total_brl', case when v_area > 0 then v_area * v_effective else null end);
end; $$;

revoke all on function public.subdivision_get_lot_price_context_v4(uuid, uuid, public.operating_module, text, uuid, uuid, integer) from public, anon, authenticated;
grant execute on function public.subdivision_get_lot_price_context_v4(uuid, uuid, public.operating_module, text, uuid, uuid, integer) to service_role;
