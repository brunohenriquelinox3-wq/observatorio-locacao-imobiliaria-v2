-- A198 — Construtor de estrutura por loteamento: Quadras com quantidades variáveis de Lotes.
-- Não cria disponibilidade, mapa geográfico, reserva, venda, cliente, contrato, preço, cobrança ou financeiro.

create or replace function public.subdivision_list_draft_structure_v2(
...
returns table (block_id uuid, block_number integer, lot_count integer)
...
begin
  ...
  return query
  select block.id, block.block_number, count(lot.id)::integer
  from public.subdivision_blocks block
  left join public.subdivision_lots lot
    on lot.block_id = block.id
  ...
  group by block.id, block.block_number
...
begin
  ...
  if v_input_block_count <> pg_catalog.jsonb_array_length(p_blocks)
    or v_input_block_count < 1
    or v_unique_block_count <> v_input_block_count
    or v_requested_lot_count < 1
    or exists (
      select 1 from pg_catalog.jsonb_to_recordset(p_blocks) as candidate(block_number integer, lot_count integer)
      where candidate.block_number not between 1 and 999 or candidate.lot_count not between 1 and 100
    ) then
    raise exception using errcode = '22023', message = 'SUBDIVISION_STRUCTURE_INVALID';
  end if;
  ...
  where event.command_name = 'subdivision_apply_draft_structure_v2'
  ...
  if not p_replace_existing and exists (
    select 1
    from public.subdivision_blocks block
    join lateral pg_catalog.jsonb_to_recordset(p_blocks) as candidate(block_number integer, lot_count integer)
      on candidate.block_number = block.block_number
    where block.organization_id = p_organization_id
      and block.development_id = p_development_id
      and block.state = 'draft'::public.party_lifecycle_state
  ...
  for v_block in
    select candidate.block_number, candidate.lot_count
    from pg_catalog.jsonb_to_recordset(p_blocks) as candidate(block_number integer, lot_count integer)
    order by candidate.block_number asc
  loop
    ...
    on conflict (organization_id, development_id, block_number) do update
      set state = 'draft'::public.party_lifecycle_state, updated_at = pg_catalog.now()
    returning id into v_block_id;

    if p_replace_existing then
      with archived as (
        update public.subdivision_lots lot
        set state = 'archived'::public.party_lifecycle_state, updated_at = pg_catalog.now()
        where lot.organization_id = p_organization_id
          and lot.block_id = v_block_id
          and lot.state = 'draft'::public.party_lifecycle_state
          and lot.lot_number > v_block.lot_count
        returning 1
    ...
    select p_organization_id, v_block_id, series.lot_number, 'draft'::public.party_lifecycle_state, p_actor_user_id
    from pg_catalog.generate_series(1, v_block.lot_count) as series(lot_number)
    on conflict (organization_id, block_id, lot_number) do update
      set state = 'draft'::public.party_lifecycle_state, updated_at = pg_catalog.now();
  end loop;

  if p_replace_existing then
    with archived_lots as (
      update public.subdivision_lots lot
      set state = 'archived'::public.party_lifecycle_state, updated_at = pg_catalog.now()
      from public.subdivision_blocks block
      where block.id = lot.block_id
        ...
        and not exists (
          select 1 from pg_catalog.jsonb_to_recordset(p_blocks) as candidate(block_number integer, lot_count integer)
          where candidate.block_number = block.block_number
        )
      returning 1
    ) select v_archived_lot_count + count(*)::integer into v_archived_lot_count from archived_lots;

    with archived_blocks as (
      update public.subdivision_blocks block
      set state = 'archived'::public.party_lifecycle_state, updated_at = pg_catalog.now()
      where block.organization_id = p_organization_id
        and block.development_id = p_development_id
        and block.state = 'draft'::public.party_lifecycle_state
  ...
  join public.subdivision_blocks block on block.id = lot.block_id and block.organization_id = lot.organization_id
  where lot.organization_id = p_organization_id
    and block.development_id = p_development_id
    and block.state = 'draft'::public.party_lifecycle_state
    and lot.state = 'draft'::public.party_lifecycle_state;
  ...
  v_result := pg_catalog.jsonb_build_object(
    ...
    'archived_block_count', v_archived_block_count,
    'archived_lot_count', v_archived_lot_count
  );
  ...
  values (
    ...
    'subdivision_apply_draft_structure_v2', 'allowed', 'subdivision_development', p_development_id,
    pg_catalog.jsonb_build_object(
      'module', p_module::text,
      'purpose_code', pg_catalog.trim(p_purpose_code),
      'development_id_present', true,
      'input_block_count', v_input_block_count,
      'input_lot_count', v_requested_lot_count,
...
revoke all on function public.subdivision_list_draft_structure_v2(uuid, uuid, public.operating_module, text, uuid) from public, anon, authenticated;
...
grant execute on function public.subdivision_list_draft_structure_v2(uuid, uuid, public.operating_module, text, uuid) to service_role;
...
begin
  ...
  select event.payload_redacted -> 'result' into v_existing_result
  from public.admin_audit_events event
  where event.command_name = 'subdivision_archive_draft_block_v2'
    and event.correlation_id = p_correlation_id
    and event.actor_user_id = p_actor_user_id
    and event.organization_id = p_organization_id
    and event.outcome = 'allowed'
  ...
  values (
    p_correlation_id, p_actor_user_id, p_organization_id,
    'subdivision_archive_draft_block_v2', 'allowed', 'subdivision_block', p_block_id,
    pg_catalog.jsonb_build_object(
      'module', p_module::text,
      'purpose_code', pg_catalog.trim(p_purpose_code),
      'development_id_present', true,
...
comment on function public.subdivision_apply_draft_structure_v2(uuid, uuid, public.operating_module, text, uuid, jsonb, boolean, uuid)
  is 'A198: reconcilia Quadras e Lotes de rascunho por loteamento; remoção é lógica e requer confirmação explícita.';
