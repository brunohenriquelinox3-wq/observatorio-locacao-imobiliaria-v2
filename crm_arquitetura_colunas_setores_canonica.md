...
| --- | --- | --- |
| **Painel Loteadora** *(primeira tela interna)* | Alertas e indicadores executivos definidos de estoque, proposta/reserva, contratos, dossiê, carteira, direitos, repasses e restrições no escopo autorizado. | É a porta de entrada **dentro** de Loteadora, depois da seleção global SUPER ADM → ADM → LOTEADORA → VENDAS URBANAS → LOCAÇÃO; direciona ao setor/caso fonte e não altera fatos ou policy. |
...
| **Obras e Infraestrutura** *(módulo posterior)* | Futuramente: baseline, pacotes, medições, riscos, mudanças, compromissos e análises de obra. Na fundação: apenas marcos, evidências, restrições, owners e alertas do empreendimento/fase/lote. | Marco técnico pode contextualizar ou restringir elegibilidade conforme policy, mas não muda estoque, contrato, carteira, repasse ou entrega por si só. |
| **Relatórios contextuais** *(sem setor próprio)* | Leituras, filas e métricas em Estoque, Vendas/Contratos, Financeiro/Carteira, Repasses/Distribuição e Sócios/Parceiros; Painel Loteadora e ADM consolidam poucos indicadores executivos. | Toda métrica declara fórmula, escopo, estado, `as_of`, fonte/limitação e link à origem; filtro, drill-down e exportação não ampliam policy. |
