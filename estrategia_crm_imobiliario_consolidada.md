...
| Distribuição | Base, versão de plano, tipo de parcela/evento, percentual/fixo, faixa, recebedor individual ou grupo contratado, gatilho, bloqueio e alçada | Documento, pagamento, retenção, obrigação e análise da natureza econômica. |
...
### Propostas, Reservas e Contratos: jornada comercial única

Na Loteadora, **Reserva** é uma etapa de `Propostas, Reservas e Contratos`, e não um setor de menu separado na primeira versão. Proposta, reserva, contrato, situação comercial do lote, dossiê e carteira mantêm objetos e estados próprios, mas compartilham correlação e regras de transição. A reserva registra proposta, lote `Qn · Ln`, prazo, condição, owner, alçada, evidência e idempotência; somente uma reserva ativa incompatível pode sobreviver por lote/escopo comercial. A concorrência é resolvida por comando transacional e regra no dado, nunca pelo tempo da tela ou pelo clique mais rápido.

Ao vencer, a reserva entra em análise e reavalia contrato, assinatura, bloqueio, disputa, distrato/cessão e demais restrições antes de qualquer alteração de elegibilidade. Proposta não é reserva; reserva não é contrato; contrato não confirma pagamento; boleto e comprovante não confirmam caixa. A lente de Contratos Realizados por Lote usa esses fatos para alertar dossiê pendente, assinatura/versão, agenda/carteira e conflito lote × contrato, sem criar um estoque paralelo. Uma futura varredura de reservas vencidas só poderá ser ativada com job durável, idempotente, autenticado e aprovado. [42]

### Repasses e Distribuição: setor próprio conectado à carteira

**Repasses e Distribuição** é um setor próprio da Loteadora. Ele recebe eventos/controlos autorizados de Contrato, Financeiro e Carteira para formar projeções e direitos, mas não assume a emissão de boleto, cobrança, baixa, aplicação de caixa ou conciliação do comprador. Suas áreas internas são: visão de direitos; planos/versionamento; recebedores e grupos; direitos por evento; fila de alçadas e instruções; retorno/conciliação de repasse; exceções/reversões; e auditoria/prestação de contas.
...
### Navegação global e Painel Loteadora interno

A arquitetura global permanece, nesta ordem: **SUPER ADM → ADM → LOTEADORA → VENDAS URBANAS → LOCAÇÃO**. O **Painel Loteadora** não é uma coluna global e não vem antes de SUPER ADM ou ADM. Ele é a primeira tela interna depois que o usuário entra na coluna Loteadora, servindo como porta de entrada para alertas e indicadores executivos que levam aos setores fonte. O painel não substitui Cadastro de Loteamentos, Estoque/Mapa, Sócios e Parceiros, Clientes Loteadora, Propostas/Reservas/Contratos, Financeiro/Carteira ou Repasses/Distribuição; tampouco altera dados ou amplia escopo. [46]
...
| Situação de negócio | Estrutura de produto | Limite de segurança e governança |
| --- | --- | --- |
| Parceiro recebe entrada, parcela ou intermediária. | Plano de Direitos e Regra de Direito vinculados ao contrato, recebedor, lote/contrato elegível e tipo de evento. | Percentual/fixo não se torna instrução, pagamento ou liquidação por si só. |
| Dois ou mais participantes unem glebas/recebimentos. | Grupo de Participação com membros, objetos compartilhados, regra econômica e regra de visualização explicitamente contratadas. | O grupo não amplia acesso a objetos, contratos ou direitos individuais que não foram incluídos. |
| Parceiro acompanha seu negócio. | Portal mínimo de leitura e solicitação, ligado aos recortes autorizados de Contratos, Parcelas/Boletos, Cobranças/Carteira, Estoque/Mapa e Direitos/Distribuição. | Não permite baixar cobrança, alterar lote, regra, contrato, carteira, alçada ou acesso administrativo. |
| Investidor assume aporte parcelado. | Operação de Capital separa compromisso, cronograma, evento de caixa, condição, evidência, titularidade e eventual direito econômico posterior. | Aporte, caixa conciliado, participação societária e direito sobre receitas são fatos diferentes; nenhum é inferido automaticamente. |

O painel de parceiro é uma superfície de transparência controlada: ele pode mostrar contratos e lotes que compõem o vínculo individual ou grupo contratado, bem como clientes adimplentes/inadimplentes cuja carteira gera o direito econômico autorizado. A informação aparece com finalidade, escopo, `as_of` e estado explícitos; dados de clientes e de outros recebedores fora do vínculo permanecem restritos. Assim, a loteadora permite que o parceiro acompanhe o impacto de atraso e as projeções sem abrir a carteira integral da empresa. [37]

Após autenticação, o CRM resolve a identidade contra um **Grant de Portal** ativo, datado e auditável. Se houver um único contexto individual ou de grupo, a pessoa entra diretamente em sua visão; se existirem vários contextos autorizados, escolhe entre eles sem consolidar valores/objetos por padrão. Nome, e-mail, URL, filtro, ID de contrato ou ID de lote não escolhem escopo. A landing ocorre por rota interna fixa, e cada leitura revalida organização, grant, vigência, vínculo/grupo e objeto no banco/serviço. Ausência, expiração, revogação ou erro de sessão falham sem enumerar dados de outro parceiro. [39]

Para investimento ou compra/venda de participação com pagamentos em marcos, o CRM registra cada parcela do compromisso como prevista, aguardando condição, exigível, vencida/em atraso, recebida em análise, conciliada, renegociada, cancelada ou em disputa. A classificação jurídica, contábil e fiscal do negócio — e a data em que eventual titularidade produz efeitos — deve ser aprovada pelos responsáveis habilitados conforme o instrumento. O CRM preserva fatos, evidências e cronologia; ele não determina natureza societária, tributação, escrituração ou distribuição automática. [38]
...
| **LOTEADORA** | Cadastro de Loteamentos; Estoque/Mapa de Lotes; **Sócios e Parceiros**; Clientes; Vendas/Contratos; Financeiro, boletos, carteira e busca protegida. | Cadastro cria a estrutura de gleba, empreendimento, fase, quadra e lote; cada quadra é matriz dos lotes (`Q12 · L1` até `Q12 · L100`). Estoque/Mapa opera os lotes criados. Sócios e Parceiros mantém cada contrato/negociação em vínculo próprio, com modalidade, objeto, instrumento, base, condição, vigência, documento e histórico. Registro, restrição, alocação, reserva, contrato, tabela e alçada definem elegibilidade; boleto é instrução, não caixa confirmado. |
